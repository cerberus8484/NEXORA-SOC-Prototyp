#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Nexora SOC — Firewall-Collector auf DIESEM Host installieren (idempotent).
#
# Wird vom Deployment Center über den Control-Adapter `ssh-firewall-collector`
...
#     ./deploy/install-firewall-collector.sh
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
SERVICE="nexora-firewall-collector"
BIN_PATH="/usr/local/bin/${SERVICE}"
CONF_DIR="/etc/nexora"
CONF_PATH="${CONF_DIR}/firewall-collector.env"

info(){ echo "==> [firewall-collector] $*"; }
die(){  echo "FEHLER: [firewall-collector] $*" >&2; exit 1; }
...
[Unit]
Description=Nexora Firewall-Collector
...
info "Firewall-Collector $COLLECTOR_VERSION installiert — Ziel: $NEXORA_INTAKE_URL"
