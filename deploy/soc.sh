...
PROFILE="${NEXORA_PROFILE:-$(grep -E '^NEXORA_PROFILE=' "$ENV_FILE" 2>/dev/null | tail -1 | cut -d= -f2- | tr -d '"'"'"' ' || true)}"
PROFILE_ARGS=()
...
case "$cmd" in
  ...
  up)
    # immer detached
    docker compose -f "$COMPOSE" --env-file "$ENV_FILE" "${PROFILE_ARGS[@]}" up -d "$@"
    rc=$?
    ...
    if [ "$rc" -eq 0 ] && docker compose -f "$COMPOSE" --env-file "$ENV_FILE" "${PROFILE_ARGS[@]}" ps --status running --services 2>/dev/null | grep -qx web; then
      echo "==> nginx neu starten (Upstream-IP neu auflösen)"
      docker compose -f "$COMPOSE" --env-file "$ENV_FILE" "${PROFILE_ARGS[@]}" restart web >/dev/null 2>&1 || true
    fi
    exit "$rc"
    ;;
  *)
    exec docker compose -f "$COMPOSE" --env-file "$ENV_FILE" "${PROFILE_ARGS[@]}" "$cmd" "$@"
    ;;
esac
