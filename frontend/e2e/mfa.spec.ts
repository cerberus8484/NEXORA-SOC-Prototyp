...
test.describe('Login — MFA-Challenge-Flow', () => {
  test('MFA-User sieht Code-Eingabe statt Dashboard nach Passwort-Login', async ({ page }) => {
    ...
    await page.getByLabel('E-Mail').fill(MFA_EMAIL);
    await page.getByLabel('Passwort').fill(MFA_PASSWORD);
    await page.getByRole('button', { name: 'Anmelden' }).click();

    // Dashboard wird NICHT angezeigt — stattdessen MFA-Challenge
    await expect(page).toHaveURL(/\/login$/);
    await expect(page.getByText('Zwei-Faktor-Bestätigung erforderlich')).toBeVisible();
    await expect(page.getByLabel('Bestätigungscode')).toBeVisible();
  });
  ...
  test('„Zurück"-Button im MFA-Schritt kehrt zur Passwort-Eingabe zurück', async ({ page }) => {
    ...
    await expect(page.getByText('Zwei-Faktor-Bestätigung erforderlich')).not.toBeVisible();
  });
});
