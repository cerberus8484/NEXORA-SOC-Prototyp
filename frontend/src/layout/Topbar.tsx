  ...
  return (
    <header className="topbar">
      <div className="topbar-left">
        <button className="topbar-icon-btn" onClick={onMenu} aria-label={t('nav.menu')}>
          <Menu size={18} />
        </button>
        {pathname !== '/dashboard' && (
          <button
      ...
      <div className="topbar-right">
        <div className="sys-status" title={t('nav.systemStatus')}>
          <span className="status-dot" />
          <div className="sys-status-text">
            <span className="ss-label">{t('nav.systemStatus')}</span>
            <span className="ss-value">{t('nav.allSystemsNormal')}</span>
          </div>
        </div>
        ...
        <span className="topbar-clock mono">
          {now.toLocaleTimeString(i18n.language === 'de' ? 'de-DE' : 'en-GB', { hour: '2-digit', minute: '2-digit' })}
        </span>

        <div className="topbar-version">
          <span className="tv-version mono">{APP_VERSION}</span>
          <span className="tv-date mono">{now.toLocaleDateString(i18n.language === 'de' ? 'de-DE' : 'en-GB')}</span>
        </div>
        ...
        <div ref={menuRef} style={{ position: 'relative' }}>
          ...
          >
            ...
            <div className="user-meta">
              <span className="user-name">{user?.displayName || user?.email || t('nav.analyst')}</span>
          ...
          {menuOpen && (
            <div style={menuStyles.panel} role="menu" aria-label={t('nav.userMenuLabel')}>
              <div style={menuStyles.header}>
                <div style={menuStyles.headerName}>{user?.displayName || user?.email || t('nav.analyst')}</div>
