...
function errorText(err: unknown): string {
  ...
  return 'Unbekannter Fehler beim Laden der Metriken.';
}
...
export function SocMetricsDashboardPage() {
  ...
  if (!allowed) {
    return (
      <div>
        <SectionHeader title="SOC-Metriken" subtitle="Team-Performance & Detection-Qualität" />
        <Card><CardBody><EmptyState icon={<ShieldOff size={28} />} title="Zugriff verweigert" message="SOC-Metriken sind Engineer/Admin vorbehalten." /></CardBody></Card>
      </div>
    );
  }
  ...
  return (
    <div>
      <SectionHeader title="SOC-Metriken" subtitle="Team-Performance & Detection-Qualität (MTTR · FP-Rate · Last)" help="soc-metrics"
        actions={
      ...
      {error ? (
        <Card><CardBody><EmptyState icon={<Timer size={28} />} title="Konnte Metriken nicht laden" message={error} /></CardBody></Card>
      ) : loading || !data ? (
        <Card><CardBody><Spinner label="Metriken werden berechnet …" /></CardBody></Card>
      ) : (
        <>
          <div className="grid grid-kpi" style={{ marginBottom: 16 }}>
            ...
            <StatCard label="FP / klassifiziert geschlossen" value={formatRate(data.fpRate.rate)} tone={fpTone(data.fpRate.rate)} icon={<ShieldOff size={18} />} hint={`${data.fpRate.fpCount}/${data.fpRate.classifiedCount} klassifiziert`} />
            <StatCard label="Offen" value={String(data.byState.OPEN ?? 0)} tone="warning" icon={<Layers size={18} />} hint={`${data.byState.CLOSED ?? 0} geschlossen`} />
            <StatCard label="Tickets gesamt" value={String(data.meta.totalTickets)} tone="accent" icon={<Layers size={18} />} hint={data.meta.capped ? 'Stichprobe' : 'vollständig'} />
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <Panel title="Top-Rules (häufigste Offenses)" icon={<Crosshair size={15} style={{ color: 'var(--accent)' }} />}>
              {data.topRules.length === 0 ? <Dim>Keine Daten.</Dim> : (
            ...
            <Panel title="Last pro Analyst" icon={<Users size={15} style={{ color: 'var(--accent)' }} />}>
              {data.analystLoad.length === 0 ? <Dim>Keine Zuweisungen.</Dim> : (
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead><tr>{['Analyst', 'Gesamt', 'Offen'].map((h) => <th key={h} style={TH}>{h}</th>)}</tr></thead>
                  <tbody>
                    {data.analystLoad.map((a) => (
                      <tr key={a.analyst}>
                        <td style={{ ...TD, fontFamily: 'var(--font-mono)' }}>{a.analyst === '_unassigned' ? '(nicht zugewiesen)' : a.analyst}</td>
            ...
            <Panel title="Nach Status" icon={<Layers size={15} style={{ color: 'var(--text-muted)' }} />}>
              <BarList items={Object.entries(data.byStatus).map(([k, v]) => ({ label: k, value: v }))} tone="muted" />
            </Panel>

            <Panel title="Nach State" icon={<Layers size={15} style={{ color: 'var(--text-muted)' }} />}>
