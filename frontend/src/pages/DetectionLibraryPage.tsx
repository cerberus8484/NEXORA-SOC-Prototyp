...
export function DetectionLibraryPage() {
  ...
  async function handleCreate() {
    if (!form.description.trim()) { setSaveError(tr('app.descriptionRequired')); return; }
    if (!form.fieldValue.trim()) { setSaveError(tr('app.fieldValueRequired')); return; }
    const lvl = Number(form.level);
    if (!Number.isInteger(lvl) || lvl < 1 || lvl > 15) { setSaveError(tr('app.levelMustBetween115')); return; }
    ...
    try {
      ...
      setSaveSuccess(tr('detection.ruleSaved', { rule: r.ruleId, file: r.file }));
    } catch (e) {
      setSaveError(e instanceof Error ? e.message : tr('app.errorWhileSaving'));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div>
      <SectionHeader title="Detection Library" help="detection" subtitle={tr('text.loadedWazuhManagerRulesDetection')}
        actions={
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <Badge tone={enabled ? 'success' : 'muted'}>{enabled ? `${total} Rules` : tr('app.wazuhApiNotConnected')}</Badge>
            {canAct && <Button variant="primary" size="sm" icon={<Plus size={14} />} onClick={openModal} disabled={!enabled} title={!enabled ? tr('app.wazuhApiNotConnectedRule') : undefined}>{tr('label.newRule')}</Button>}
          </div>
        }
      />
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius)', width: '100%', maxWidth: 560, maxHeight: '90vh', overflow: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: '1px solid var(--border-soft)' }}>
              <span style={{ fontWeight: 700, fontSize: 14 }}>{tr('label.newCustomDetectionRule')}</span>
            ...
            <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontSize: 11.5, color: 'var(--text-dim)', background: 'var(--bg-input)', border: '1px solid var(--border-soft)', borderRadius: 4, padding: '8px 10px' }}>{tr('analysis.ruleStoredIn')}<code style={{ fontFamily: 'var(--font-mono)' }}>nexora-custom-detections.xml</code>{tr('analysis.onManagerIdRange')}</div>

              <div>
                <div style={SUBLABEL}>{tr('common.descriptionRequired')}</div>
                <input style={INPUT} value={form.description} onChange={upd('description')} placeholder={tr('app.suspiciousPowershellExecutionEncodedCommand')} />
              </div>
              ...
              <div>
                <div style={SUBLABEL}>{tr('deploy.fieldNameRequired')}</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '130px 1fr', gap: 10 }}>
                ...
                <div>
                  ...
                  <input style={{ ...INPUT, fontFamily: 'var(--font-mono)' }} value={form.fieldValue} onChange={upd('fieldValue')}
                    placeholder={FIELD_EXAMPLES[form.fieldName] ?? tr('app.searchTermRegularExpression')} />
                </div>
              </div>

              <div>
                <div style={SUBLABEL}>{tr('app.additionalGroupsCommaSeparatedOptional')}</div>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', paddingTop: 4 }}>
                <Button variant="ghost" onClick={() => setShowModal(false)}>{tr('common.cancel2')}</Button>
                {!saveSuccess
                  ? <Button variant="primary" onClick={() => void handleCreate()} disabled={saving}>{saving ? 'Speichert …' : tr('text.createRule')}</Button>
                  : <Button variant="ghost" onClick={() => setShowModal(false)}>{tr('common.close')}</Button>}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-kpi" style={{ marginBottom: 16 }}>
        ...
        <StatCard label="MITRE-Techniken" value={data ? String(kpis.techniques) : '—'} tone="accent" icon={<Target size={18} />} hint={tr('text.page')} />
        <StatCard label="Custom (≥100000)" value={data ? String(kpis.custom) : '—'} tone="accent" icon={<Hash size={18} />} hint={tr('text.page')} />
        <StatCard label="High (Level ≥12)" value={data ? String(kpis.high) : '—'} tone="danger" icon={<ShieldAlert size={18} />} hint={tr('text.page')} />
      </div>
        ...
        <CardBody style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end', padding: '12px 14px' }}>
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--bg-input)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-sm)', padding: '0 8px' }}>
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={tr('detection.searchPlaceholder')}
          ...
          <div>
            ...
            <select value={ruleType} onChange={(e) => setRuleType(e.target.value as 'all' | 'detection' | 'fp')}
              ...
              <option value="detection">{tr('text.detectionOnly')}</option>
              <option value="fp">{tr('text.falsePositiveExceptionsOnly')}</option>
            </select>
          </div>
      ...
      {error ? (
        <Card><CardBody><EmptyState icon={<Library size={28} />} title={tr('app.couldNotLoadRules')} message={tr('app.pleaseTryAgainLater')} /></CardBody></Card>
      ) : !enabled ? (
        <Card><CardBody><EmptyState icon={<Library size={28} />} title={tr('app.wazuhApiNotConnected')} message={tr('app.onceWazuhManagerApiPort')} /></CardBody></Card>
      ) : !data || (loading && !data.length) ? (
        <Card><CardBody><Spinner label={tr('app.loadingRules')} /></CardBody></Card>
      ) : rows.length === 0 ? (
        <Card><CardBody><EmptyState icon={<Library size={28} />} title={tr('text.noRules')} message={tr('app.noMatchesCurrentFilters')} /></CardBody></Card>
      ) : (
        <Card>
          <CardBody style={{ padding: 0 }}>
                ...
                {rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td style={{ ...TD, maxWidth: 420 }}>
                      {isFpRule(r) && <span style={{ marginRight: 6 }}><Badge tone="warning">{tr('analysis.fpException')}</Badge></span>}
                      {r.description}
                    </td>
                    <td style={TD}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', borderTop: '1px solid var(--border-soft)' }}>
              <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>{total} Rules · Seite {page}/{pages}{loading ? ' ' + tr('app.loading') : ''}</span>
              <span style={{ display: 'flex', gap: 6 }}>
                <Button variant="ghost" size="sm" icon={<ChevronLeft size={14} />} disabled={offset === 0} onClick={() => setOffset(Math.max(0, offset - PAGE))}>{tr('common.back')}</Button>
      ...
      <div style={{ marginTop: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--text-dim)', marginLeft: 4 }}>
            — {tr('app.publishedDetectionSnapshots')}
          </span>
        </div>

        {publishedLoading ? (
          <Card><CardBody><Spinner label={tr('app.loadingNexoraDetections')} /></CardBody></Card>
        ) : !publishedDetections || publishedDetections.length === 0 ? (
          <Card>
            <CardBody>
              <EmptyState
                icon={<BookOpen size={26} />}
                title={tr('app.noPublishedDetectionsYet')}
                message={tr('app.onceUseCaseDraftPasses')}
              />
            </CardBody>
          </Card>
        ) : (
          <Card>
            <CardBody style={{ padding: 0 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Title', 'Language', 'Severity', 'MITRE', 'Source', tr('analysis.rulePreview')].map((h) => (
