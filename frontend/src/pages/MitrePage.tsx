    ...
    Promise.allSettled([detectionApi.list({ limit: 500 }), huntApi.catalog()])
      .then(([rulesRes, huntsRes]) => {
        ...
        if (rulesRes.status === 'rejected' && huntsRes.status === 'rejected') {
          ...
          setError(e instanceof Error ? e.message : i18n.t('text.loadingFailed2'));
          return;
        }
  ...
  return (
    <div style={s.page}>
      <SectionHeader
        ...
        subtitle={enabled ? tr('app.coverageFromWazuhRulesPlus') : tr('app.wazuhNotConnectedCoverageFrom')}
        onBack={() => navigate(-1)}
        help="mitre"
      />
      ...
      <div style={s.summary}>
        ...
        <div style={s.kpi}>
          ...
          <div style={s.kpiLbl}>{tr('app.gaps')}</div>
        </div>
        <div style={s.kpi}>
      ...
      <div style={s.legend}>
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <Circle size={13} style={{ color: 'var(--text-dim)' }} />{tr('detection.noRule')}</span>
        {!enabled && <Badge tone="warning">{tr('app.wazuhOfflineLiveCoverageNot')}</Badge>}
      </div>
            ...
            <div key={tactic.id} style={s.tactic}>
              ...
              <div style={{ marginTop: 10 }}>
                {tactic.techniques.map((tech) => {
                  ...
                  return (
                    <div key={tech.id} style={s.row} title={isCovered ? tr('mitre.isDetected', { id: tech.id }) : tr('mitre.noRule', { id: tech.id })}>
                      {isCovered
