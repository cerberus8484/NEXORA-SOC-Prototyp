...
export function ThreatHuntsPage() {
  ...
  async function quickLaunch(item: HuntCatalogItem) {
    ...
    try {
      ...
      notify(e instanceof Error ? e.message : 'Start fehlgeschlagen');
  ...
  async function handleCancel() {
    ...
    catch (e) { notify(e instanceof Error ? e.message : 'Cancel fehlgeschlagen'); }
  }
  ...
  function exportFindingsReport() {
    ...
    const md = [
      ...
      ...findings.flatMap((f) => [
        ...
        f.description || '_keine Beschreibung_',
  ...
  function exportFindingsCsv() {
    if (!selSession) return;
    if (findings.length === 0) { notify('Keine Findings zum Exportieren.'); return; }
  ...
  async function findingAction(kind: 'ticket' | 'evidence', fid: string) {
    ...
    try {
      if (kind === 'ticket') {
        ...
        if (newTicketId) {
          ...
          setToast({ text: 'Ticket-Erstellung angestoßen.' });
        }
      } else {
    ...
    } catch (e) { setToast({ text: e instanceof Error ? e.message : 'Aktion fehlgeschlagen' }); }
  }
  ...
  async function requestAction(kind: string, command?: string, reason?: string) {
    ...
    try {
      ...
      notify('Anfrage gestellt — wartet auf Genehmigung (Vier-Augen).');
    } catch (e) { notify(e instanceof Error ? e.message : 'Anfrage fehlgeschlagen'); }
  }
  ...
  async function decideAction(actionId: string, decision: 'approve' | 'reject', detail?: string) {
    ...
    try {
      ...
      await refreshSelected();
    } catch (e) { notify(e instanceof Error ? e.message : 'Entscheidung fehlgeschlagen'); }
  }
  ...
  async function executeAction(actionId: string, reauthPassword: string) {
    ...
    try {
      ...
      await refreshSelected();
      notify('Containment ausgeführt.');
    } catch (e) {
      notify(e instanceof Error ? e.message : 'Ausführung fehlgeschlagen');
  ...
  async function resetResponseCircuit() {
    ...
    } catch (e) { notify(e instanceof Error ? e.message : 'Entsperren fehlgeschlagen'); }
  }
  ...
  async function runCmd(cmd?: string) {
    ...
    try {
      ...
      notify(e instanceof Error ? e.message : 'Command nicht erlaubt');
  ...
  function makeRdpFile(host: string, ip?: string) {
    ...
    notify('RDP-Datei erzeugt — kein Credential-Handling, nutze sie verantwortungsvoll.');
  }
  function openRdp() {
            ...
            {!sel ? (
              <div style={{ ...s.panel, padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--text-dim)' }}>· vorgefertigte Hunts — ein Klick startet</span>
                </div>
                <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 12 }}>Wähle einen Hunt oder eine laufende Session links. Die Console zeigt echte Backend-Logs.</div>
                {/* „leer" ≠ „Fehler": Ladefehler ehrlich anzeigen, nicht ewig „wird geladen". */}
                {catalogError ? (
                  <div style={{ fontSize: 12.5, color: 'var(--danger)' }}>Hunt-Katalog nicht ladbar — Abruf fehlgeschlagen. Dies ist kein leerer Katalog; bitte später erneut versuchen.</div>
                ) : catalog === null ? (
                  <div style={{ fontSize: 12.5, color: 'var(--text-dim)' }}>Katalog wird geladen …</div>
                ) : catalog.length === 0 ? (
                  <div style={{ fontSize: 12.5, color: 'var(--text-dim)' }}>Kein Hunt im Katalog verfügbar.</div>
                ) : (
              ...
              <>
                ...
                {pollDegraded && (
                  <div style={{ marginBottom: 14 }}>
                    <ErrorCard message="Live-Aktualisierung fehlgeschlagen — Backend nicht erreichbar. Angezeigte Daten sind möglicherweise veraltet." />
                  </div>
                )}
                  ...
                  <>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                      ...
                      <div style={{ ...s.panel, padding: 16 }}>
                        ...
                        {findings.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Noch keine Findings.</div> :
                ...
                {section === 'console' && (
                  <div style={{ ...s.panel, padding: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                      ...
                      <Button variant="ghost" size="sm" icon={<Square size={13} />} onClick={() => void handleCancel()} disabled={!canCancel || (sel.status !== 'active' && sel.status !== 'planned')} title={!canCancel ? 'Nur für Admins' : undefined}>Stop</Button>
                    ...
                    <div style={{ ...s.console, height: 420 }} ref={consoleRef}>
                      {logs.length === 0
                        ? <div style={{ color: 'var(--text-dim)' }}>Noch keine Logs …</div>
                        : logs.map((l) => (
                          <div key={l.id}>
                  ...
                  <div style={s.panel}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      ...
                      <tbody>
                            ...
                            <td style={s.td}>
                              <span style={{ display: 'inline-flex', gap: 4, color: 'var(--text-dim)' }}>
                                ...
                                {canAct && (
                                  ...
                                  <button type="button" aria-label="Ticket erstellen" style={s.iconBtn}
                                    onClick={(e) => { e.stopPropagation(); void findingAction('ticket', f.id); }}>
                                    <Ticket size={14} />
                                  </button>
                                )}
                                {canAct && (
                                  <button type="button" aria-label="Als Evidence sichern" style={s.iconBtn}
                        ...
                        {findings.length === 0 && <tr><td colSpan={8} style={{ ...s.td, color: 'var(--text-dim)', textAlign: 'center' }}>Keine Findings</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
                ...
                {section === 'timeline' && (
                  <div style={{ ...s.panel, padding: 16 }}>
                    ...
                    {chain.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Noch keine Ereignisse.</div> :
                      chain.map((c, i) => {
                  ...
                  <div style={{ ...s.panel, padding: 16 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                      ...
                      <input value={cmdInput} onChange={(e) => setCmdInput(e.target.value)}
                        ...
                        disabled={!canAct}
                        placeholder={canAct ? 'Safe command eingeben (z.B. netstat -ano)…' : 'Nur für Analyst+'}
                    ...
                    <div style={{ ...s.console, height: 300 }}>
                      {commands.length === 0
                        ? <div style={{ color: 'var(--text-dim)' }}>Noch keine Commands ausgeführt.</div>
                  ...
                  <div style={{ ...s.panel, padding: 16 }}>
                    ...
                    {targetHosts.map((th) => (
                      <div key={th.host} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--border-soft)' }}>
                        ...
                        <Button variant="ghost" size="sm" onClick={() => makeRdpFile(th.host)}>RDP-Datei</Button>
                      </div>
                    ))}
                    <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 10 }}>
                      Agent-Status/OS folgen mit der Host-Inventar-Anbindung. RDP erzeugt nur eine .rdp-Datei — keine Credentials, kein Auto-Connect.
                    </div>
                  </div>
                )}
                  ...
                  <div style={s.panel}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      ...
                      <tbody>
                        ...
                        {artifacts.length === 0 && <tr><td colSpan={4} style={{ ...s.td, color: 'var(--text-dim)', textAlign: 'center' }}>Noch keine Artifacts — „Add to Evidence Center" am Finding erzeugt Evidence mit Chain of Custody.</td></tr>}
                      </tbody>
                    </table>
                    <div style={{ padding: '10px 16px', fontSize: 11.5, color: 'var(--text-dim)' }}>
                      Über Findings gesicherte Evidence liegt im <Link to="/evidence" style={{ color: 'var(--accent)' }}>Evidence Center</Link> (append-only, SHA-256).
                    </div>
                  </div>
                )}
                ...
                {section === 'tickets' && (
                  <div style={s.panel}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead><tr>{['Linked At', 'Ticket', 'Quelle', 'Summary'].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr></thead>
                      <tbody>
                        {ticketLinks.map((l) => (
                          <tr key={l.id}>
                        ...
                        {ticketLinks.length === 0 && <tr><td colSpan={4} style={{ ...s.td, color: 'var(--text-dim)', textAlign: 'center' }}>Keine Ticket-Verknüpfungen — „Create Ticket" am Finding erzeugt eine.</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
                ...
                {section === 'export' && (
                  <div style={{ ...s.panel, padding: 16 }}>
                    ...
                    <div style={{ fontSize: 11.5, color: 'var(--text-dim)', marginTop: 12, lineHeight: 1.6 }}>
                      <b style={{ color: 'var(--text-muted)' }}>Findings Report</b>: Hunt-Kopf + alle Findings mit MITRE und Empfehlung.<br />
                      <b style={{ color: 'var(--text-muted)' }}>Evidence Bundle</b>: Session, Findings, Commands, Artifacts, Logs und Ticket-Links als ein JSON-Paket.<br />
                      <b style={{ color: 'var(--text-muted)' }}>Console Log</b>: alle Live-Console-Zeilen mit Zeitstempel und Level.
                    </div>
                  </div>
                )}
              </>
              ...
              {showPanel && selFinding ? (
                <FindingDetailPanel
                  ...
                  onVerdictSet={(updated) => {
                    notify(`Verdict '${updated.verdict || '—'}' gespeichert.`);
                ...
                <div style={{ ...s.panel, padding: 24, textAlign: 'center', color: 'var(--text-dim)' }}>
                  ...
                  <div style={{ fontSize: 12, marginTop: 4 }}>Wähle ein Finding in der Tabelle, um Details, MITRE-Kontext und Aktionen zu sehen.</div>
                </div>
              )}
            </div>
          )}
