...
function pageTitle(slug: string): string {
  if (!slug) return i18n.t('wiki.internalTitle');
  return getWikiManualPage(slug)?.title ?? i18n.t('app.wikiEntryNotFound');
}

function pageSummary(slug: string): string {
  if (!slug) return i18n.t('app.manualNexoraClearInternalWithout');
  return getWikiManualPage(slug)?.summary ?? i18n.t('text.thereNoFullyWrittenInternal');
}
...
function QuickStartVisual() {
  ...
  const steps = [
    {
      title: tr('app.1OpenDashboard'),
      body: tr('app.whereYouFirstSeeWhether'),
    ...
    {
      title: tr('app.2OpenTickets'),
      body: tr('app.pullOpenPrioritisedCasesFront'),
    ...
    },
    {
      ...
      body: tr('app.readSummarySourceStatusFirst'),
    ...
    {
      title: tr('app.4CheckEvidence'),
      body: tr('app.onlyThenMoveIntoEvidence'),
  ...
  return (
    <Card style={s.sectionCard}>
      <div style={s.sectionHeader}>
        <h3 style={s.sectionTitle}>{tr('app.quickStartPictures')}</h3>
...
function DeploymentVisual() {
  ...
  return (
    <Card style={s.sectionCard}>
      <div style={s.sectionHeader}>
        <h3 style={s.sectionTitle}>{tr('wiki.exampleDeployFields')}</h3>
        <Badge tone="warning">{tr('app.tooltips')}</Badge>
      </div>
      <div style={s.visualGrid}>
        <div style={s.visualCard}>
          ...
          <div style={s.demoShell}>
            ...
            <div style={s.demoFields}>
              ...
              <div style={s.demoField}>
                <div style={s.demoLabel}>{tr('deploy.targetNode')} <span style={s.bulb}><Lightbulb size={11} /></span></div>
              ...
              <div style={{ ...s.demoField, ...s.demoFieldWide }}>
                <div style={s.demoLabel}>{tr('app.passwordConfirmation')} <span style={s.bulb}><Lightbulb size={11} /></span></div>
                <div style={{ ...s.demoInput, ...s.demoInputMuted }}>{tr('settings.currentPassword')}</div>
              </div>
            </div>
            <div style={s.demoButton}>{tr('deploy.addConnector')}</div>
          </div>
          <div style={s.visualSub}>{tr('app.meantLookLikeRealForm')}</div>
        </div>
        <div style={s.visualCard}>
          <div style={s.visualHead}>
            <div style={s.visualTitle}>{tr('app.whatTooltipsExplain')}</div>
            ...
            <div style={s.demoHintList}>
              <div style={s.demoHintItem}>
                <div style={s.demoHintCopy}>
                  ...
                  <div style={s.demoHintBody}>{tr('app.realManagementAddressProxmoxHost')}</div>
                </div>
                <span style={s.bulb}><Lightbulb size={14} /></span>
              </div>
              <div style={s.demoHintItem}>
                <div style={s.demoHintCopy}>
                  <div style={s.demoHintTitle}>{tr('deploy.targetNode')}</div>
                  <div style={s.demoHintBody}>{tr('misc.exactNodeNameFromProxmox')}</div>
                </div>
                <span style={s.bulb}><Lightbulb size={14} /></span>
              </div>
              <div style={s.demoHintItem}>
                <div style={s.demoHintCopy}>
                  <div style={s.demoHintTitle}>{tr('app.passwordConfirmation')}</div>
                  <div style={s.demoHintBody}>{tr('app.securityStepBeforeSavingWithout')}</div>
                </div>
                <span style={s.bulb}><Lightbulb size={14} /></span>
              </div>
            </div>
          </div>
          <div style={s.visualSub}>{tr('app.cardRightExplainsNotJust')}</div>
        </div>
      </div>
    </Card>
  );
      ...
      <div style={s.hero}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <Badge tone="accent" dot>{tr('wiki.internalManual')}</Badge>
            <Badge>{tr('text.noIframe')}</Badge>
          </div>
          <h1 style={s.heroTitle}>{tr('app.operationPeopleNotMachines')}</h1>
          <p style={s.heroCopy}>{tr('wiki.introNote')}</p>
        </div>
        <div style={s.heroPanel}>
          <div style={s.heroPanelLabel}>{tr('text.whatAbout')}</div>
          <div style={s.heroPanelValue}>{tr('app.internalManualRealOperation')}</div>
          <p style={{ ...s.sideText, marginTop: 2 }}>{tr('wiki.understandFirst')}</p>
        </div>
      </div>
      ...
      <div style={s.layout}>
        <aside style={s.sideWrap}>
          <Card className="card-pad">
            <div style={s.sideTitle}>{tr('text.howUseWiki')}</div>
            <p style={s.sideText}>{tr('app.findTopicLeftOpenEntry')}</p>
          </Card>
    ...
    <>
      <Card className="card-pad" style={s.summaryCard}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ fontSize: 17, fontWeight: 700, color: 'var(--text)' }}>{tr('text.whatYouFindHere')}</div>
        </div>
        <p style={s.articleLead}>{tr('wiki.whatYouFind')}</p>
        <div style={s.keyFacts}>
          <div style={s.keyFact}>
            <div style={s.keyLabel}>{tr('app.newcomers')}</div>
            <div style={s.keyValue}>{tr('misc.startFirstStepsTicketsEvidence')}</div>
          </div>
          <div style={s.keyFact}>
            <div style={s.keyLabel}>{tr('app.administrators')}</div>
            <div style={s.keyValue}>{tr('app.workThroughIntegrationsSecurityServices')}</div>
          </div>
        </div>
      </Card>
  ...
  return (
    <>
      <Card className="card-pad" style={s.summaryCard}>
        ...
        <div style={s.chips}>
          <Badge tone="accent">{tr('app.stepStep')}</Badge>
          <Badge>{tr('nav.internalOperation')}</Badge>
          <Badge>{tr('text.visualAids')}</Badge>
        </div>
      </Card>
      ...
      <Card style={s.sectionCard}>
        <div style={s.sectionHeader}>
          ...
          <Badge tone="warning">{tr('text.readFirst')}</Badge>
        </div>
      ...
      {page.sections.map((section) => (
        <Card key={section.title} style={s.sectionCard}>
          <div style={s.sectionHeader}>
            ...
            {section.steps ? <Badge tone="success">Ablauf</Badge> : <Badge>{tr('text.note')}</Badge>}
          </div>
      ...
      {page.related && page.related.length > 0 && (
        <Card style={s.sectionCard}>
          <div style={s.sectionHeader}>
            ...
            <Badge tone="accent">{tr('app.nextSteps')}</Badge>
          </div>
          <div style={s.relatedWrap}>
...
function WikiMissing() {
  ...
  return (
    <Card className="card-pad">
      <EmptyState
        title={tr('text.wikiEntryStillMissing')}
        message={tr('app.internalManualPageTopicHas')}
      />
    </Card>
  );
}
