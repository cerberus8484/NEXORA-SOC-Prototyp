...
// Der Filterzustand darf NIE der angezeigte Text sein: nach einem Sprach-
// wechsel verglich 'Alle' gegen 'All' und der Filter griff nicht mehr.
// ALL ist ein stabiler Wert; uebersetzt wird ausschliesslich beim Rendern.
const ALL = '*';
const CATEGORIES = [ALL, 'Persistence', 'Lateral Movement', 'Execution', 'Network', 'Exfiltration', 'Defense Evasion'];
...
export function HuntLibraryPage() {
  ...
  useEffect(() => {
    huntApi.catalog()
      ...
      .catch(() => setError(i18n.t('ui.catalogCouldNotLoaded')))
  ...
  async function launch(item: HuntCatalogItem) {
    ...
    try {
      ...
      setLaunchError(e instanceof Error ? e.message : tr('common.startFailed'));
    } finally {
      setLaunching('');
    }
  }
  ...
  const categoryChips = [ALL, ...CATEGORIES.filter((c) => c !== ALL && usedCategories.includes(c))];
  ...
  return (
    <div>
      <SectionHeader
        title="Hunt Library"
        subtitle={tr('hunts.prebuiltCount', { count: catalog.length })}
        help="hunts"
      ...
      <div style={s.filters}>
        <div style={{ position: 'relative' as const, flex: 1, minWidth: 200 }}>
          ...
          <input style={{ ...s.search, paddingLeft: 30 }} placeholder={tr('hunts.searchPlaceholder')} value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
          <Filter size={13} style={{ color: 'var(--text-dim)' }} />
          {RISKS.map((r) => (
            <button key={r} style={{ ...s.chip, ...(riskFilter === r ? s.chipOn : {}) }} onClick={() => setRiskFilter(r)}>
              {r === ALL ? tr('hunts.riskAll') : r}
            </button>
          ))}
        </div>
      ...
      <div style={s.grid}>
        {visible.length === 0 && (
          <div style={s.empty}>{tr('text.noHuntsFoundAdjustFilter')}</div>
        )}
        {visible.map((item) => (
          <div key={item.key} style={s.card}>
            ...
            <div style={s.cardFoot}>
              ...
              >
                {launching === item.key ? 'Starte …' : tr('hunts.start')}
              </Button>
            </div>
          </div>
        ))}
