    ...
    <div style={s.overlay} role="dialog" aria-modal="true" aria-labelledby="cp-title"
      ...
      <div style={s.modal}>
        <div style={s.modalHead}>
          <span id="cp-title" style={{ fontWeight: 700, fontSize: 14 }}>Enrollment-Profil anlegen</span>
          <button type="button" aria-label="Schließen" onClick={onClose} style={s.closeBtn}><X size={16} /></button>
        </div>
        <div style={s.modalBody}>
          <div style={s.formGroup}>
            <label htmlFor="cp-name" style={s.label}>{formLabel('Name *', (
              <ExampleHint
                title="Interner Name des Enrollment-Profils"
                text="Der Name beschreibt spaeter, fuer welchen Node-Typ dieses Profil gedacht ist."
                exampleLabel="Typischer Profilname"
                rows={[
                  { label: 'Sensor', value: 'branch-sensor-lab' },
                  { label: 'Connector', value: 'tenant-a-connector' },
                ]}
                footer="Nimm einen Namen, an dem man Rolle und Einsatzort direkt erkennt."
              />
            ))}</label>
          ...
          <div style={s.formGroup}>
            <label htmlFor="cp-role" style={s.label}>{formLabel('Rolle *', (
              <ExampleHint
                title="Welche Aufgabe die spaetere Node bekommt"
                text="Die Rolle bestimmt, wie die Node im System einsortiert und spaeter bewertet wird."
                exampleLabel="Einfacher Start"
                rows={[
                  { label: 'Standard-Agent', value: 'Normal Agent' },
                  { label: 'Netzsicht', value: 'Network Sensor' },
                ]}
                footer="Waehle die Rolle nach dem echten Einsatzzweck, nicht nach dem Hostnamen."
              />
            ))}</label>
          ...
          <div style={s.formGroup}>
            <label style={s.label}>{formLabel('Capabilities (read-only)', (
              <ExampleHint
                title="Welche Faehigkeiten das Profil mitgeben soll"
                text="Hier legst du fest, welche technischen Rechte oder Funktionspakete die Node spaeter melden darf."
                exampleLabel="Typische Mischung"
                rows={[
                  { label: 'Basis', value: 'heartbeat, inventory' },
                  { label: 'Sensor', value: 'netflow, packet-metadata' },
                ]}
                footer="Nur das aktivieren, was die Node im Betrieb wirklich braucht."
              />
            ))}</label>
            <div style={s.capGrid}>
      ...
      <div style={{ ...s.modal, maxWidth: 520 }}>
        ...
        <div style={s.modalBody}>
          <div style={{ fontSize: 12.5, color: 'var(--warning)', marginBottom: 10 }}>
            Dieser Token wird <strong>nur einmal</strong> angezeigt. Jetzt kopieren und sicher hinterlegen —
            danach ist er nicht mehr abrufbar (im Backend liegt nur der Hash).
          </div>
          ...
          {detail && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
                ...
                <Field label="Rolle">{ROLE_LABELS[detail.node.role]}</Field>
              ...
              {credentials.length === 0 && <span style={s.hint}>Keine Credentials.</span>}
              {credentials.map((c) => (
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
                {detail.capabilities.length === 0 && <span style={s.hint}>Keine gemeldet.</span>}
              ...
              {detail.heartbeats.length === 0 && <span style={s.hint}>Noch keine Heartbeats.</span>}
  ...
  if (!can.admin(user?.role)) {
    return (
      <div style={{ ...s.page, paddingTop: 48, textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 600 }}>Zugriff verweigert</div>
...
function ProvisioningContent() {
  ...
  async function handleMint(profileId: string) {
    try {
      ...
      showToast(e instanceof Error ? e.message : 'Token-Erzeugung fehlgeschlagen');
    }
  }
    ...
    <div style={s.page}>
      ...
      <div style={s.head}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{ fontSize: 12.5, color: 'var(--text-dim)', marginTop: 2 }}>
            Enrollte Nodes, ihr Heartbeat-Zustand und Enrollment-Profile. Rein lesend — Apply/Remote gibt es bewusst nicht.
          </div>
        </div>
      ...
      <div style={s.panel}>
        <div style={s.panelHead}>
          ...
          <div style={{ marginLeft: 'auto' }}>
            <Button variant="primary" size="sm" icon={<Plus size={13} />} onClick={() => { setCreateError(''); setCreateOpen(true); }}>
              Profil anlegen
            </Button>
          </div>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Name', 'Rolle', 'Capabilities', 'Status', 'Aktion'].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {profiles.length === 0 && (
                <tr><td colSpan={5} style={{ ...s.td, textAlign: 'center', color: 'var(--text-dim)', padding: 24 }}>
                  Noch keine Enrollment-Profile. „Profil anlegen" startet die Registrierung.
                </td></tr>
              )}
              {profiles.map((p) => (
                <tr key={p.id}>
                  ...
                  <td style={s.td}>
                    <Button variant="ghost" size="sm" icon={<KeyRound size={12} />}
                      disabled={p.status !== 'active'} onClick={() => void handleMint(p.id)} title="Token erzeugen">
                      Token
                    </Button>
                  </td>
                </tr>
      ...
      <div style={s.panel}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Name', 'Rolle', 'IP', 'Version', 'Status', 'Zuletzt gesehen'].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {nodes.length === 0 && (
