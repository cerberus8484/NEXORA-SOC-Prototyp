    ...
    <Card style={{ marginBottom: 14 }}>
      ...
      <CardBody>
        ...
        {node.collectors.length === 0 ? (
          <EmptyState title="Keine Kollektoren gemeldet" message="Dieser Knoten hat keine Collector-Zustände im Snapshot." />
        ) : (
  ...
  return (
    <div style={s.page}>
      <SectionHeader
        title="Data-Plane"
        help="dataplane"
        subtitle="Read-only — Live-Status der Korrelations-Pipeline (Collector-Hub · Intake · Outbox)"
        actions={
        ...
        agg && agg.nodes > 0 ? (
          <>
            <div style={s.kpis}>
              ...
              <Kpi icon={<Inbox size={18} />} tone="purple" value={agg.intake.total.toLocaleString('de-DE')} label="Intake gesamt" />
              <Kpi icon={<Send size={18} />} tone="warning" value={agg.outbox.pending.toLocaleString('de-DE')} label="Outbox pending" />
              <Kpi icon={<AlertTriangle size={18} />} tone={agg.collectorsFailed + agg.outbox.failed > 0 ? 'danger' : 'muted'} value={agg.collectorsFailed + agg.outbox.failed} label="Fehler (Coll.+Outbox)" />
            </div>
          ...
          <EmptyState
            title="Noch kein Data-Plane-Knoten gemeldet"
            message="Sobald ein Knoten seinen Status über die Brücke meldet, erscheint hier seine Pipeline-Gesundheit."
          />
        )
      )}
    </div>
