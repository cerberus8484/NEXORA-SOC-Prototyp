...
import { TimelineInspector, TimelineView } from '../features/analysis/timeline/TimelineView';
import { buildTimelineGroups } from '../features/analysis/timeline/timelineModel';
...
import { readInvestigationEvent, readInvestigationTab, writeInvestigationEvent, writeInvestigationTab } from '../features/analysis/investigationUrlState';
...
export function AnalysisPage() {
  ...
  const section = readInvestigationTab(params);
  const selectedTimelineEventId = readInvestigationEvent(params);
  const setSection = useCallback((tab: DeckSection) => {
    setParams((current) => writeInvestigationTab(current, tab), { replace: true });
  }, [setParams]);
  const setSelectedTimelineEvent = useCallback((eventId: string) => {
  ...
  const timelineGroups = useMemo(() => active ? buildTimelineGroups(active, evidence, timeline, networkCorr) : [], [active, evidence, timeline, networkCorr]);
  const selectedTimelineEvent = useMemo(
    () => section === 'timeline' ? timelineGroups.find((group) => group.id === selectedTimelineEventId) ?? null : null,
    [section, selectedTimelineEventId, timelineGroups],
  );
  ...
  async function createFollowUp() {
    ...
    try {
      ...
      setActionError(e instanceof Error ? e.message : tr('ui.followUpTicketCouldNot'));
    } finally {
  ...
  async function addKiNote(text: string) {
    if (!active) return;
    await mutate({ notes: appendNote(active.notes ?? '', text, { label: tr('label.aiAssessmentApplied'), at: new Date() }) });
  }
  ...
  async function deleteTicket() {
    ...
    if (!(await confirm({
      title: tr('analysis.deleteTicketQuestion', { label }),
      message: tr('ui.ticketWillDeletedPermanentlyAction'),
      confirmLabel: tr('ui.deletePermanently'),
      danger: true,
    }))) return;
    setBusy(true);
    setError('');
    try {
      ...
      setError(e instanceof Error ? e.message : tr('ui.deletionFailed'));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function saveAnalysis() {
    ...
    try {
      ...
      setSaveError(e instanceof Error ? e.message : tr('ui.analysisCouldNotSaved'));
  ...
  async function saveAnalystState(nextState: AnalystState) {
    ...
    try {
      ...
      setSaveError(e instanceof Error ? e.message : tr('ui.checklistPlaybookCouldNotSaved'));
    }
  }
  ...
  async function loadCrossRefs() {
    ...
    catch (e) { setActionError(e instanceof Error ? e.message : tr('ui.crossReferenceSearchFailed')); } finally { setXrefBusy(false); }
  }
  ...
  async function enrichIocs() {
    ...
    try {
      ...
      setIocReps(m);
    } catch (e) { setActionError(e instanceof Error ? e.message : tr('ui.iocReputationCouldNotLoaded')); } finally { setIocBusy(false); }
  }
  ...
  async function enrichTi() {
    ...
    catch (e) { setActionError(e instanceof Error ? e.message : tr('analysis.tiLookupFailed')); } finally { setTiBusy(false); }
  }
  ...
  async function exportEvidence() {
    ...
    try {
      ...
      a.click();
    } catch (e) { setActionError(e instanceof Error ? e.message : tr('analysis.evidenceExportFailed')); }
  }
  ...
  async function saveEvidenceSnapshot() {
    ...
    try {
      ...
      setEvSaved(true);
    } catch (e) { setActionError(e instanceof Error ? e.message : tr('ui.evidenceSnapshotCouldNotSaved')); }
  }
  ...
  async function saveTiEvidence() {
    ...
    try {
      ...
      setTiSaved(true);
    } catch (e) { setActionError(e instanceof Error ? e.message : tr('ui.threatIntelEvidenceCouldNot')); }
  }
  ...
  async function quickFp() {
    ...
    try {
      ...
      if (!r.ok) {
        setFpQuickMsg({ ok: false, text: (r.errors?.length ? r.errors.join(' · ') : tr('ui.scopeIncomplete')) + ' ' + tr('ui.pleaseCompleteEditor') });
        setExceptionOpen(true);
        return;
      }
      const text = r.action === 'applied'
        ? (r.restartRequired ? tr('ui.falsePositiveRuleWrittenValidated') : tr('text.falsePositiveRuleApplied'))
        : r.idempotent
          ? tr('label.falsePositiveRuleAlreadyExists')
          : tr('ui.falsePositiveRuleCreatedForwarded');
      ...
      const text = apiErrorText(e, tr('text.falsePositiveRuleFailed'));
      const isValidation = e instanceof ApiError && e.status >= 400 && e.status < 500 && e.status !== 401;
      setFpQuickMsg({ ok: false, text: isValidation ? tr('analysis.addSelectorHint', { text }) : text });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      ...
      {!tickets && !error && <Spinner label={tr('ui.loadingTickets')} />}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1 }}>
          ...
          {actionError && (
            <div role="alert" style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, color: 'var(--danger)', background: 'var(--danger-soft)', border: '1px solid var(--danger)', borderRadius: 'var(--radius-sm)', padding: '8px 10px' }}>
              ...
              <button onClick={() => setActionError('')} aria-label={tr('ui.dismissError')} style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', fontSize: 14, lineHeight: 1 }}>✕</button>
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 300px', gap: 18, alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0 }}>
              {section === 'timeline' && <TimelineView t={active} ev={evidence} tl={timeline} network={networkCorr} loading={tlLoading} selectedEventId={selectedTimelineEventId} onSelectEvent={setSelectedTimelineEvent} />}
              ...
              <Card style={{ padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <Button variant="ghost" size="sm" disabled={savingAnalysis || !canAct} onClick={() => void saveAnalysis()}>{analysisSaved ? tr('common.savedCheckSuffix') : tr('common.save')}</Button>
                </div>
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={5}
                  placeholder={tr('ui.observationsHypothesesNextStepsSaved')}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {section === 'timeline' && <TimelineInspector event={selectedTimelineEvent} onOpenNetwork={() => setSection('network')} />}
              ...
              <Card style={{ padding: '12px 14px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <Button variant="ghost" size="sm" disabled={!canAct || evSaved} onClick={() => void saveEvidenceSnapshot()}>{evSaved ? tr('analysis.savedAsEvidenceCheck') : 'Add as Evidence'}</Button>
                  <Button variant="ghost" size="sm" onClick={() => void exportEvidence()}>{tr('analysis.exportEvidenceJson')}</Button>
                </div>
              </Card>
              ...
              <Card style={{ padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  {relatedCases.length === 0 && <Button variant="ghost" size="sm" disabled={xrefBusy} onClick={() => void loadCrossRefs()}>{xrefBusy ? 'Suche …' : tr('common.searchVerb')}</Button>}
                </div>
                {relatedCases.length === 0 ? (
                  <div style={{ fontSize: 11.5, color: 'var(--text-dim)' }}>{tr('ui.noTicketsFoundSameIocs')}</div>
                ) : relatedCases.map((r) => (
      ...
      {tickets && all.length === 0 && (canAdmin
        ? <EmptyState title={tr('text.noTicketsAvailable2')} message={tr('ui.thereCurrentlyNoTicketsSystem')} />
        : <EmptyState title={tr('text.noTicketsAssignedYou')} message={tr('ui.analysisWorkbenchShowsOnlyYour')} />)}
      {tickets && all.length > 0 && !active && <EmptyState title={tr('ui.noTicketSelected')} message={tr('ui.pickTicketFromSwitcherHeader')} />}
    ...
    <div style={{ padding: '6px 0', borderBottom: '1px solid var(--border-soft)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ display: 'flex', gap: 3 }}>
          ...
          <button style={btn} title={saved ? tr('analysis.savedAsEvidence') : 'Add as Evidence'} disabled={!ticketId || saved} onClick={() => void addEvidence()}>{saved ? <Check size={14} style={{ color: 'var(--success)' }} /> : <Plus size={14} />}</button>
      ...
      {err && <div style={{ fontSize: 11, color: 'var(--danger)', marginTop: 4 }}>{tr('analysis.enrichmentFailed')}</div>}
      {saveErr && <div role="alert" style={{ fontSize: 11, color: 'var(--danger)', marginTop: 4 }}>{tr('ui.savingEvidenceFailed')}</div>}
      {res && (
...
function CrossRefBadge({ refs, onSelect }: { refs: { id: string; ticketNr: string; title: string; priority: string }[]; onSelect: (id: string) => void }) {
  ...
  return (
    <span style={{ position: 'relative', display: 'inline-flex' }}>
      <button onClick={() => setOpen((v) => !v)} title={tr('analysis.showCrossReferences')}
        style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, display: 'inline-flex' }}>
        <Badge tone="warning"><GitBranch size={10} /> {refs.length}</Badge>
      </button>
      {open && (
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
      ...
      <Sec title="Summary" icon={<FileText size={15} style={{ color: 'var(--accent)' }} />} style={{ gridColumn: '1 / -1' }}>
        <p style={{ margin: 0, fontSize: 13, color: 'var(--text)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{t.description || t.category || tr('text.noDescriptionRecordedTicket')}</p>
        <div style={{ marginTop: 8, fontSize: 12, color: 'var(--text-dim)' }}>{tr('common.sourceColon')} <strong style={{ color: 'var(--text-muted)' }}>{t.source || 'manual'}</strong>{t.offenseId ? ` · Offense ${t.offenseId}` : ''}</div>
      </Sec>
      ...
      <Sec title="Extracted IoCs" icon={<AlertTriangle size={15} style={{ color: 'var(--warning)' }} />} style={{ gridColumn: '1 / -1' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', marginBottom: 10 }}>
          ...
          {hasIp && <Button variant="ghost" size="sm" icon={<Globe size={13} />} disabled={p.iocBusy} onClick={p.onEnrichIocs}>{p.iocBusy ? tr('ui.checking') : tr('ui.checkReputation')}</Button>}
        </div>
        {iocs.length === 0 ? <span style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('text.noIocsTicket')}</span>
          : shownIocs.length === 0 ? <span style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('text.noIocsType')}</span>
      ...
      <Sec title="Command Deck" icon={<Terminal size={15} style={{ color: 'var(--success)' }} />}>
        ...
        <textarea value={p.cmdOut} onChange={(e) => p.setCmdOut(e.target.value)} placeholder={tr('ui.pasteOutputHereNoReal')} style={{ width: '100%', minHeight: 60, marginTop: 6, background: 'var(--bg-input)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-sm)', color: 'var(--text)', padding: 8, fontSize: 11.5, fontFamily: 'var(--font-mono)' }} />
        <Button variant="ghost" size="sm" icon={cmdSaved ? <Check size={13} /> : <Plus size={13} />} disabled={!p.cmdOut.trim() || cmdSaved} onClick={() => void addCmdEvidence()} style={{ marginTop: 6 }}>{cmdSaved ? tr('analysis.savedAsEvidence') : 'Add Output as Evidence'}</Button>
        {cmdErr && <div role="alert" style={{ fontSize: 11, color: 'var(--danger)', marginTop: 4 }}>{tr('ui.commandOutputCouldNotSaved')}</div>}
      </Sec>

      <Sec title="Evidence-backed Rationale" icon={<ListChecks size={15} style={{ color: 'var(--accent)' }} />}>
        {p.bullets.length === 0 ? <span style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('text.noEvidenceBackedFactsDerived')}</span> :
        ...
        <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 6 }}>{tr('misc.derivedFromEvidenceTimelineFull')}</div>
      </Sec>

      <Sec title="Recommended Blocks / Watchlist" icon={<ShieldAlert size={15} style={{ color: 'var(--danger)' }} />}>
        {blocks.length === 0 ? <span style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('ui.noMaliciousSuspiciousIocsBlock')}</span> :
        ...
        <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 6 }}>{tr('misc.watchlistRecordsIocNotesActive')}</div>
      </Sec>

      <Sec title="Recommendation" icon={<ShieldCheck size={15} style={{ color: 'var(--danger)' }} />}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
          {RECO_ACTIONS.map((a) => <button key={a} disabled title={tr('ui.actionFollowsNoRemoteExecution')} style={{ fontSize: 11, padding: '5px 10px', borderRadius: 'var(--radius-pill)', border: '1px solid var(--border-soft)', background: 'transparent', color: 'var(--text-muted)' }}>{a}</button>)}
        </div>
        <textarea value={p.reco} onChange={(e) => p.setReco(e.target.value)} placeholder={tr('ui.recommendationJustification')} style={{ width: '100%', minHeight: 56, background: 'var(--bg-input)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-sm)', color: 'var(--text)', padding: 8, fontSize: 12 }} />
        <Button variant="primary" size="sm" style={{ marginTop: 6 }} disabled={p.savingAnalysis} onClick={p.onSaveAnalysis}>{p.analysisSaved ? tr('common.savedCheck') : p.savingAnalysis ? tr('common.saving') : 'Save Recommendation'}</Button>
      </Sec>
      ...
      <Sec title="Decision" icon={<ShieldCheck size={15} style={{ color: 'var(--accent)' }} />} style={{ gridColumn: '1 / -1' }}>
        {p.kiFp?.isFp && p.decision !== 'fp' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 14, padding: '9px 12px', borderRadius: 'var(--radius-sm)', background: 'var(--accent-soft)', border: '1px solid var(--accent)' }}>
            ...
            <span style={{ fontSize: 12.5, color: 'var(--text)' }}>
              <strong>{tr('ui.aiAssessmentFalsePositive')}</strong> ({p.kiFp.confidence}% Confidence){p.kiFp.reason ? ` — ${p.kiFp.reason}` : ''}
            </span>
            <Button variant="ghost" size="sm" style={{ marginLeft: 'auto' }}
              onClick={() => { p.setDecision('fp'); if (!p.fpReason.trim() && p.kiFp?.reason) p.setFpReason(p.kiFp.reason); }}>{tr('common.apply')}</Button>
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 10 }}>
          <Button variant="primary" size="sm" icon={<FileText size={13} />} disabled={p.savingAnalysis} onClick={p.onSaveAnalysis}>{p.savingAnalysis ? 'Speichert …' : tr('ui.saveAnalysis')}</Button>
          {p.analysisSaved && <span style={{ fontSize: 12, color: 'var(--success)' }}>{tr('common.savedCheckLower')}</span>}
          <span style={{ fontSize: 11, color: 'var(--text-dim)', marginLeft: 'auto' }}>{tr('ui.decisionConfidenceRecommendationNotesPersist')}</span>
        </div>
        ...
        {p.decision === 'fp' && (
          <div style={{ marginTop: 14, padding: 14, borderRadius: 'var(--radius-sm)', background: 'var(--bg-card-soft)', border: '1px solid var(--danger)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>{tr('ui.falsePositiveJustificationAmpActions')}</span>
            </div>
            <label style={{ ...LABEL, display: 'block', marginBottom: 5 }}>{tr('ui.justificationRequired2')}</label>
            <textarea value={p.fpReason} onChange={(e) => p.setFpReason(e.target.value)} placeholder={tr('ui.whyFalsePositiveEG')}
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 12 }}>
              <Button variant="danger" icon={<XCircle size={14} />} disabled={!p.canAct || p.busy || !p.fpReason.trim()} title={!p.fpReason.trim() ? tr('ui.justificationRequired') : ''} onClick={p.onCloseFp}>{tr('ui.closeFalsePositive')}</Button>
              <Button variant="primary" icon={<ShieldCheck size={14} />} disabled={!p.canAct || p.fpQuickBusy || !p.fpReason.trim()} title={!p.fpReason.trim() ? tr('ui.justificationRequired') : tr('text.oneClickCreateRuleFrom')} onClick={p.onQuickFp}>{p.fpQuickBusy ? tr('common.generating') : tr('text.createFalsePositiveRule')}</Button>
              <Button variant="ghost" icon={<ShieldCheck size={14} />} onClick={p.onPrepareException}>{tr('analysis.refineExceptionInEditor')}</Button>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 10 }}><Trans i18nKey="analysis.fpWorkflowHint" components={{ c: <code />, b: <strong /> }} /></div>
          </div>
        )}
      </Sec>
    </div>
...
const categoryTone = (): Record<string, { label: string; color: string }> => ({
  facts: { label: i18n.t('ui.confirmed'), color: 'var(--success)' },
  indicators: { label: 'Indikator', color: 'var(--warning)' },
  gaps: { label: i18n.t('ui.gap'), color: 'var(--danger)' },
  assessment: { label: i18n.t('common.assessment'), color: 'var(--purple)' },
  actions: { label: i18n.t('ui.action'), color: 'var(--accent)' },
  traceability: { label: i18n.t('common.source'), color: 'var(--text-muted)' },
  ...
  return (
    <Card style={{ padding: '14px 16px' }}>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        <Button variant="ghost" size="sm" onClick={() => { onSendToTicket(md); triggerSent(); }}>{sent ? tr('ui.attachedTicket') : 'Send to Ticket System'}</Button>
      </div>
      <div data-testid="incident-report-preview" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {incident.sections.map((s) => {
          const tone = categoryTone()[s.category ?? 'meta'] ?? categoryTone().meta;
          return (
