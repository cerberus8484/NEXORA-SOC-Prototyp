...
export function YaraPage() {
  ...
  async function handleScan() {
    ...
    } catch (e) { setScanError(e instanceof Error ? e.message : tr('yara.errors.scan')); }
  ...
  async function handleToggle(rule: YaraRule) {
    ...
    catch (e) { setActionError(e instanceof Error ? e.message : tr('yara.updateFailed')); }
  }

  async function handleDelete(id: string) {
    setActionError('');
    if (!(await confirm({
      title: tr('yara.deleteQuestion'),
      message: tr('common.cannotBeUndone'),
      confirmLabel: tr('common.delete'),
      danger: true,
    }))) return;
    try { await yaraApi.delete(id); load(); }
    catch (e) { setActionError(e instanceof Error ? e.message : tr('yara.deleteFailed')); }
  }

  async function handleCreate() {
    if (!fName.trim() || !fPatternVal.trim()) { setFormError(tr('yara.nameAndPatternRequired')); return; }
    ...
    try {
      ...
      load();
    } catch (e) { setFormError(e instanceof Error ? e.message : tr('common.error')); }
  ...
  return (
    <div style={s.page}>
      <SectionHeader
        title="YARA Engine"
        subtitle={tr('yara.subtitle')}
        help="yara"
        actions={canAct && <Button variant="primary" size="sm" icon={<Plus size={14} />} onClick={() => setShowForm((v) => !v)}>{tr('label.newRule')}</Button>}
      />
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
        {[[tr('yara.rulesTotal'), rules.length, 'var(--text)'], [tr('common.active'), active, 'var(--accent)'], [tr('text.inactive'), rules.length - active, 'var(--text-dim)']].map(([l, v, c]) => (
      ...
      <div style={s.grid}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {showForm && (
            <Card>
              <CardBody>
                <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12, color: 'var(--text)' }}>{tr('label.newRule')}</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <div><div style={s.label}>{tr('common.description')}</div><input style={s.input} value={fDesc} onChange={(e) => setFDesc(e.target.value)} placeholder="Optional" /></div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: 8 }}>
                    <div><div style={s.label}>{tr('yara.patternType')}</div>
                    ...
                    <div><div style={s.label}>{tr('yara.patternRequired')}</div><input style={{ ...s.input, fontFamily: 'var(--font-mono)' }} value={fPatternVal} onChange={(e) => setFPatternVal(e.target.value)} placeholder={fPatternType === 'regex' ? '\\bpassword\\b' : fPatternType === 'hex' ? '4D 5A' : 'mimikatz'} /></div>
                  </div>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text)', cursor: 'pointer' }}>
                    <input type="checkbox" checked={fNocase} onChange={(e) => setFNocase(e.target.checked)} />{tr('yara.ignoreCase')}</label>
                  {formError && <div style={{ fontSize: 12, color: 'var(--danger)' }}>{formError}</div>}
                  <div style={{ display: 'flex', gap: 8 }}>
                    <Button variant="primary" onClick={() => void handleCreate()} disabled={saving}>{saving ? '…' : tr('common.save')}</Button>
                    <Button variant="ghost" onClick={() => setShowForm(false)}>{tr('common.cancel2')}</Button>
                  </div>
                </div>
              </CardBody>
            </Card>
          ...
          {rules.length === 0
            ? <EmptyState title={tr('yara.noRules')} />
            : rules.map((rule) => (
              <Card key={rule.id}>
                <div
                  role="button"
                ...
                >
                  ...
                  {canAct && <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-dim)', display: 'inline-flex' }} onClick={(e) => { e.stopPropagation(); void handleToggle(rule); }} title={rule.enabled ? tr('common.disable') : tr('common.enable')}>
                    {rule.enabled ? <ToggleRight size={16} style={{ color: 'var(--accent)' }} /> : <ToggleLeft size={16} />}
                  </button>}
                  {isAdmin && <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--danger)', display: 'inline-flex' }} onClick={(e) => { e.stopPropagation(); void handleDelete(rule.id); }} title={tr('common.delete')}>
                    <Trash2 size={14} />
                  </button>}
                </div>
                {expanded === rule.id && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Card>
            <CardBody>
              ...
              <div style={s.label}>{tr('yara.inputPlaceholder')}</div>
              ...
              <Button variant="primary" icon={<Play size={13} />} onClick={() => void handleScan()} disabled={!scanInput.trim() || scanning} style={{ width: '100%', justifyContent: 'center' }}>
                {scanning ? 'Scanne …' : tr('yara.scanAgainstActive', { count: rules.filter(r => r.enabled).length })}
              </Button>
                ...
                <div style={{ marginTop: 14 }}>
                  <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 8 }}>
                    {scanResult.scannedRules} Regel(n) geprüft — {scanResult.matches.length === 0
                      ? <span style={{ color: 'var(--success)' }}>{tr('yara.noMatch')}</span>
