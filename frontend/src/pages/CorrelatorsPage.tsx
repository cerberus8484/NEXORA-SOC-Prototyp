    ...
    return (
      <div style={{ ...s.page, paddingTop: 48, textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-dim)', marginTop: 6 }}>
          Diese Seite ist ab der Analyst-Rolle zugänglich.
        </div>
      </div>
    );
  }
      ...
      <div style={s.head}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{ fontSize: 12.5, color: 'var(--text-dim)', marginTop: 2 }}>
            Bekannte Correlatoren, ihr Queue-Zustand und die genehmigungspflichtige Konfiguration.
            Rein lesend + kontrollierter Config-Lifecycle — Apply/Restart/Remote gibt es bewusst nicht.
          </div>
        </div>
      ...
      <div style={s.panel}>
        <div style={s.panelHead}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <select aria-label="Risiko-Filter" style={s.select} value={riskFilter} onChange={(e) => setRiskFilter(e.target.value as RiskClass | 'all')}>
              {RISK_OPTIONS.map((r) => <option key={r} value={r}>{r === 'all' ? 'Alle' : riskLabel(r as RiskClass)}</option>)}
            </select>
            <label style={{ fontSize: 11.5, color: 'var(--text-dim)' }}>Quelle</label>
            <select aria-label="Quellen-Filter" style={s.select} value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value)}>
              {sources.map((src) => <option key={src} value={src}>{src === 'all' ? 'Alle' : src}</option>)}
            </select>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Correlator', 'Engine', 'Inputs / Outputs', 'Risiko', 'Letzte Aktivität', 'Queue'].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={6} style={{ ...s.td, textAlign: 'center', color: 'var(--text-dim)', padding: 24 }}>
                  Keine Correlatoren für die aktuelle Filterung.
                </td></tr>
              )}
              {filtered.map((c) => (
