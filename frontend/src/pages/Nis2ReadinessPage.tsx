...
function AssessmentForm({ detail, onSave, busy }: { detail: Nis2ControlDetail; onSave: (b: UpdateAssessmentBody) => void; busy: boolean }) {
  ...
  return (
    <div>
      ...
      <div style={s.group}>
        <label htmlFor="n2-due" style={s.label}>{tr('app.due')}</label>
        <input id="n2-due" type="date" style={s.input} value={form.dueDate ?? ''} onChange={(e) => set('dueDate', e.target.value)} />
      </div>
      <div style={s.group}>
        <label htmlFor="n2-review" style={s.label}>{tr('app.lastChecked')}</label>
        <input id="n2-review" type="date" style={s.input} value={form.lastReviewedAt ?? ''} onChange={(e) => set('lastReviewedAt', e.target.value)} />
      </div>
      <div style={s.group}>
        <label htmlFor="n2-notes" style={s.label}>Notizen {form.status === 'not_applicable' && <span style={{ color: 'var(--warning)' }}>{tr('app.justificationRequired')}</span>}</label>
      ...
      <Button variant="primary" size="sm" icon={busy ? undefined : <Check size={13} />} disabled={busy} onClick={() => onSave(form)}>
        {busy ? tr('common.saving') : tr('app.saveAssessment')}
      </Button>
    </div>
  );
}
...
function EvidenceForm({ onAdd, busy, error }: { onAdd: (b: AddEvidenceBody) => void; busy: boolean; error: string }) {
  ...
  return (
    <div style={{ marginTop: 10, borderTop: '1px solid var(--border-soft)', paddingTop: 10 }}>
      <div style={{ ...s.label, marginBottom: 8 }}>{tr('app.addEvidence')}</div>
      <div style={s.group}>
      ...
      <div style={s.group}>
        <input style={s.input} value={form.evidenceRef} maxLength={1024} onChange={(e) => set('evidenceRef', e.target.value)} placeholder={tr('app.referenceUrlNoSecretsHttp')} />
      </div>
      ...
      <Button variant="ghost" size="sm" icon={<Plus size={13} />} disabled={busy || !form.title.trim() || !form.evidenceRef.trim()} onClick={() => onAdd(form)}>
        {busy ? tr('app.adding') : 'Evidence verlinken'}
      </Button>
    </div>
  );
}
...
function IncidentLinkForm({ onLink, busy, error }: { onLink: (ticketId: string) => void; busy: boolean; error: string }) {
  ...
  return (
    <div style={{ marginTop: 10, borderTop: '1px solid var(--border-soft)', paddingTop: 10 }}>
      <div style={{ ...s.label, marginBottom: 8 }}>{tr('app.linkIncidentTicket')}</div>
      <div style={{ ...s.hint, marginBottom: 8 }}>{tr('app.noteTicketTitleStoredEvidence')}</div>
      <div style={s.group}>
        <select style={s.input} value={ticketId} onChange={(e) => setTicketId(e.target.value)}>
          <option value="">{tr('app.chooseIncident')}</option>
          {tickets.map((t) => (
      ...
      <Button variant="ghost" size="sm" icon={<Link2 size={13} />} disabled={busy || !ticketId} onClick={() => onLink(ticketId)}>
        {busy ? tr('app.linking') : tr('app.linkIncidentEvidence')}
      </Button>
    </div>
  );
}
...
function DetailPanel({ controlKey, isAdmin, onChanged }: { controlKey: string; isAdmin: boolean; onChanged: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    catch (e) { setError(e instanceof Error ? e.message : i18n.t('text.loadingFailed2')); }
  }, [controlKey]);
  ...
  async function saveAssessment(body: UpdateAssessmentBody) {
    ...
    catch (e) { setError(e instanceof Error ? e.message : tr('common.savingFailed2')); }
  ...
  async function addEvidence(body: AddEvidenceBody) {
    ...
    catch (e) { setEvError(e instanceof Error ? e.message : tr('evidence.failed')); }
    finally { setBusy(false); }
  }
  async function removeEvidence(id: string) {
    setBusy(true);
    try { await nis2Api.removeEvidence(id); await load(); onChanged(); }
    catch (e) { setError(e instanceof Error ? e.message : tr('common.removeFailed')); }
    finally { setBusy(false); }
  }
  async function linkIncident(ticketId: string) {
    ...
    catch (e) { setLinkErr(e instanceof Error ? e.message : tr('app.linkingFailed')); }
    finally { setBusy(false); }
  }
      ...
      <div style={s.detailBody}>
        ...
        {addressedNoEvidence && (
          <div style={{ ...s.warn, marginBottom: 12 }}>
            ...
            <span>{tr('app.statusHandledButNoEvidence')} <strong>{tr('app.notCompleteEvidence')}</strong>{tr('app.pleaseAddEvidenceAdjustStatus')}</span>
          </div>
        )}
        ...
        {detail.evidence.map((ev) => (
          <div key={ev.id} style={s.evItem}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {isAdmin && (
                <button type="button" aria-label={tr('evidence.remove')} disabled={busy} onClick={() => void removeEvidence(ev.id)}
        ...
        {isAdmin ? (
          ...
          <div style={{ ...s.hint, marginTop: 12 }}>{tr('app.readOnlyViewChangesReserved')}</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <>
      <div style={s.kpis}>
        ...
        <KpiCard icon={<AlertTriangle size={13} />} label={tr('app.reviewNeeded')} value={summary.needsReview} valueColor={summary.needsReview ? 'var(--warning)' : undefined} />
        <KpiCard icon={<Clock size={13} />} label={tr('app.overdue')} value={summary.overdue} valueColor={summary.overdue ? 'var(--danger)' : undefined} />
        <KpiCard icon={<RefreshCw size={13} />} label={tr('app.reviewDue')} value={summary.reviewDue} valueColor={summary.reviewDue ? 'var(--warning)' : undefined} />
      </div>
      <div style={s.panel}>
        <div style={s.panelHead}>
        ...
        <div style={{ padding: '12px 16px' }}>
          ...
          <div style={{ ...s.label, marginBottom: 6 }}>{tr('metrics.statusDistribution')}</div>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>{['Control', 'Status', 'Owner', tr('app.due2'), 'Evidence', 'Incidents', 'Signal'].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {controls.map((c) => {
                  const sigLabel = c.overdue ? tr('app.overdue2') : c.needsReview ? 'review' : c.missingEvidence ? tr('app.noEvidence') : 'ok';
                  return (
                    <tr key={c.key}>
...
export function Nis2ReadinessPage() {
  ...
  async function load() {
    ...
    } catch (e) { setLoadError(e instanceof Error ? e.message : tr('text.loadingFailed2')); }
  }
  useEffect(() => { void load(); }, []);
      ...
      <div style={s.layout}>
        <div style={s.panel}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>{['Control', 'Status', 'Evidence', 'Owner', tr('app.due2'), tr('app.lastChecked'), 'Signal'].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {rows.map((c) => {
        ...
        {selected
          ...
          : <div style={s.panel}><div style={s.detailBody}><div style={s.hint}>{tr('app.chooseControlFromRegistrySee')}</div></div></div>}
      </div>
      </>)}
    </div>
  );
