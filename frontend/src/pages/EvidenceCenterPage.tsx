...
export function EvidenceCenterPage() {
  ...
  function selectRow(id: string) {
    ...
    evidenceApi.get(id)
      ...
      .catch((e: unknown) => setActionError(e instanceof Error ? e.message : 'Evidence konnte nicht geladen werden'));
  }
  function refreshList() {
    evidenceApi.recent(500)
      .then((r) => setItems(r.data))
      .catch((e: unknown) => setActionError(e instanceof Error ? e.message : 'Liste konnte nicht aktualisiert werden'));
  }
  function doCustody(action: 'reviewed' | 'verified' | 'flagged') {
    ...
    evidenceApi.custody(sel.id, action)
      ...
      .catch((e: unknown) => setActionError(e instanceof Error ? e.message : `Custody-Aktion "${action}" fehlgeschlagen`));
  }
  function addNote(note: string) {
    ...
    evidenceApi.custody(sel.id, 'note', note.trim())
      .then((r) => { setSel(r.data); refreshList(); })
      .catch((e: unknown) => setActionError(e instanceof Error ? e.message : 'Notiz konnte nicht gespeichert werden'));
  }
  ...
  function exportEvidence() {
    ...
    evidenceApi.exportTicket(sel.ticketId)
      ...
      .catch((e: unknown) => setActionError(e instanceof Error ? e.message : 'Export fehlgeschlagen'));
  }
  ...
  return (
    <div>
      <SectionHeader title="Evidence Center" help="evidence" subtitle="Central overview of evidence items and chain of custody"
        onBack={() => navigate(-1)}
        actions={<Badge tone={items ? 'success' : 'muted'}>{items ? `${items.length} Items` : 'lädt …'}</Badge>} />
      ...
      {error ? (
        <Card><CardBody><EmptyState icon={<Archive size={28} />} title="Konnte Evidence nicht laden" message="Bitte später erneut versuchen oder Backend prüfen." /></CardBody></Card>
      ) : !items ? (
        <Card><CardBody><Spinner label="Evidence wird geladen …" /></CardBody></Card>
      ) : items.length === 0 ? (
        <Card><CardBody><EmptyState icon={<Archive size={28} />} title="Noch keine Evidence" message={'Sobald in der Analyse „Add enrichment as Evidence" / „Als Evidence sichern" genutzt wird (oder andere Quellen anliefern), erscheinen die Items hier.'} /></CardBody></Card>
      ) : (
      ...
      <Card>
        <CardBody style={{ padding: '12px 14px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
            ...
            {sel.ticketId && (
              <Button variant="ghost" size="sm" icon={<ExternalLink size={13} />} onClick={() => onJumpTicket(sel.ticketId)} title="Verknüpftes Ticket öffnen">Ticket</Button>
            )}
