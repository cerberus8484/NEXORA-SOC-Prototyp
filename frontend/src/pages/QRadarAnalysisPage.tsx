...
function relTime(iso?: string) {
  ...
  if (m < 60) return i18n.t('time.minutesAgoTight', { count: m });
  const h = Math.round(m / 60);
  return h < 24
    ? i18n.t('time.hoursAgoTight', { count: h })
    : i18n.t('time.daysAgoTight', { count: Math.round(h / 24) });
}
...
function QRadarLinkButton({ offenseId }: { offenseId: number }) {
  ...
  if (!QRADAR_BASE_URL) {
    return (
      <span title={tr('ui.qradarBaseUrlNotConfigured')}>
        <Button variant="ghost" size="sm" icon={<ExternalLink size={13} />} disabled>{tr('ui.openQradar')}</Button>
      </span>
    );
  }
  return (
    <Button variant="ghost" size="sm" icon={<ExternalLink size={13} />} onClick={() => openInQRadar(offenseId)}>{tr('ui.openQradar')}</Button>
  );
}
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
            {o.mitreT.length > 0
              ...
              : <span style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('ui.noTechniquesLinked')}</span>}
          </div>
        </div>
      </Card>
      ...
      <Card style={{ padding: '14px 16px' }}>
        ...
        <div style={{ marginTop: 10, fontSize: 11.5, color: 'var(--text-dim)' }}>{tr('text.detailedEventTimelineEventsTab')}</div>
      </Card>
    </div>
  );
}
...
function EventsTab({ offenseId }: { offenseId: number }) {
  ...
  if (state === 'loading') return <Card style={{ padding: '14px 16px' }}><Spinner label={tr('ui.loadingEvents')} /></Card>;
  if (state === 'error') return <Card style={{ padding: '14px 16px' }}><EmptyState icon={<AlertTriangle size={28} style={{ color: 'var(--danger)' }} />} title={tr('ui.eventsCannotLoaded')} message={tr('ui.fetchingQradarEventsFailedPlease')} /></Card>;
  if (state === 'empty') return <Card style={{ padding: '14px 16px' }}><EmptyState title={tr('text.noEvents2')} message={tr('ui.noEventsAvailableOffense')} /></Card>;
...
function IocRow({ ip }: { ip: string }) {
  ...
  async function enrich() {
    ...
    catch (e) { setEnrichError(e instanceof Error ? e.message : tr('analysis.tiLookupFailed')); }
...
function IoCsTab({ o }: { o: QRadarOffense }) {
  ...
  return (
    <Card style={{ padding: '14px 16px' }}>
      ...
      {ips.length === 0
        ? <EmptyState title={tr('text.noIps')} message={tr('text.noSourceDestinationIpsOffense')} />
        : ips.map((ip) => <IocRow key={ip} ip={ip} />)}
      <div style={{ marginTop: 10, fontSize: 11, color: 'var(--text-dim)' }}>{tr('text.threatIntelContextNotAutomatic')}</div>
    </Card>
  );
}
...
// ── NOTES Tab ──────────────────────────────────────────────────────────────
const checklist = () => [
  i18n.t('ui.sourceIpCheckedThreatIntel'),
  i18n.t('detection.logSourcesValidated'),
  i18n.t('ui.affectedHostsIsolatedChecked'),
  i18n.t('detection.mitreVerified'),
  i18n.t('ui.correlationOtherOffensesChecked'),
  'Eskalation entschieden',
  'Ticket in Nexora angelegt',
  i18n.t('reports.created'),
];
...
function NotesTab({ offenseId, canAct }: { offenseId: number; canAct: boolean }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSaveError(e instanceof Error ? e.message : tr('common.savingFailed2'));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
      ...
      <Card style={{ padding: '14px 16px' }}>
        ...
        {notesState === 'loading'
          ? <Spinner label={tr('ui.loadingNotes')} />
          : notesState === 'error'
            ? <div style={{ fontSize: 12, color: 'var(--danger)', marginBottom: 12 }}>{tr('ui.notesCannotLoadedRequestFailed')}</div>
            : notesState === 'empty'
            ? <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 12 }}>{tr('analysis.noNotesYet')}</div>
            : (
        ...
        {canAct && (
          <div style={{ marginTop: 6 }}>
            <textarea
              ...
              placeholder={tr('ui.newNoteOffense')}
              disabled={saving}
              maxLength={5000}
            />
        ...
        {!canAct && (
          <div style={{ fontSize: 11.5, color: 'var(--text-dim)', marginTop: 8 }}>{tr('analysis.notesNeedAnalyst')}</div>
        )}
      </Card>
...
function ReportTab({ o, canAct }: { o: QRadarOffense; canAct: boolean }) {
  ...
  const notesText = notesError
    ? tr('ui.notesCannotLoadedRequestFailed2')
    : notes.length > 0
  ...
  return (
    <Card style={{ padding: '14px 16px' }}>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {canAct && <Button variant="ghost" size="sm" disabled={busy} onClick={() => void create(o)}>{busy ? tr('common.creating') : 'Create Nexora Ticket'}</Button>}
...
function EnrichmentSidebar({ o, canAct }: { o: QRadarOffense | null; canAct: boolean }) {
  ...
  if (!o) return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <Card style={{ padding: '12px 14px' }}><EmptyState title={tr('text.noOffense')} message={tr('ui.selectOffense')} /></Card>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <Card style={{ padding: '12px 14px' }}>
        ...
        {!targetIp ? (
          <div style={{ fontSize: 11.5, color: 'var(--text-dim)' }}>{tr('ui.noSourceIpAvailable')}</div>
        ) : !tiResult ? (
          <>
          ...
          <>
            ...
            <Button variant="ghost" size="sm" disabled={busy} onClick={() => void enrich()} style={{ width: '100%' }}>{tr('analysis.checkAgain')}</Button>
          </>
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {canAct && (
            <Button variant="primary" size="sm" icon={<ChevronRight size={13} />} disabled={ticketBusy} onClick={() => void create(o)}>
              {ticketBusy ? tr('common.creating') : tr('text.createNexoraTicket')}
            </Button>
          )}
          {canAct && (
            <Button variant="ghost" size="sm" icon={<XCircle size={13} />} disabled={ticketBusy} onClick={() => void create(o, { asFalsePositive: true })}>{tr('tickets.closeAsFalsePositive')}</Button>
          )}
  ...
  return (
    <Card style={{ padding: '48px 24px', textAlign: 'center' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
        ...
        <div>
          <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>{tr('qradar.notConfigured')}</div>
...
export function QRadarAnalysisPage() {
  ...
  const statusBadge = connected === null
    ? null
    : connected
      ? <Badge tone="success">{tr('settings.connectedCaps')}</Badge>
      : <Badge tone="muted">{tr('text.notConnected2')}</Badge>;
    ...
    return (
      <div style={{ padding: '32px 20px' }}>
        <EmptyState
          ...
          message={tr('ui.qradarAnalysisCenterRequiresLeast')}
        />
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <Button variant="ghost" size="sm" icon={<RefreshCw size={13} />} onClick={load}>{tr('common.refresh')}</Button>
      </div>

      {error && <div style={{ marginBottom: 14 }}><ErrorCard message={error} /></div>}
      {loading && <Spinner label={tr('ui.loadingQradarOffenses')} />}
      ...
      {!loading && !error && connected && offenses && offenses.length === 0 && (
        <Card style={{ padding: '36px 16px' }}>
          <EmptyState
            title={tr('text.noOpenOffenses')}
            message={tr('ui.qradarConnectedThereCurrentlyNo')}
          />
        </Card>
      )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr 300px', gap: 14, flex: 1, alignItems: 'start' }}>
            ...
            <Card style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '72vh' }}>
              ...
              <div style={{ overflowY: 'auto', flex: 1 }}>
                {queue.length === 0
                  ? <div style={{ padding: 20, fontSize: 12, color: 'var(--text-dim)', textAlign: 'center' }}>{tr('text.noOffenses')}</div>
                  : queue.map((o) => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
              ...
              ) : <EmptyState title={tr('ui.noOffenseSelected')} message={tr('ui.pickOffenseFromQueueLeft')} />}
            </div>
