...
const statusFilter = () => [
  { value: '', label: i18n.t('label.allStatuses') },
...
const sourceFilter = () => [
  { value: '', label: i18n.t('analysis.allSources') },
...
export function TicketsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (myReq === reqId.current) setError(err instanceof Error ? err.message : i18n.t('common.error'));
    }
  }, [filters, myName]);
  ...
  async function mutate(ids: string[], body: Partial<Ticket> & Record<string, unknown>) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : tr('tickets.errors.action'));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : tr('tickets.deleteFailed'));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function selectAllFiltered() {
    ...
    try {
      ...
      setSelectAllError(err instanceof Error ? err.message : tr('tickets.selectionLoadFailed'));
    } finally {
  ...
  async function confirmBulkDelete() {
    ...
    try {
      ...
      setBulkError(err instanceof Error ? err.message : tr('tickets.bulkDeleteFailed'));
    } finally {
      setBusy(false);
      setBulkProgress(null);
    }
  ...
  return (
    ...
    <div onContextMenu={(e) => { if (ctx) e.preventDefault(); }}>
      <SectionHeader
        ...
        actions={canAct && (
          <Button variant="primary" icon={<Plus size={16} />} onClick={() => navigate('/tickets/new')}>{tr('label.newTicket')}</Button>
        )}
      />
      ...
      <Card style={{ padding: '12px 16px', marginBottom: 16, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ minWidth: 130 }}><Select options={stateFilter()} value={filters.state} onChange={(e) => patchFilters({ state: e.target.value })} /></div>
        <span style={{ ...filterLabel, marginLeft: 8 }}>Status</span>
        <div style={{ minWidth: 170 }}><Select options={statusFilter()} value={filters.status} onChange={(e) => patchFilters({ status: e.target.value })} /></div>
        <span style={{ ...filterLabel, marginLeft: 8 }}>{tr('common.priority')}</span>
        ...
        <span style={{ ...filterLabel, marginLeft: 8 }}>{tr('common.source')}</span>
        <div style={{ minWidth: 180 }}><Select options={sourceFilter()} value={filters.source} onChange={(e) => patchFilters({ source: e.target.value })} /></div>
        <div style={{ position: 'relative', marginLeft: 8, flex: '1 1 220px', maxWidth: 320 }}>
          ...
          <Input
            type="search"
            aria-label={tr('tickets.searchLabel')}
        ...
        {myName && (
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 8, cursor: 'pointer' }}>
            <Toggle checked={filters.mineOnly} onChange={(next) => patchFilters({ mineOnly: next })} label={tr('tickets.onlyMineLong')} />
            <span style={filterLabel}>{tr('tickets.onlyMine')}</span>
          </label>
        )}
        ...
        <Card style={{ padding: '10px 16px', marginBottom: 12, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', borderColor: 'var(--accent)' }}>
          ...
          {total > selected.size && (
            <Button variant="ghost" size="sm" disabled={busy || selectingAll} onClick={() => void selectAllFiltered()}>
              {selectingAll ? 'Lade …' : tr('tickets.selectAllFiltered', { total })}
            </Button>
          )}
          <Button variant="danger" size="sm" icon={<XCircle size={14} />} disabled={busy} onClick={closeSelected}>{tr('tickets.closeSelected')}</Button>
          {canDelete && (
            <Button variant="danger" size="sm" icon={<Trash2 size={14} />} disabled={busy} onClick={openBulkDelete}>{tr('tickets.deleteSelected')}</Button>
          )}
          <Button variant="ghost" size="sm" icon={<X size={14} />} onClick={() => setSelected(new Set())}>{tr('tickets.clearSelection')}</Button>
        </Card>
      )}
      ...
      {selectAllError && (
        <Card style={{ padding: '8px 14px', marginBottom: 12, display: 'flex', gap: 10, alignItems: 'center', borderColor: 'var(--warning)' }}>
          ...
          <Button variant="ghost" size="sm" icon={<X size={14} />} style={{ marginLeft: 'auto' }} onClick={() => setSelectAllError('')}>{tr('common.close')}</Button>
        </Card>
      )}
      ...
      {bulkNotice && (
        <Card style={{ padding: '8px 14px', marginBottom: 12, display: 'flex', gap: 10, alignItems: 'center', borderColor: bulkNotice.tone === 'ok' ? 'var(--accent)' : 'var(--warning)' }}>
          ...
          <Button variant="ghost" size="sm" icon={<X size={14} />} style={{ marginLeft: 'auto' }} onClick={() => setBulkNotice(null)}>{tr('common.close')}</Button>
        </Card>
      )}

      <Card>
        {error ? <ErrorCard message={error} /> : !tickets ? <Spinner /> : tickets.length === 0 ? (
          <EmptyState title={tr('tickets.none')} message={tr('tickets.noneForFilter')} />
        ) : (
          <>
            <table className="table">
              <thead>
                <tr>
                  {canAct && (
                    <th style={{ width: 36 }}>
                      <input type="checkbox" aria-label={tr('tickets.selectAll')}
                  ...
                  <th>Nr.</th><th>Titel</th><th>{tr('common.priority')}</th><th>State</th><th>Status</th><th>{tr('common.source')}</th><th>Analyst</th><th>Erstellt</th>
                    ...
                    <tr key={t.id}
                      ...
                      {canAct && (
                        <td onClick={(e) => { e.stopPropagation(); toggle(t.id); }}>
                          <input type="checkbox" aria-label={tr('tickets.selectTicket', { id: t.ticketNr || t.id })} checked={sel} onChange={() => toggle(t.id)} onClick={(e) => e.stopPropagation()} />
                        </td>
                      )}
                      <td className="mono" style={{ color: 'var(--text-muted)' }}>{t.ticketNr || '—'}</td>
                      <td><Link to={`/tickets/${t.id}`} style={{ color: 'var(--text)' }} onClick={(e) => e.stopPropagation()}>{t.title || tr('tickets.untitled')}</Link></td>
                      ...
                      {canDelete && (
                        <td style={{ textAlign: 'right' }} onClick={(e) => e.stopPropagation()}>
                          <button
                            type="button"
                            aria-label={tr('tickets.deleteTicket', { id: t.ticketNr || t.id })}
                            title={tr('analysis.deleteTicket')}
          ...
          {(ctx.ticket.state ?? 'OPEN') === 'OPEN' && (
            <>
              ...
              <button style={{ ...ctxItem, color: 'var(--warning)' }} onClick={() => openFpModal(ctx.ticket)}>
                <ShieldOff size={14} />{tr('tickets.closeAsFalsePositive')}</button>
              <button style={{ ...ctxItem, color: 'var(--danger)' }} onClick={() => mutate([ctx.ticket.id], { state: 'CLOSED' })}>
                <XCircle size={14} />{tr('tickets.closeCase')}</button>
            </>
          )}
          {canDelete && (
            <>
              ...
              <button style={{ ...ctxItem, color: 'var(--danger)' }} onClick={() => openDeleteModal(ctx.ticket)}>
                <Trash2 size={14} />{tr('tickets.delete')}</button>
            </>
          )}
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--danger)', borderRadius: 'var(--radius)', padding: 24, width: 440, maxWidth: '90vw', boxShadow: 'var(--shadow-pop)' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              ...
              <div>
                <div id="fp-modal-title" style={{ fontSize: 14, fontWeight: 700 }}>{tr('ui.closeFalsePositive')}</div>
                <div style={{ fontSize: 12, color: 'var(--text-dim)', marginTop: 2 }}>
                  {fpModal.ticket.ticketNr && <span className="mono">{fpModal.ticket.ticketNr} · </span>}
                  {fpModal.ticket.title || tr('tickets.untitled')}
                </div>
              </div>
            </div>

            <label style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4, color: 'var(--text-dim)', display: 'block', marginBottom: 6 }}>{tr('tickets.reasonRequired')}</label>
            <textarea
              autoFocus
              aria-label={tr('ui.justificationRequired2')}
              value={fpReason}
              onChange={(e) => setFpReason(e.target.value)}
              placeholder={tr('tickets.falsePositiveReason')}
            ...
            />
            <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 6, marginBottom: 16 }}>{tr('tickets.closedWithReason')}<code>false_positive</code>{tr('tickets.reasonStoredInNotes')}</div>

            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <Button variant="ghost" onClick={() => setFpModal(null)}>{tr('common.cancel2')}</Button>
              <Button variant="danger" icon={<ShieldOff size={14} />} disabled={!fpReason.trim() || busy} onClick={() => void confirmFp()}>
                {busy ? tr('text.saving') : tr('ui.closeFalsePositive')}
              </Button>
            </div>
          </div>
        </div>
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="del-modal-title"
          ...
          <div ref={deletePanelRef} style={{ background: 'var(--bg-card)', border: '1px solid var(--danger)', borderRadius: 'var(--radius)', padding: 24, width: 440, maxWidth: '90vw', boxShadow: 'var(--shadow-pop)' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              ...
              <div>
                <div id="del-modal-title" style={{ fontSize: 14, fontWeight: 700 }}>{tr('tickets.deleteQuestion')}</div>
                <div style={{ fontSize: 12, color: 'var(--text-dim)', marginTop: 2 }}>
                  {deleteModal.ticketNr && <span className="mono">{deleteModal.ticketNr} · </span>}
                  {deleteModal.title || tr('tickets.untitled')}
                </div>
              </div>
            </div>

            <div style={{ fontSize: 12.5, color: 'var(--text-muted)', marginBottom: 16, lineHeight: 1.5 }}>{tr('tickets.deleteWarning')}</div>

            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <Button variant="ghost" disabled={busy} onClick={() => setDeleteModal(null)}>{tr('common.cancel2')}</Button>
              <Button variant="danger" icon={<Trash2 size={14} />} autoFocus disabled={busy} onClick={() => void confirmDelete()}>
                {busy ? tr('tickets.deleting') : tr('tickets.deletePermanently')}
              </Button>
            </div>
          </div>
        </div>
          ...
          <div ref={bulkPanelRef} style={{ background: 'var(--bg-card)', border: '1px solid var(--danger)', borderRadius: 'var(--radius)', padding: 24, width: 460, maxWidth: '90vw', boxShadow: 'var(--shadow-pop)' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-dim)', marginTop: 2 }}>{tr('tickets.bulkDeleteIntro')}</div>
              </div>
            </div>

            <div style={{ fontSize: 12.5, color: 'var(--text-muted)', marginBottom: 16, lineHeight: 1.5 }}>{tr('tickets.bulkDeleteWarning')}</div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <Button variant="ghost" disabled={busy} onClick={() => setBulkDeleteOpen(false)}>{tr('common.cancel2')}</Button>
              <Button variant="danger" icon={<Trash2 size={14} />} autoFocus disabled={busy} onClick={() => void confirmBulkDelete()}>
                {busy ? tr('tickets.deleting') : tr('tickets.deletePermanentlyCount', { count: selected.size })}
              </Button>
            </div>
          </div>
        </div>
  ...
  return (
    <Card style={{ padding: '10px 16px', marginBottom: 16, display: 'flex', gap: 18, alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: 0.4 }}>{tr('common.priority')}</span>
