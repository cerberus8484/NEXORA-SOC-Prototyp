...
export function UseCaseDeveloperPage() {
  ...
  async function generate() {
    ...
    try {
      ...
      setMsg({ ok: false, text: e instanceof Error ? e.message : 'Generierung fehlgeschlagen (Provider/Ollama?)' });
  ...
  async function open(id: string) {
    ...
    } catch (e) { setMsg({ ok: false, text: e instanceof Error ? e.message : 'Use-Case konnte nicht geladen werden' }); }
  }
  ...
  async function transition(action: 'review' | 'approve' | 'reject' | 'publish') {
    ...
    try {
      ...
      setMsg({ ok: false, text: e instanceof Error ? e.message : 'Aktion fehlgeschlagen' });
    } finally { setBusy(false); }
  }

  return (
    <div>
      <SectionHeader title="KI Use-Case Developer" subtitle="Entwickelt Detection-Use-Case-Entwürfe aus Evidence, Tickets & Findings — Mensch prüft, dann Veröffentlichung" help="use-case-developer" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr 320px', gap: 14, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Card>
            <CardBody style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <Button variant="primary" size="sm" icon={<Lightbulb size={14} />} disabled={!canAct || busy} title={!canAct ? 'Erfordert Analyst+' : ''} onClick={() => void generate()}>
                {busy ? 'Generiere …' : 'Use Case generieren'}
              </Button>
              <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>Erzeugt nur einen Entwurf — keine Regel wird aktiviert.</div>
            </CardBody>
          </Card>
          ...
          <Card style={{ overflow: 'hidden' }}>
            ...
            <div style={{ maxHeight: '52vh', overflowY: 'auto' }}>
              {listError ? <div style={{ padding: 16, fontSize: 12, color: 'var(--danger)' }}>Entwürfe nicht ladbar — Abruf fehlgeschlagen. Dies ist kein leerer Zustand; bitte später erneut versuchen.</div>
                : !list ? <div style={{ padding: 16 }}><Spinner label="Lädt …" /></div>
                : list.length === 0 ? <div style={{ padding: 16, fontSize: 12, color: 'var(--text-dim)' }}>Noch keine Entwürfe.</div>
                : list.map((d) => (
                  <button key={d.id} onClick={() => void open(d.id)} style={{ display: 'block', width: '100%', textAlign: 'left', background: sel?.id === d.id ? 'var(--accent-soft)' : 'transparent', border: 'none', borderLeft: sel?.id === d.id ? '3px solid var(--accent)' : '3px solid transparent', padding: '9px 12px', cursor: 'pointer', borderBottom: '1px solid var(--border-soft)' }}>
                    ...
                    <div style={{ fontSize: 12.5, color: 'var(--text)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.title || '(ohne Titel)'}</div>
        ...
        <div style={{ minWidth: 0 }}>
          {!sel ? (
            <Card><CardBody><EmptyState icon={<Lightbulb size={28} />} title="Kein Entwurf gewählt" message="Generiere einen Use Case oder wähle links einen Entwurf." /></CardBody></Card>
          ) : (
            <Card>
              <CardBody style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--text-dim)', marginLeft: 'auto' }}>{sel.generatedBy}{sel.generatedBy?.includes('stub') ? ' · Beispiel-Entwurf (kein LLM)' : ''}</span>
                </div>
                <h2 style={{ margin: 0, fontSize: 17, color: 'var(--text)' }}>{sel.title || '(ohne Titel)'}</h2>

                <Field title="Beschreibung" text={sel.description} />
                <Field title="Detection-Ziel" text={sel.detectionGoal} />
                <Block title="Datenquellen" items={sel.dataSources} />
                <Block title="Benötigte Felder" items={sel.requiredFields} />
                ...
                <Block title="Playbook-Schritte" items={sel.playbookSteps} ordered />

                {/* ── Export-Vorschau ── */}
                <div style={{ borderTop: '1px solid var(--border-soft)', paddingTop: 12, marginTop: 4 }}>
                  <div style={{ ...SUB, marginBottom: 8 }}>Regel-Export-Vorschau</div>
                  <div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 8 }}>
                    Reine Vorschau — kein Schreiben, keine Aktivierung.
                  </div>
                  ...
                  {/* Code-Vorschau */}
                  {exportBusy && <div style={{ padding: 8, fontSize: 12, color: 'var(--text-dim)' }}>Generiere Vorschau …</div>}
                  {!exportBusy && !exportResult && (
                    <div style={{ padding: '8px 0', fontSize: 12, color: 'var(--text-dim)' }}>
                      Format wählen und «Laden» klicken.
                    </div>
                  )}
                  {exportResult && (
                    <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Card>
            <CardBody>
              ...
              {!quality ? <span style={DIM}>Kein Entwurf bewertet.</span> : (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    <Badge tone={quality.passed ? 'success' : 'danger'}>{quality.passed ? 'Bestanden' : 'Nicht bestanden'}</Badge>
          ...
          {sel && (
            <Card>
              <CardBody style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <Button variant="ghost" size="sm" icon={<Send size={13} />} disabled={!canAct || busy || sel.status !== 'draft'} onClick={() => void transition('review')}>Zur Prüfung geben</Button>
                <Button variant="primary" size="sm" icon={<Check size={13} />} disabled={!canApply || busy || sel.status !== 'in_review'} title={!canApply ? 'Erfordert Engineer+' : ''} onClick={() => void transition('approve')}>Genehmigen</Button>
                ...
                <Button variant="danger" size="sm" icon={<Rocket size={13} />} disabled={!canAdmin || busy || sel.status !== 'approved'} title={!canAdmin ? 'Erfordert Admin' : ''} onClick={() => void transition('publish')}>Veröffentlichen</Button>
                <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 4 }}>Analyst erstellt + prüft · Engineer/Admin entscheidet · Admin veröffentlicht. Kein Auto-Write.</div>
              </CardBody>
            </Card>
          )}
        </div>
