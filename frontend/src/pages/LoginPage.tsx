...
export function LoginPage() {
  ...
  async function onSubmitMfa(e: FormEvent) {
    ...
    if (ve) { setError(t(ve.key, ve.params)); return; }
    setError('');
    setBusy(true);
    try {
  ...
  return (
    <div style={{ display: 'grid', placeItems: 'center', height: '100%', background: 'radial-gradient(circle at 50% 28%, var(--bg-elevated), var(--bg))' }}>
      <form className="card card-pad" style={{ width: 372, padding: 32 }} onSubmit={onSubmit}>
        ...
        <p style={{ color: 'var(--text-dim)', fontSize: 13, margin: '0 0 24px' }}>{t('login.tagline')}</p>

        {step === 'mfa' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-dim)', fontSize: 13 }}>
              <ShieldCheck size={16} />
              <span>{t('mfa.challengeRequired')}</span>
            </div>
            ...
            <Button type="submit" variant="primary" disabled={busy}>{busy ? t('mfa.verifying') : t('mfa.verify')}</Button>
        ...
        {step === 'setup' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-dim)', fontSize: 13 }}>
              <ShieldCheck size={16} />
              <span>{t('mfa.setupRequired')}</span>
            </div>
            <Field label={t('mfa.secretLabel')} hint={t('mfa.secretHint')}>
            ...
            <Field label={t('mfa.codeLabel')} hint={t('mfa.setupCodeHint')}>
            ...
            {error && <div role="alert" style={{ color: 'var(--danger)', fontSize: 13 }}>{error}</div>}
            <Button type="submit" variant="primary" disabled={busy}>{busy ? t('mfa.activating') : t('mfa.activate')}</Button>
        ...
        {step === 'password' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <Button type="submit" variant="primary" disabled={busy}>{busy ? t('login.signingIn') : t('login.signIn')}</Button>
              ...
              <>
                ...
                {ssoEnabled && (
                  <Button type="button" variant="ghost" disabled={busy}
                    ...
                    <LogIn size={16} style={{ marginRight: 8 }} /> {t('login.signInWithSso')}
                  </Button>
                )}
                {passkeyEnabled && isPasskeySupported() && (
                  <Button type="button" variant="ghost" disabled={busy} onClick={handlePasskeyLogin}>
                    <Fingerprint size={16} style={{ marginRight: 8 }} /> {t('login.signInWithPasskey')}
                  </Button>
                )}
              </>
            )}
