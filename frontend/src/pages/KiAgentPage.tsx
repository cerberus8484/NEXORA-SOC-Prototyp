...
function KiFunctionsCard() {
  return (
    <Card>
      <CardHeader title="KI-Analysefunktionen (Übersicht)" />
      <CardBody>
        <div style={{ marginBottom: 8 }}>
          <span style={s.dimLabel}>
            9 Funktionen aktiv — operative Nutzung im Analysis-Deck → Tab „KI Analyse"
          </span>
        </div>
        <div>
  ...
  return (
    <Card>
      <CardHeader
        ...
        actions={<Badge tone={hitl ? 'success' : 'warning'} dot>{hitl ? 'Manuelle Freigabe aktiv' : 'Autonomie aktiv'}</Badge>}
      />
      <CardBody>
        <div style={s.hitlBox}>
          <div style={s.hitlStatus}>
            <ShieldCheck size={18} style={{ color: hitl ? 'var(--success)' : 'var(--warning)', flexShrink: 0 }} />
            <span style={s.hitlLabel}>Modus: {hitl ? 'Manuelle Freigabe' : 'Autonomie aktiv (siehe Policies)'}</span>
          </div>
          <p style={s.hitlBody}>
            Der KI-Agent führt keine Aktionen automatisch aus. Jeder Vorschlag muss von einem
            Analysten, Engineer oder Admin explizit genehmigt oder abgelehnt werden.
            Confidence-Schwellen, Autonomie-Modi (advisory / assisted / autonomous) und
            Aktionsklassen-Decken (z. B. host_response = advisory-only) werden in den
            Autonomy-Policies konfiguriert.
          </p>
          <div style={s.linkRow}>
            <ExternalLink size={13} style={{ color: 'var(--accent)', flexShrink: 0 }} />
            <Link to="/autonomy-policies" style={s.link}>
              Autonomy-Policies konfigurieren →
            </Link>
          </div>
        </div>
      </CardBody>
  ...
  return (
    <Card>
      ...
      <CardBody>
        <div style={{ marginBottom: 8 }}>
          <span style={s.dimLabel}>
            Read-only. Durchsetzung serverseitig (authenticate.js + Middleware).
          </span>
        </div>
        <div>
  ...
  return (
    <Card>
      <CardHeader
        ...
        actions={<Badge tone="muted">Geplant</Badge>}
      />
      <CardBody>
        <div style={s.plannedBox}>
          <div style={s.plannedNote}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              ...
              <span style={s.plannedLabel}>Noch nicht verfügbar</span>
            </div>
            Prompts liegen aktuell serverseitig im{' '}
            <code style={{ fontFamily: 'var(--font-mono)', fontSize: 11 }}>OllamaLlmProvider._buildPromptFromBundle()</code>.
            Ein editierbares Template-System (je Kind: Triage, Report Draft, …) ist für eine
            spätere Phase geplant. Wenn verfügbar, können Prompts hier pro Funktion angepasst,
            getestet und versioniert werden — ohne Neustart des Servers.
          </div>
          <div style={{ marginTop: 4 }}>
            <span style={s.dimLabel}>Verfügbare Funktionen / Kinds</span>
    ...
    <div style={s.page}>
      <div style={s.header}>
        ...
        <div style={s.headerText}>
          <h1 style={{ ...s.title, display: 'flex', alignItems: 'center', gap: 6 }}>
            KI Agent — Konfiguration
            <HelpTip topic="ki-agent" />
          </h1>
          <p style={s.subtitle}>
            Provider, Modell und Wissensbasis des lokalen KI-Agenten. Operative KI-Funktionen
            (Triage, Empfehlungen, Approve/Reject) liegen im Analyse-Deck unter dem Tab „KI Analyse".
          </p>
        </div>
      </div>
        ...
        <Card>
          <CardBody>
            <EmptyState
              ...
              message="Die KI-Agent-Konfiguration ist Administratoren vorbehalten. Die Durchsetzung erfolgt serverseitig."
            />
          </CardBody>
        </Card>
      ) : (
