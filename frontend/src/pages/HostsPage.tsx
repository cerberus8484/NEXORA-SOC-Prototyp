    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      <SectionHeader
        ...
        actions={
          <>
            ...
            <Button variant="success" icon={<Plus size={15} />} disabled={!isAdmin} onClick={() => setShowAdd(true)} title={isAdmin ? 'Neuen Wazuh-Agent registrieren' : 'Nur Admin'}>Add Host</Button>
      ...
      {notConfigured && agents && (
        <div style={{ marginBottom: 16, padding: '10px 14px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-card-soft)', border: '1px solid var(--border)', color: 'var(--text)', fontSize: 12.5 }}>
          <strong style={{ color: 'var(--text-muted)' }}>Nicht verbunden</strong> — die Wazuh-API ist noch nicht in der Integrations-Konfiguration hinterlegt. Sobald sie gespeichert ist, erscheinen hier automatisch die echten registrierten Agents.
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 440px', gap: 16, flex: 1 }}>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={10} style={{ ...TD, textAlign: 'center', padding: 48, color: 'var(--text-dim)' }}>
                    {agents === null ? 'Hosts werden geladen …' : notConfigured ? 'Wazuh-API nicht konfiguriert' : 'Keine Hosts für diesen Filter'}
                  </td></tr>
                ) : filtered.map((h) => (
        ...
        {sel ? <HostDetail host={sel} posture={selPosture} tab={tab} onTab={setTab} loading={invLoading && !notConfigured} isAdmin={isAdmin} onDelete={removeManualHost} /> : (
          <Card style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <EmptyState title="Kein Host ausgewählt" message="Host anklicken für Inventory & Heartbeat." />
          </Card>
        )}
      </div>
    </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          ...
          {isManual && isAdmin && onDelete && (
            <Button variant="ghost" size="sm" icon={<Trash2 size={14} />} onClick={() => onDelete(host.id)}
              style={{ marginLeft: 'auto', color: 'var(--danger)' }} title="Manuelles Asset entfernen">Entfernen</Button>
          )}
        </div>
      ...
      <div style={{ padding: '14px 18px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {tab === 'overview' && (
          <>
            ...
            {loading && <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Inventory wird geladen …</div>}
...
function InventoryHighlights({ inv, title = 'Inventory Highlights', expanded }: { inv: NonNullable<RegisteredHost['inventory']>; title?: string; expanded?: boolean }) {
  ...
  return (
    <Panel title={title} icon={<ShieldCheck size={14} />}>
      ...
      <KvDot
        k="Antivirus"
        v="— (nicht im syscollector)"
        title="AV-Erkennung erfordert die Paketliste pro Host. Der syscollector-Endpunkt liefert nur die Gesamtzahl der Pakete, nicht die einzelnen Paketnamen."
      />
      <KvDot
        k="Fehlende Patches"
        v="— (kein Patch-Endpunkt)"
        title="Patch-Status nicht im Wazuh-syscollector-Inventory verfügbar. Wird mit Wazuh-Vulnerability-Endpunkt (Posture) abgedeckt."
      />
      {inv.assetTags && inv.assetTags.length > 0 && <Kv k="Asset Tags" v={inv.assetTags.join(', ')} />}
    </Panel>
  );
  ...
  if (!v && !s) {
    return (
      <Panel title="Vulnerabilities & SCA" icon={<ShieldCheck size={14} />}>
        <div style={{ fontSize: 11.5, color: 'var(--text-dim)' }}>Keine SCA-/CVE-Daten für diesen Host (oder Indexer nicht konfiguriert).</div>
      </Panel>
    );
  }
    ...
    <Panel title="Vulnerabilities & SCA" icon={<ShieldCheck size={14} />}>
      {v && (
        <>
          ...
          <Kv k="CVEs gesamt" v={String(v.total)} />
          {v.topCves.length > 0 && (
  ...
  return (
    <Panel title="Risk Summary" icon={<ShieldCheck size={14} />}>
      ...
      {risk.factors.length > 0 ? (
        ...
        <div style={{ marginTop: 10, fontSize: 11.5, color: 'var(--text-dim)' }}>Keine erhöhten Risikofaktoren aus der verfügbaren Telemetrie.</div>
      )}
      <div style={{ marginTop: 8, fontSize: 10.5, color: 'var(--text-dim)' }}>
        Heuristik aus Heartbeat/Inventory/Ports — kein CVE-Scan. Volle SCA/Vuln-Bewertung folgt mit den Wazuh-SCA-Endpunkten.
      </div>
    </Panel>
  );
}
...
function TransparentRiskBadge({ host, posture }: { host: RegisteredHost; posture: HostPosture | null }) {
  ...
  const tooltip = risk.level === 'unknown'
    ? 'Zu wenig Daten für eine ehrliche Einstufung (kein Heartbeat-Alter + kein Posture).'
    : risk.reasons.length === 0
      ? 'Keine erhöhten Risikosignale aus den verfügbaren Daten.'
  ...
  return (
    <Panel title="Transparentes Risiko-Level" icon={<ShieldCheck size={14} />}>
      ...
      <div style={{ marginTop: 8, fontSize: 10.5, color: 'var(--text-dim)', fontStyle: 'italic' }}>
        Abgeleitet aus: Heartbeat-Status, Heartbeat-Alter, SCA-Compliance, CVEs (Posture-Endpunkt).
        Nur aus real verfügbaren Daten — kein erfundener Score.
      </div>
    </Panel>
  );
}
