...
export function formatUserAuditEntry(entry: UserAuditEntry): FormattedUserAuditEntry {
  ...
  switch (entry.action) {
    case 'USER_CREATED':
      description = i18n.t('users.userCreatedWith', { email });
      break;

    case 'USER_ROLE_CHANGE': {
      const from = extractString(meta, 'fromRole');
      const to   = extractString(meta, 'toRole');
      description = i18n.t('users.roleChangedDetail', { email, from, to });
      break;
    }
    ...
    case 'USER_ACTIVE_CHANGE': {
      ...
      description = i18n.t('users.userStateWith', { state, email });
      break;
    }

    case 'USER_PASSWORD_RESET':
      description = i18n.t('users.passwordResetDetail', { email });
      break;

    default:
      description = i18n.t('audit.eventWith', { action: entry.action });
      break;
  }
