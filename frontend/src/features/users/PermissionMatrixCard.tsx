...
function PermCell({ allowed }: { allowed: boolean }) {
  ...
  return allowed ? (
    ...
    <Minus size={14} color="var(--text-dim, #6b7280)" aria-label={tr('text.notPermitted')} />
  );
}
...
export function PermissionMatrixCard() {
  ...
  return (
    <div style={s.card}>
      ...
      <table style={s.table} aria-label={tr('users.permissionMatrixSubtitle')}>
        <thead>
          <tr>
      ...
      <p style={s.hint}>{tr('users.enforcementServerSide')}</p>
    </div>
  );
}
