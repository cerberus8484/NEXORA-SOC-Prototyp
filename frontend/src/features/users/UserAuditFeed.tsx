...
export function UserAuditFeed() {
  ...
  useEffect(() => {
    ...
    auditApi
      ...
      .catch((err: unknown) => {
        if (controller.signal.aborted) return;
        setError(err instanceof Error ? err.message : 'Audit-Feed konnte nicht geladen werden');
      })
      .finally(() => {
  ...
  return (
    <div style={s.card}>
      <div style={s.header}>
        <span style={s.title}>
          <Activity size={14} /> Audit-Aktivität
        </span>
        <a href="/audit" style={s.feedLink}>
      ...
      {!loading && !error && entries.length === 0 && (
        <EmptyState title="Keine Benutzer-Events" message="Es wurden noch keine Rollen- oder Kontoänderungen protokolliert." />
      )}
      {!loading && !error && entries.length > 0 && (
