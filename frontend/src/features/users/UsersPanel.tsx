...
export function UsersPanel() {
  ...
  async function onResetMfa(password: string) {
    ...
    try {
      ...
      setMfaNotice(`MFA für „${mfaTarget.email}" zurückgesetzt — der Nutzer kann sich wieder nur mit Passwort anmelden und MFA neu einrichten.`);
      setMfaTarget(null);
    } catch (err) {
      const status = err instanceof ApiError ? err.status : 0;
      setMfaError(
        status === 403 ? 'Passwort ungültig.'
          : status === 404 ? 'Nutzer nicht gefunden.'
  ...
  useEffect(() => {
    ...
    userApi.list()
      ...
      .catch((err: unknown) => {
        if (alive) setListError(err instanceof Error ? err.message : 'Laden fehlgeschlagen');
      });
    return () => { alive = false; };
  }, []);
  ...
  async function onRole(target: ManagedUser, role: string) {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : 'Rollenänderung fehlgeschlagen');
    } finally {
      setBusyId('');
    }
  }
  ...
  async function onActive(target: ManagedUser) {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : 'Statusänderung fehlgeschlagen');
    } finally {
      setBusyId('');
    }
  }
  ...
  async function onResetPassword(target: ManagedUser) {
    if (!(await confirm({
      ...
      message: `Passwort für „${target.email}" wirklich zurücksetzen? Das alte Passwort wird sofort ungültig.`,
  ...
  async function copyTempPassword() {
    ...
    try {
      ...
      setActionError(err instanceof Error ? `Kopieren fehlgeschlagen: ${err.message}` : 'Kopieren fehlgeschlagen');
    }
  }
  ...
  const headerTitle = (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
      <ShieldCheck size={15} /> Benutzer &amp; Rollen
    </span>
  );
  const headerAction = (
    <Button size="sm" variant="primary" onClick={showForm ? closeForm : openForm}>
      {showForm
        ? <><X size={13} style={{ marginRight: 4 }} />Abbrechen</>
        : <><UserPlus size={13} style={{ marginRight: 4 }} />Benutzer anlegen</>}
    </Button>
  );
    ...
    <Card>
        ...
        {showForm && (
          <form onSubmit={(e) => void onCreateSubmit(e)} style={s.inlineForm}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 140px', gap: 8, alignItems: 'end' }}>
              ...
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>Passwort (min. 8 Zeichen)</span>
                <input
              ...
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>Rolle</span>
                <Select options={ROLE_OPTIONS} value={form.role} disabled={creating}
                  onChange={(e) => setForm((f) => ({ ...f, role: e.target.value }))} />
              </label>
            </div>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <Button size="sm" variant="ghost" type="button" disabled={creating} onClick={closeForm}>Abbrechen</Button>
              <Button size="sm" variant="primary" type="submit" disabled={creating}>
                {creating ? 'Wird angelegt…' : 'Speichern'}
              </Button>
            </div>
          </form>
        )}
          ...
          <div style={s.tableSection}>
            ...
            {users !== null && !listError && filtered.length === 0 && (
              <EmptyState
                ...
                message={search || roleFilter ? 'Filter anpassen oder Suche leeren.' : 'Noch keine Benutzerkonten vorhanden.'}
              />
            )}
                ...
                {paginated.items.map((u) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={s.nameCol}>
                        <div style={s.name}>
                          <span style={{ ...s.statusDot, background: u.isActive ? 'var(--success, #22c55e)' : 'var(--text-dim, #6b7280)' }} title={u.isActive ? 'Aktiv' : 'Inaktiv'} />
                      ...
                      <Button
                        ...
                        title={!canDeactivate(me, u) ? 'Den eigenen Account nicht deaktivieren' : undefined}
                      ...
                      {isAdmin && (
                        <Button
                          ...
                          disabled={busyId === u.id}
                          title={`Passwort von ${u.email} zurücksetzen`}
                      ...
                      {isAdmin && (
                        <Button
                          ...
                          disabled={busyId === u.id}
                          title={`MFA von ${u.email} zurücksetzen (Recovery bei verlorenem Authenticator)`}
                          aria-label={`MFA von ${u.email} zurücksetzen`}
            ...
            {users !== null && filtered.length > PAGE_SIZE && (
              <div style={s.pagination}>
                <span>
                  {filtered.length === 0
                    ? '0 Benutzer'
                    : `${paginated.fromIndex}–${paginated.toIndex} von ${paginated.totalCount}`}
                </span>
                <div style={s.pageNav}>
                  <Button
                    size="sm" variant="ghost"
      ...
      <ArmPasswordDialog
        ...
        description={mfaTarget
          ? `Zum Zurücksetzen der MFA von „${mfaTarget.email}" dein eigenes Passwort bestätigen. Der Nutzer kann sich danach wieder nur mit Passwort anmelden und MFA neu einrichten. Die Aktion wird protokolliert.`
          : ''}
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 id="reset-pw-title" style={{ margin: 0, fontSize: 15, fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: 8, color: 'var(--text)' }}>
                <KeyRound size={16} /> Temporäres Passwort
              </h2>
              <Button size="sm" variant="ghost" icon={<X size={14} />} aria-label="Dialog schließen" onClick={() => setResetResult(null)} />
            </div>
            <div style={{ fontSize: 12.5, color: 'var(--text-dim)' }}>
              Neues Passwort für <strong style={{ color: 'var(--text)' }}>{resetResult.email}</strong>:
            </div>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>Temporäres Passwort</span>
              <div style={{ display: 'flex', gap: 8 }}>
                <input
                  readOnly value={resetResult.tempPassword} aria-label="Temporäres Passwort"
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <Button size="sm" variant="ghost" onClick={() => setResetResult(null)}>Schließen</Button>
            </div>
          </div>
        </div>
      )}
