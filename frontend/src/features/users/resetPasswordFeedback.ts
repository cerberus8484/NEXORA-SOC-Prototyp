...
export const RESET_PASSWORD_WARNING =
  'Dieses temporäre Passwort wird nur EINMAL angezeigt. Bitte sicher (out-of-band) an den Benutzer übermitteln — nicht per Klartext-E-Mail.';
...
export function resetPasswordResult(res: { data?: { tempPassword?: unknown } } | null | undefined): ResetPasswordResult {
  ...
  return { ok: false, message: 'Antwort ohne temporäres Passwort — Reset fehlgeschlagen.' };
}
...
export function resetPasswordErrorMessage(err: unknown): string {
  const base = err instanceof Error ? err.message : 'Unbekannter Fehler';
  return `Passwort-Reset fehlgeschlagen: ${base}`;
}
