...
export function HuntNotesPanel({ notes, canAct, onAdd }: Props) {
  ...
  return (
    <Card>
      ...
      <CardBody>
        {notes.length === 0 ? (
          <div style={{ fontSize: 12.5, color: 'var(--text-dim)' }}>Noch keine Notizen.</div>
        ) : (
          notes.map((n) => (
        ...
        {canAct && (
          <div style={{ marginTop: 10 }}>
            <textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Neue Notiz …" aria-label="Neue Notiz" style={ta} />
