  ...
  return (
    <Card className="card-pad" style={{ display: 'flex', flexDirection: 'column', gap: 12, height: '100%' }}>
      <div className="row-between">
        ...
        {canCreate && <Button size="sm" variant="primary" icon={<Plus size={14} />} onClick={onNew}>{tr('label.newSession')}</Button>}
      </div>

      <div style={{ position: 'relative' }}>
        <Search size={15} style={{ position: 'absolute', left: 10, top: 10, color: 'var(--text-dim)' }} />
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder={tr('hunts.searchSessionsPlaceholder')} style={{ paddingLeft: 32 }} />
      </div>
      <Select
        value={status}
        onChange={(e) => setStatus(e.target.value)}
        options={[
          { value: '', label: tr('label.allStatuses') },
          { value: 'planned', label: tr('common.planned') }, { value: 'active', label: tr('common.active') },
          { value: 'completed', label: 'Abgeschlossen' }, { value: 'failed', label: tr('common.failed') },
          { value: 'cancelled', label: 'Abgebrochen' },
        ]}
      />

      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, overflow: 'auto', flex: 1 }}>
        {filtered.length === 0 ? <EmptyState title={tr('text.noSessions')} /> : filtered.map((s) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 11.5, color: 'var(--text-dim)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.scope || tr('ui.noScope')}</div>
      ...
      <div style={{ borderTop: '1px solid var(--border-soft)', paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div className="row-between">
          ...
          <button onClick={reset} style={{ background: 'none', border: 'none', color: 'var(--accent)', fontSize: 11, cursor: 'pointer' }}>{tr('analysis.reset')}</button>
        </div>
        <Select
          value={analyst}
          onChange={(e) => setAnalyst(e.target.value)}
          options={[{ value: '', label: tr('label.allAnalysts') }, ...analysts.map((a) => ({ value: a, label: `Analyst ${a.slice(0, 8)}` }))]}
        />
