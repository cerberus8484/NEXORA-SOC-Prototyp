...
export function FindingDetailPanel(p: FindingDetailPanelProps) {
  ...
  async function handleSetVerdict(v: FindingVerdict) {
    ...
    try {
      ...
      setVerdictError('Verdict konnte nicht gesetzt werden.');
    } finally {
  ...
  return (
    <div style={s.panel}>
      <div style={s.head}>
        ...
        <button style={s.iconBtn} onClick={p.onNext} title="Nächstes"><ChevronRight size={16} /></button>
        <button style={s.iconBtn} onClick={p.onClose} title="Schließen"><X size={16} /></button>
      </div>
        ...
        {(tab === 'actions' || tab === 'overview') && (
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
              ...
              <Button variant="ghost" size="sm" icon={<Monitor size={13} />} onClick={p.onOpenRdp} disabled={!isWindows} title={isWindows ? 'RDP-Datei erzeugen' : 'Nur für Windows-Hosts'}>Open RDP Session</Button>
            </div>
              ...
              <div style={{ ...s.label, marginBottom: 6 }}>
                ...
                {verdict !== '' && (
                  <span style={{ marginLeft: 8 }}>
                    <Badge tone={verdictTone(verdict)}>{VERDICT_LABELS[verdict]}</Badge>
                  </span>
                )}
              </div>
