    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, alignItems: 'start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Card>
          ...
          <CardBody>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <Button variant="primary" icon={<Play size={15} />} disabled={!canAct || session.status !== 'planned'} onClick={onStart}>{tr('hunts.startSession')}</Button>
              <Button variant="ghost" icon={<CheckCircle2 size={15} />} disabled={!canAct || session.status !== 'active'} onClick={onComplete}>{tr('ui.complete')}</Button>
              <Button variant="ghost" icon={<FileDown size={15} />} disabled={findings.length === 0} onClick={exportFindings}>{tr('hunts.exportFindings')}</Button>
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          ...
          <div style={{ display: 'flex', gap: 8, padding: 12, borderTop: '1px solid var(--border-soft)', alignItems: 'center' }}>
            ...
            <Button variant="primary" onClick={() => void runCmd()} disabled={!canAct || saving}>{saving ? '…' : tr('ui.run')}</Button>
          </div>
        </Card>
        ...
        <Card>
          <CardHeader
            ...
            actions={canAct && <Button size="sm" variant="ghost" icon={<Plus size={13} />} onClick={() => setShowFinding((v) => !v)}>{tr('ui.addFinding')}</Button>}
          />
          <CardBody>
            {showFinding && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14, padding: 12, background: 'var(--bg-card-soft)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-soft)' }}>
                ...
                <textarea style={{ ...inlineInput, minHeight: 60, resize: 'vertical' }} placeholder={tr('common.descriptionOptional')} value={fDesc} onChange={(e) => setFDesc(e.target.value)} />
                <div style={{ display: 'flex', gap: 8 }}>
                  <Button size="sm" variant="primary" onClick={() => void submitFinding()} disabled={!fTitle.trim() || savingFinding}>{savingFinding ? '…' : tr('common.save')}</Button>
                  <Button size="sm" variant="ghost" onClick={() => setShowFinding(false)}>{tr('common.cancel2')}</Button>
                </div>
              </div>
            )}
            {findings.length === 0
              ? <EmptyState title={tr('text.noFindings')} />
              : findings.map((f) => (
        ...
        <Card>
          <CardHeader
            ...
            actions={canAct && <Button size="sm" variant="ghost" icon={<Plus size={13} />} onClick={() => setShowArtifact((v) => !v)}>{tr('ui.addArtefact')}</Button>}
          />
          <CardBody>
            {showArtifact && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14, padding: 12, background: 'var(--bg-card-soft)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-soft)' }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <input style={inlineInput} placeholder={tr('analysis.valueRequiredPlaceholder')} value={aValue} onChange={(e) => setAValue(e.target.value)} />
                </div>
                <input style={inlineInput} placeholder={tr('common.descriptionOptional')} value={aDesc} onChange={(e) => setADesc(e.target.value)} />
                <div style={{ display: 'flex', gap: 8 }}>
                  <Button size="sm" variant="primary" onClick={() => void submitArtifact()} disabled={!aValue.trim() || savingArtifact}>{savingArtifact ? '…' : tr('common.save')}</Button>
                  <Button size="sm" variant="ghost" onClick={() => setShowArtifact(false)}>{tr('common.cancel2')}</Button>
                </div>
              </div>
            )}
            {artifacts.length === 0
              ? <EmptyState title={tr('text.noArtefacts')} />
              : artifacts.map((a) => (
        ...
        <Card className="card-pad">
          ...
          {commands.length + artifacts.length + findings.length === 0
            ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('ui.noEventsYet2')}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <Card>
          ...
          <CardBody>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <CountBox label={tr('common.critical')} value={sevCount('critical')} tone="danger" />
              <CountBox label={tr('tickets.priorities.high')} value={sevCount('high')} tone="danger" />
              <CountBox label={tr('tickets.priorities.medium')} value={sevCount('medium')} tone="warning" />
              <CountBox label={tr('tickets.priorities.low')} value={sevCount('low')} tone="accent" />
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          ...
          <CardBody>
            {mitreTags.length === 0
              ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('ui.noMitreTagsFromFindings')}</div>
        ...
        <Card>
          ...
          <CardBody>
            {commands.length === 0
              ? <EmptyState title={tr('text.noCommands')} />
