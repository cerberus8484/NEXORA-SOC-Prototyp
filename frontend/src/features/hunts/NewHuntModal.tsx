...
export function NewHuntModal({ onClose, onCreate, initialValues }: NewHuntModalProps) {
  ...
  async function submit() {
    if (!sel) return;
    if (!target.trim()) { setError(tr('ui.targetRequired')); return; }
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : tr('common.createFailed'));
      setBusy(false);
    }
  }
    ...
    <div style={s.overlay} onClick={onClose} aria-hidden="true">
      ...
      >
        <div style={s.head}>
          ...
          <span id="new-hunt-modal-title" style={{ fontSize: 15, fontWeight: 700, flex: 1 }}>{tr('label.newHuntPrebuiltHunts')}</span>
          <button aria-label={tr('common.close')} onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-dim)' }}><X size={18} /></button>
        </div>

        <div style={s.body}>
          <div style={s.label}>{tr('ui.selectHunt')}</div>
          <div style={s.grid}>
            {catalog.map((item) => (
          ...
          <div style={{ marginTop: 12 }}>
            ...
            {tickets.length > 0 && (
              <input
                ...
                placeholder={tr('ui.searchTicketsNumberTitle')}
            ...
            <select
              ...
              aria-label={tr('ui.linkTicket')}
            >
              <option value="">{tr('ui.doNotLinkTicket')}</option>
            ...
            {tickets.length === 0 && (
              <div style={{ marginTop: 4, fontSize: 11, color: 'var(--text-dim)' }}>{tr('hunts.noOpenTickets')}</div>
            )}
          </div>
        ...
        <div style={s.foot}>
          <Button variant="ghost" onClick={onClose} disabled={busy}>{tr('common.cancel2')}</Button>
          <Button variant="primary" onClick={() => void submit()} disabled={busy}>{busy ? 'Starte …' : tr('hunts.start')}</Button>
        </div>
      </div>
    </div>
  );
