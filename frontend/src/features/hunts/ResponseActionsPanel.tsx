...
function kindLabel(kind: string) {
  if (kind === 'isolate_host')      return i18n.t('hunts.isolateHost');
...
function statusLabel(status: string) {
  const m: Record<string, string> = {
    ...
    executing: i18n.t('ui.running2'), completed: 'Abgeschlossen', failed: i18n.t('common.failed'), expired: 'Abgelaufen',
  };
...
export function ResponseActionsPanel({ actions, canRequest, canApprove, canExecute, currentUserId, onRequest, onApprove, onReject, onExecute, circuitOpen, onResetCircuit }: Props) {
  ...
  async function submitRequest() {
    if (kind === 'privileged_command' && !command.trim()) { setReqError(tr('ui.commandRequired')); return; }
    ...
    try {
      ...
      setReqError(e instanceof Error ? e.message : tr('common.requestFailed'));
  ...
  async function execute(a: HuntResponseAction) {
    ...
    try {
      ...
      setExecError((prev) => ({ ...prev, [a.id]: e instanceof Error ? e.message : tr('ui.executionFailed') }));
    } finally {
      setExecuting('');
    }
  }
  ...
  return (
    <div>
      ...
      {circuitOpen && canExecute && (
        <div style={{ ...s.card, ...s.cardDanger, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 12.5, flex: 1 }}>
            <strong>Containment-Kanal gesperrt</strong>{tr('hunts.lockedAfterFailures')}</span>
      ...
      {canRequest && (
        <div style={s.section}>
          <div style={s.label}>{tr('ui.newRequestFourEyesNo')}</div>
          <div style={s.kindRow}>
            {KINDS.map((k) => (
          ...
          <input style={s.input} placeholder={tr('ui.justificationRecommended')}
          ...
          <Button variant="primary" size="sm" icon={<Send size={13} />}
            ...
            {busy ? tr('text.requesting') : 'Anfrage stellen'}
          </Button>
        </div>
      )}
          ...
          {pending.map((a) => {
            ...
            return (
              <div key={a.id} style={cardStyle}>
                ...
                <div style={s.meta}>
                  <span>{tr('ui.requested')} <strong>{a.requestedBy?.slice(0, 12) || '—'}</strong></span>
                  <span>{hhmmss(a.requestedAt)}</span>
                  {isOwnRequest(a) && <span style={{ color: 'var(--warning)' }}>{tr('ui.fourEyesYouCannotApprove')}</span>}
                </div>
                ...
                {canDecide && (
                  <div style={{ marginTop: 8 }}>
                    ...
                    <input style={s.input}
                      placeholder={tr('ui.legalBasisApprovalWorksCouncil')}
                    ...
                    <div style={s.rejectBox}>
                      <input style={{ ...s.input, marginBottom: 0, flex: 1 }}
                        placeholder={tr('analysis.rejectionReason')}
                      ...
                      <Button variant="primary" size="sm" icon={<CheckCircle2 size={13} />}
                        ...
                        title={!(authBasis[a.id] ?? '').trim() ? tr('label.approvalOnlyDocumentedLegalBasis') : undefined}
          ...
          {armed.map((a) => {
            ...
            return (
              <div key={a.id} style={{ ...s.card, ...s.cardDanger }}>
                ...
                <div style={s.meta}>
                  <span>{tr('ui.requested')} <strong>{a.requestedBy?.slice(0, 12) || '—'}</strong></span>
                  {a.approvedBy && <span>{tr('ui.approved')} <strong>{a.approvedBy.slice(0, 12)}</strong> · {hhmmss(a.approvedAt)}</span>}
                  {a.authorizationBasis && <span style={{ color: 'var(--text-muted)' }}>Grundlage: {a.authorizationBasis}</span>}
                </div>

                {own ? (
                  <div style={{ fontSize: 11.5, color: 'var(--warning)', marginTop: 8 }}>{tr('hunts.fourEyesSelf')}</div>
                ) : (
                  <div style={{ marginTop: 8 }}>
                    {/* Ausführung erzwingt eine frische Passwort-Bestätigung (Reauth). */}
                    <input style={s.input} type="password"
                      aria-label={tr('deploy.passwordConfirmationReAuthentication')}
                      placeholder={tr('ui.passwordConfirmationReAuthentication')}
                      value={pw}
                    ...
                    <Button variant="primary" size="sm" icon={<Zap size={13} />}
                      ...
                      {executing === a.id ? tr('ui.running') : tr('ui.run')}
                    </Button>
                  </div>
                )}
              </div>
          ...
          {resolved.map((a) => (
            <div key={a.id} style={s.card}>
              ...
              <div style={s.meta}>
                ...
                {a.approvedBy && <span>{tr('ui.decided')} <strong>{a.approvedBy.slice(0, 12)}</strong> · {hhmmss(a.approvedAt)}</span>}
                {a.authorizationBasis && <span style={{ color: 'var(--text-muted)' }}>Grundlage: {a.authorizationBasis}</span>}
      ...
      {actions.length === 0 && !canRequest && (
        <div style={{ fontSize: 12.5, color: 'var(--text-dim)' }}>{tr('text.noResponseActions')}</div>
      )}
    </div>
  );
}
