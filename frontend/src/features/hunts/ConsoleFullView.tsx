    ...
    <div style={s.backdrop} onClick={onClose} role="dialog" aria-modal="true" aria-label="Console Vollansicht">
      ...
      <div style={s.modal} onClick={(e) => e.stopPropagation()}>
        <div style={s.head}>
          ...
          <Button variant="ghost" size="sm" icon={<X size={14} />} onClick={onClose} aria-label={tr('common.close')}>{tr('common.close')}</Button>
        </div>
        <div style={s.body}>
          {lines.length === 0
            ? <div style={{ padding: '0 18px', color: 'var(--text-dim)' }}>{tr('ui.noLogsYet2')}</div>
            : lines.map((l, i) => (
