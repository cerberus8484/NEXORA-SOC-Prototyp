...
export const NIS2_HEADER_SUBTITLE = i18n.t('app.workingSupportEvidenceKeepingNot');

const STATUS_LABEL: Record<AssessmentStatus, string> = {
  not_started:        i18n.t('text.notStarted'),
  in_progress:        i18n.t('tickets.inProgress'),
  evidence_collected: 'Evidence gesammelt',
  needs_review:       i18n.t('app.reviewNeeded'),
  addressed:          'Bearbeitet',
  not_applicable:     i18n.t('text.notApplicable'),
};
...
export function controlSignal(c: Pick<Nis2ControlRow, 'overdue' | 'needsReview' | 'missingEvidence' | 'reviewDue' | 'status'>): ControlSignal {
  if (c.overdue) return { tone: 'danger', label: i18n.t('app.overdue') };
  if (c.needsReview) return { tone: 'warning', label: i18n.t('app.reviewNeeded') };
  if (c.missingEvidence) return { tone: 'warning', label: i18n.t('text.noEvidence') };
  // reviewDue (P_NIS2_3): addressed + Evidence vorhanden, aber Nachweis seit > Kadenz nicht re-reviewed.
  if (c.reviewDue) return { tone: 'warning', label: i18n.t('app.reviewDue') };
  if (c.status === 'not_applicable') return { tone: 'muted', label: i18n.t('text.notApplicable') };
...
export function incidentCoverageText(
  ...
  return i18n.t('compliance.controlsWithEvidence', { done: summary.incidentEvidenceTotal, total: summary.controlsTotal });
}
