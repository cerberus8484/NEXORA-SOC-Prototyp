...
export function UseCaseModal({ currentValue, onPick, onClose }: UseCaseModalProps) {
  ...
  async function refresh() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Use-Cases konnten nicht geladen werden');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function add() {
    ...
    if (!value) { setError('Bitte einen Use-Case eingeben'); return; }
    setBusy(true);
    setError('');
    try {
        ...
        <div style={LIST}>
          ...
          {!loading && items.length === 0 && (
            <div style={{ color: 'var(--text-muted)', fontSize: 12, padding: 8 }}>
              Noch keine Use-Cases — unten den ersten anlegen.
            </div>
          )}
          {items.map((u) => (
            <div key={u.id} style={ITEM}>
              ...
              <Button size="sm" variant="ghost" icon={<Trash2 size={13} />} disabled={busy} onClick={() => void remove(u.id)} title="Löschen" />
            </div>
          ))}
        </div>
