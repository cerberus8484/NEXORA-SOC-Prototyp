...
const SOURCE_OPTS = [
  { value: 'manual', label: 'Manuell eingefügt' },
...
export function ImportRawModal({ ticketId, onApply, onClose }: ImportRawModalProps) {
  ...
  async function parse() {
    ...
    if (!trimmed) { setError('Bitte Roh-Text eingeben'); return; }
    setBusy(true);
    setError('');
    setResult(null);
    try {
      ...
      setError(err instanceof Error ? err.message : 'Smart-Parser fehlgeschlagen');
    } finally {
      setBusy(false);
    }
  }
    ...
    <div style={OVERLAY} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={PANEL}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <strong style={{ fontSize: 14 }}>Smart-Parser — Roh-Text importieren</strong>
          <Button size="sm" variant="ghost" icon={<X size={14} />} onClick={onClose} style={{ marginLeft: 'auto' }} />
        </div>

        {/* Erklärung */}
        <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>
          IP-Adresse, Hash, URL, Command, Registry-Key … einfügen. Der Smart-Parser erkennt
          den Typ automatisch und füllt Felder im Ticket vor.
        </p>

        {/* Quelle */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 6 }}>
          <label style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 600 }}>Quelle</label>
          <Select
            options={SOURCE_OPTS}
            value={sourceType}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={PREVIEW}>
              {parsedIocs.length > 0
                ...
                : <span style={{ color: 'var(--text-muted)' }}>Keine IOCs extrahiert</span>
              }
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', borderTop: '1px solid var(--border-soft)', paddingTop: 12 }}>
          ...
          {!result ? (
            ...
            <Button variant="success" icon={<Check size={14} />} onClick={apply}>
              Übernehmen
            </Button>
          )}
        </div>
