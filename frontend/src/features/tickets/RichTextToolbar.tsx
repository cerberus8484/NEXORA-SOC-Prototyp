...
export function RichTextToolbar({ targetRef, value, onChange, disabled }: Props) {
  ...
  return (
    <div className="rtb">
      <Btn title={tr('ui.undoCtrlZ')} off><Undo2 size={15} /></Btn>
      ...
      <span className="rtb-sep" />
      <Btn title={tr('ui.bulletedList')} onClick={() => linePrefix('- ')}><List size={15} /></Btn>
      <Btn title="Nummerierte Liste" onClick={() => linePrefix('1. ')}><ListOrdered size={15} /></Btn>
      <span className="rtb-sep" />
      <Btn title={tr('ui.insertLink')} onClick={() => surround('[', '](https://)')}><Link2 size={15} /></Btn>
