    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {value.length === 0 && (
        <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>
          Noch keine Artefakte — Roh-Wert (URL, IP, Hash, Befehl, Base64 …) einfügen, der Typ wird automatisch erkannt.
        </div>
      )}
          ...
          <div key={p.id} style={{ border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-sm)', padding: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <Badge tone={p.type ? 'accent' : 'muted'}>
                {p.type ? <><Zap size={10} style={{ marginRight: 3 }} />{p.type}</> : 'Auto-Erkennung'}
              </Badge>
            ...
            <Field label="Roh-Wert (Auto-Erkennung beim Einfügen)">
              <Textarea
      ...
      {!readOnly && (
        <Button size="sm" variant="ghost" icon={<Plus size={13} />} onClick={() => onChange([...value, newPayload()])}>
          Artefakt hinzufügen
        </Button>
      )}
    </div>
