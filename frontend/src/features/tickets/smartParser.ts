...
export function detectAndParse(raw: string): ParsedPayload {
  ...
  if (/^https?:\/\//i.test(t) || /^ftp:\/\//i.test(t)) {
    ...
    try {
      ...
      if (SUSP_URL_EXTS.includes(ext)) notes.push(`⚠️ Verdächtige Dateiendung: .${ext}`);
      f.notes = notes.join('\n');
  ...
  if (/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(t)) {
    ...
    const notes: string[] = [];
    if (LOOKALIKES.some((l) => domain.toLowerCase().includes(l))) notes.push('⚠️ Mögliches Typosquatting der Domain');
    if (/^\d+$/.test(user)) notes.push('Absender-Localpart ist nur Zahlen — ungewöhnlich');
  ...
  if (/^([a-z0-9-]+\.)+[a-z]{2,}$/i.test(t) && !t.includes('/')) {
    ...
    const notes: string[] = [];
    if (SUSP_TLDS.includes(tld.toLowerCase())) notes.push(`⚠️ Verdächtige TLD: .${tld}`);
  ...
  if (/^[a-zA-Z]:\\/.test(t) || /^\/[a-z/]/.test(t)
    ...
    const notes: string[] = [];
    if (SUSP_EXTS.includes(ext)) notes.push(`⚠️ Ausführbare Datei: .${ext}`);
  ...
  if (/^(cmd(\.exe)?|powershell(\.exe)?|pwsh|bash|sh|python[23]?|wscript|cscript|mshta|rundll32|regsvr32|certutil|bitsadmin|msiexec)\b/i.test(t)
    ...
    if (/iex\s*\(|invoke-expression/i.test(t)) notes.push('⚠️ IEX / Invoke-Expression — dynamische Ausführung');
  ...
  if (t.length > 20 && /^[A-Za-z0-9+/]+=*$/.test(t) && t.length % 4 === 0) {
    ...
    try {
      ...
      if (/http|wget|curl|iex|invoke|download/i.test(decoded)) notes.push('⚠️ Verdächtige Keywords im dekodierten Inhalt');
    } catch {
      notes.push('Dekodierung fehlgeschlagen');
    }
  ...
  if (t.length > 20 && /^([a-f0-9]{2}\s?)+$/i.test(t)) {
    ...
    if (pairs) {
      ...
      if (/[\x00-\x08\x0e-\x1f]/.test(decoded)) decoded = '(Binärdaten — nicht darstellbar)';
    }
  ...
  if (/Invoke-|Import-Module|New-Object|IEX\b|wget\s|curl\s|chmod\s|sudo\s/i.test(t) || t.startsWith('#!')) {
    ...
    if (/base64|frombase64/i.test(t)) notes.push('⚠️ Base64-Verarbeitung im Script');
  ...
  if (/^Mozilla\/|^curl\/|^python-requests|^Wget\/|^Go-http|^Apache-|^libwww/i.test(t)) {
    ...
    if (t.length < 30) notes.push('Sehr kurzer UA — möglicherweise Tool/Script');
