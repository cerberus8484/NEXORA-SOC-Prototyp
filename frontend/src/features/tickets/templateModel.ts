...
export function generateTemplate(form: TicketForm, opts: TemplateOptions = {}): string {
  ...
  const freeField = sec(i18n.t('ui.notesAdditions')) + '│\n'.repeat(4) + secEnd();
  ...
  const netDevRows = row('Port(s)', d.port)
    ...
    + row(i18n.t('ui.deviceType'), d.deviceType);
  ...
  const tpl = [
    ...
    // ── 1. Beschreibung ──
    textBlock(i18n.t('tickets.descriptionAnalysisCaps'), d.description),
    ...
    (d.title || d.useCase || d.datetime || d.category || d.ticketNr) ? (
      ...
      + row(i18n.t('common.priority'), prio)
    ...
    (d.user || d.email || d.dept) ? (
      sec(i18n.t('tickets.userIdentityCaps'))
      + row('Username', d.user)
      + row('E-Mail', d.email)
      + row(i18n.t('profile.department'), d.dept)
    ...
    textBlock(i18n.t('tickets.logSourcesEvidenceCaps'), d.logs),
    textBlock('MASSNAHMEN', d.actions),
    textBlock(i18n.t('tickets.recommendationNextStepsCaps'), d.recommendation),
