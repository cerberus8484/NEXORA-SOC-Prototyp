...
export function VorlagenModal({ currentForm, onLoad, onClose }: VorlagenModalProps) {
  ...
  async function refresh() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : tr('ui.templatesCouldNotLoaded'));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function saveCurrent() {
    ...
    if (!name) { setError(tr('ui.pleaseEnterTemplateName')); return; }
    setBusy(true);
    setError('');
    try {
      ...
      <div style={PANEL}>
        ...
        <div style={LIST}>
          ...
          {!loading && templates.length === 0 && (
            <div style={{ color: 'var(--text-muted)', fontSize: 12, padding: 8 }}>{tr('tickets.noTemplatesYet')}</div>
          )}
          {templates.map((t) => (
            <div key={t.id} style={ITEM}>
              <span style={{ flex: 1, fontSize: 13, fontWeight: 600 }}>{t.name}</span>
              <Button size="sm" variant="ghost" icon={<Download size={13} />} disabled={busy} onClick={() => load(t)}>{tr('text.load')}</Button>
              <Button size="sm" variant="ghost" icon={<Trash2 size={13} />} disabled={busy} onClick={() => void remove(t.id)} title={tr('common.delete')} />
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', borderTop: '1px solid var(--border-soft)', paddingTop: 12 }}>
          <Input
            ...
            placeholder={tr('ui.nameNewTemplate')}
            style={{ flex: 1 }}
          />
          <Button size="sm" variant="success" icon={<Save size={14} />} disabled={busy} onClick={() => void saveCurrent()}>{tr('tickets.saveCurrent')}</Button>
        </div>
      </div>
    </div>
  );
