...
export function TicketEditor({ initial = {}, ticketId, readOnly = false, saving = false, onSave }: TicketEditorProps) {
  ...
  return (
    <div>
      <SectionHeader
        title={initial.ticketNr ? `Ticket ${initial.ticketNr}` : 'Neues Ticket'}
        subtitle="Incident dokumentieren"
        actions={!ro && (
          <>
            ...
            {ticketId && (
              <Button variant="ghost" icon={<Zap size={15} />} onClick={() => setShowImportRaw(true)} title="Smart-Parser: Roh-Text (IP, Hash, URL …) importieren">
                Smart-Import
              </Button>
            )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '45fr 55fr', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <Card>
            <CardHeader
              title={<span style={{ color: 'var(--accent)' }}>Analyse &amp; Befunde</span>}
              actions={!ro && <Button size="sm" variant="ghost" icon={<BookMarked size={14} />} onClick={() => setShowVorlagen(true)}>Vorlage laden / speichern</Button>}
            />
            <CardBody>
              <Field label="Beschreibung / Was ist passiert?">
                <div>
                  ...
                  <textarea
                    ...
                    maxLength={MAX_DESC}
                    style={{ minHeight: 460 }}
                    placeholder="Detaillierte Beschreibung des Vorfalls. Was wurde beobachtet? Welche Logs? Welcher Zeitraum?"
                  />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 8 }}>
                <Field label="IOCs (Indicators of Compromise)">
                  <Textarea value={form.iocs ?? ''} onChange={set('iocs')} readOnly={ro} style={{ minHeight: 110 }} className="input-mono" placeholder="IP-Adressen, Domains, Hashes … (einen pro Zeile)" />
                </Field>
                <Field label="Log-Quellen / Evidence">
                ...
                <Field label="Durchgeführte Maßnahmen">
                  <ChecklistField value={form.actions ?? ''} onChange={(v) => setVal('actions', v)} readOnly={ro} placeholder="Maßnahme hinzufügen …" />
                </Field>
                <Field label="Empfehlung / Next Steps">
                  <ChecklistField value={form.recommendation ?? ''} onChange={(v) => setVal('recommendation', v)} readOnly={ro} placeholder="Next Step hinzufügen …" />
                </Field>
              </div>
            </CardBody>
          </Card>
          ...
          <Card>
            <CardHeader title="Interne Notizen (nicht für Report)" />
            <CardBody>
              <Textarea value={form.notes ?? ''} onChange={set('notes')} readOnly={ro} style={{ minHeight: 90 }} placeholder="Persönliche Notizen, offene Fragen, Follow-up …" />
            </CardBody>
          </Card>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Accordion title="Ticket & Offense" icon={<Shield size={16} />} badge={completion(form, ['ticketNr', 'offenseId', 'datetime', 'priority', 'state', 'status', 'analyst', 'customer'])} defaultOpen>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <Field label="Datum / Zeit (UTC)"><Input type="datetime-local" value={form.datetime ?? ''} onChange={set('datetime')} readOnly={ro} /></Field>
              <Field label="Priorität"><Select options={PRIORITY_OPTS} value={form.priority ?? 'medium'} onChange={set('priority')} disabled={ro} /></Field>
              ...
              {(form.state ?? 'OPEN') === 'CLOSED' && (
                <Field label="Schließgrund"><Select options={CLOSE_REASON_OPTS} value={form.closeReason ?? ''} onChange={set('closeReason')} disabled={ro} /></Field>
              )}
              <Field label="Analyst"><Input value={form.analyst ?? ''} onChange={set('analyst')} readOnly={ro} placeholder="Analyst auswählen" /></Field>
          ...
          <Accordion title="Benutzer / Identity" icon={<UserRound size={16} />} badge={completion(form, ['user', 'email', 'dept', 'manager', 'userType', 'accStatus'])} defaultOpen>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <Field label="Abteilung"><Input value={form.dept ?? ''} onChange={set('dept')} readOnly={ro} placeholder="IT / Finance / HR" /></Field>
              <Field label="Manager"><Input value={form.manager ?? ''} onChange={set('manager')} readOnly={ro} placeholder="Name des Vorgesetzten" /></Field>
          ...
          <Accordion title="Netzwerk & Geräte" icon={<Network size={16} />} badge={completion(form, ['srcIp', 'srcFqdn', 'dstIp', 'dstFqdn', 'port', 'protocol', 'vpn'])} defaultOpen>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <Field label="Gerätetyp"><Select options={DEVICETYPE_OPTS} value={form.deviceType ?? ''} onChange={set('deviceType')} disabled={ro} /></Field>
            </div>
          </Accordion>
          ...
          <Accordion
            title="Threat Intel"
            icon={<Radar size={16} />}
            badge={!ro && <Button size="sm" variant="ghost" icon={<Plus size={13} />} onClick={() => setTiEntries((t) => [...t, emptyTiEntry()])}>TI Eintrag hinzufügen</Button>}
          >
