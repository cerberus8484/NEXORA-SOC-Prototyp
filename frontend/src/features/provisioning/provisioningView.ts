...
export function relativeTime(iso: string | null, now: number = Date.now()): string {
  ...
  if (min < 60) return i18n.t('time.minutesAgoShort', { count: min });
  const hrs = Math.floor(min / 60);
  if (hrs < 24) return i18n.t('time.hoursAgoShort', { count: hrs });
  const days = Math.floor(hrs / 24);
  return i18n.t('time.daysAgoShort', { count: days });
}
...
export function revokeConfirmText(node: InstalledNode): string {
  return i18n.t('provisioning.revokeCredentialConfirm', { node: node.name });
}
...
export function retireConfirmText(node: InstalledNode): string {
  return i18n.t('provisioning.disableNodeConfirm', { node: node.name });
}
