...
export function entitiesOfInterest(items: EntityItem[]): InterestRow[] {
  ...
  for (const e of items) {
    ...
    if (!reason && e.kind === 'ip') {
      ...
      if (note.includes('destination')) reason = 'Externes Ziel';
      else if (note.includes('post-nat')) reason = 'Post-NAT-Quelle';
    }
    if (!reason) continue;
...
export function buildEntityGraph(t: Ticket, ev: ParsedEvidence): EntityGraph {
  ...
  addEdge(proc, user, 'ausgeführt von');
  addEdge(parent, proc, 'spawned');
  addEdge(origin, ip, 'verbindet zu');
  addEdge(origin, domain, 'auflösung');
  // inferred — kausale Vermutung
  addEdge(origin, file, 'erzeugt', true);
  addEdge(origin, url, 'lädt', true);
