...
export const INVESTIGATION_CHECKLIST = [
  'IoCs geprüft', 'VirusTotal geprüft', 'AbuseIPDB geprüft', 'Host geprüft', 'User-Aktivität geprüft', 'Empfehlung dokumentiert',
];

export const COMMAND_TEMPLATES = (target = '<ziel>', ip = '<ip>') => [
  `nslookup ${target}`,
  `whois ${target}`,
  `ping ${ip}`,
  `tracert ${ip}`,
