        ...
        <div style={{ display: 'flex', gap: 12, minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 17, fontWeight: 800, color: 'var(--text)' }}>{t.title || '(ohne Titel)'}</span>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 5 }}>
              {[ev.detection.sourceSystem || t.source, ev.detection.ruleId ? `Rule ${ev.detection.ruleId}` : null, t.description || ev.detection.description].filter(Boolean).join(' — ') || 'Keine Kurzbeschreibung vorhanden.'}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end', flexShrink: 0 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <div ref={ref} style={{ position: 'relative' }}>
                <Button variant="ghost" size="sm" icon={<MoreVertical size={15} />} aria-label="Weitere Aktionen" onClick={() => setMenu((m) => !m)} />
                {menu && (
                  <div style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, background: 'var(--bg-elevated)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius)', boxShadow: '0 8px 24px rgba(0,0,0,.3)', zIndex: 50, minWidth: 180, padding: '4px 0' }}>
                    {canAdmin
                      ? <button onClick={() => { setMenu(false); onDelete(); }} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', textAlign: 'left', padding: '8px 14px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--danger)', fontSize: 12.5 }}><Trash2 size={13} /> Ticket löschen</button>
                      : <div style={{ padding: '8px 14px', fontSize: 12, color: 'var(--text-dim)' }}>Keine weiteren Aktionen.</div>}
                  </div>
                )}
              </div>
            </div>
