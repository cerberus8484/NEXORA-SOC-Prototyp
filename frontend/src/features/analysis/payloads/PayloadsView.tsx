...
function CopyIcon({ value }: { value: string }) {
  ...
  return <button type="button" aria-label="Kopieren" title={done ? 'Kopiert' : 'Kopieren'} onClick={() => { navigator.clipboard?.writeText(value); setDone(true); setTimeout(() => setDone(false), 1200); }} style={{ background: 'transparent', border: 'none', color: done ? 'var(--success)' : 'var(--text-dim)', cursor: 'pointer', flexShrink: 0, padding: 2 }}><Copy size={13} /></button>;
}
...
function SessionActivitySection({ rows }: { rows: SessionActivityRow[] }) {
  return (
    <Section
      title="Session-Aktivität / Honeypot-Befehle"
    ...
    >
      ...
      <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 8 }}>Beobachtete Angreifer-Eingaben aus der Honeypot-Session. Befehle werden als Text angezeigt, nie ausgeführt.</div>
    </Section>
  );
}
    ...
    return (
      <Card style={{ padding: '40px 24px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, textAlign: 'center', maxWidth: 480, margin: '0 auto' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>Noch keine Payload-Evidence vorhanden</div>
          <div style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.55 }}>
            Dieses Ticket enthält derzeit keine strukturiert importierten Datei-, Prozess- oder Payload-Artefakte.
          </div>
        </div>
      </Card>
    );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={grid3}>
        ...
        <Section title="Indicators Found" icon={<ListChecks size={15} style={{ color: 'var(--accent)' }} />} right={<Badge tone="accent">{indicators.length}</Badge>}>
          {indicators.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Keine auffälligen Indikatoren abgeleitet.</div>
            : indicators.map((i) => (
      ...
      <div style={grid2}>
        ...
        <Section title="Decoded Script / Extracted Content" icon={<Code2 size={15} style={{ color: 'var(--accent)' }} />}>
          ...
          {tab === 'decoded' ? (
            decoded ? <CodeBlock code={decoded} /> : <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Kein Encoded-Command zum Dekodieren vorhanden.</div>
          ) : (
            strings.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Keine extrahierbaren Strings (URLs/IPs/Pfade/Registry).</div>
              : <div style={{ display: 'flex', flexDirection: 'column' }}>
                {strings.map((s) => (
      ...
      <Section title="Payload Relationships" icon={<Boxes size={15} style={{ color: 'var(--accent)' }} />}>
        {chain.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Keine Kette aus der Evidence ableitbar.</div>
          : (
        ...
        <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 10 }}>Abgeleitet aus Prozess-/Datei-Evidence + extrahiertem Decoded-Inhalt. Keine erfundenen Knoten.</div>
      </Section>
      </>
      )}
    </div>
