...
export function WazuhExceptionBuilderModal({ ticketId, evidence, initialReason = '', onClose }: Props) {
  ...
  const genPreview = async () => {
    ...
    catch (e) { setPreview({ ok: false, errors: [e instanceof Error ? e.message : tr('analysis.previewFailed')], warnings: [], xml: '', ruleId: 0, file: '', applied: false }); }
  ...
  const doForward = async () => {
    ...
    try {
      ...
      setActionMsg(r.ok
        ? { ok: true, text: r.idempotent ? tr('analysis.alreadyForwarded') : tr('text.forwardedWaitingApprovalEngineerAdministrato') }
        : { ok: false, text: (r.errors || [tr('analysis.forwardFailed')]).join(' · ') });
      loadExceptions();
    } catch (e) { setActionMsg({ ok: false, text: apiErrorText(e, tr('analysis.forwardFailed')) }); }
    finally { setBusy(false); }
  };
  ...
  const doApply = async () => {
    ...
    try {
      ...
      setActionMsg(r.ok
        ? { ok: true, text: r.idempotent ? tr('analysis.exceptionAlreadyExists') : tr('analysis.writtenValidatedRestartRequired') }
        : { ok: false, text: (r.errors || [tr('analysis.applyFailed')]).join(' · ') });
      loadExceptions();
    } catch (e) { setActionMsg({ ok: false, text: apiErrorText(e, tr('analysis.applyFailed')) }); }
    finally { setBusy(false); setConfirmApply(false); }
  };

  const doRestart = async (ex: WazuhFpException) => {
    if (!(await confirm({
      title: tr('services.restartWazuhQuestion'),
      message: tr('analysis.reloadsChangedRulesetBriefInterruption'),
      confirmLabel: tr('label.restart'),
      danger: true,
    }))) return;
    setBusy(true);
    try { const r = await ticketApi.fpRestart(ticketId, ex.id); setActionMsg(r.ok ? { ok: true, text: tr('text.managerRestartedExceptionActive') } : { ok: false, text: (r.errors || []).join(' · ') }); loadExceptions(); }
    catch (e) { setActionMsg({ ok: false, text: apiErrorText(e, tr('services.restartFailed2')) }); }
    finally { setBusy(false); }
  };

  const doRevert = async (ex: WazuhFpException) => {
    if (!(await confirm({
      title: tr('analysis.removeExceptionQuestion'),
      message: tr('analysis.ruleDeletedFromFileConfirm'),
      confirmLabel: tr('common.remove'),
      danger: true,
    }))) return;
    setBusy(true);
    try { const r = await ticketApi.fpRevert(ticketId, ex.id); setActionMsg(r.ok ? { ok: true, text: tr('analysis.ruleRemovedRestartRequiredTake') } : { ok: false, text: (r.errors || []).join(' · ') }); loadExceptions(); }
    catch (e) { setActionMsg({ ok: false, text: apiErrorText(e, tr('analysis.revertFailed')) }); }
    finally { setBusy(false); }
  };
    ...
    <div style={OVERLAY} onClick={onClose} aria-hidden="true">
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          <h3 id="wazuh-fp-modal-title" style={{ margin: 0, fontSize: 16, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 8 }}><ShieldCheck size={18} style={{ color: 'var(--accent)' }} /> {tr('analysis.fpExceptionTitle')}</h3>
          <button aria-label={tr('common.close')} onClick={onClose} style={{ background: 'transparent', border: 'none', color: 'var(--text-dim)', cursor: 'pointer' }}><X size={18} /></button>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--warning)', background: 'var(--warning-soft)', border: '1px solid var(--warning)', borderRadius: 'var(--radius-sm)', padding: '7px 10px', margin: '8px 0 14px' }}>
          <AlertTriangle size={14} /> <strong>This modifies Wazuh rule files.</strong> {tr('analysis.applyWritesValidates')}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div><label style={LBL}>{tr('text.agentInformationOnly')}</label><input style={{ ...INP, opacity: 0.7 }} value={evidence.metadata?.agentId || '—'} readOnly /></div>
          <div><label style={LBL}>Source IP / CIDR / Liste <span style={{ color: 'var(--text-dim)' }}>{tr('analysis.optionalMulticast')}</span></label><input style={INP} value={srcips} onChange={(e) => setSrcips(e.target.value)} placeholder="192.168.240.109, 192.168.240.0/24" /></div>
          <div>
        ...
        {ruleTarget?.isFrequencyRule && ruleTarget.baseRuleId && ruleTarget.baseRuleId !== ruleId && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginTop: 10, padding: '8px 11px', borderRadius: 'var(--radius-sm)', background: 'var(--warning-soft)', border: '1px solid var(--warning)' }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--text)' }}>
              <Trans
                i18nKey="analysis.frequencyRuleHint"
                values={{ ruleId, baseRuleId: ruleTarget.baseRuleId }}
                components={{ c: <code />, b: <strong /> }}
              />
            </span>
            <Button variant="ghost" size="sm" style={{ marginLeft: 'auto' }} onClick={() => setRuleId(ruleTarget.baseRuleId as string)}>
              {tr('analysis.scopeToBaseRule', { baseRuleId: ruleTarget.baseRuleId })}
            </Button>
          </div>
        )}

        <div style={{ marginTop: 12 }}>
          <label style={LBL}>{tr('analysis.justificationRequired')}</label>
          <textarea style={{ ...INP, fontFamily: 'inherit', minHeight: 52 }} value={reason} onChange={(e) => setReason(e.target.value)} placeholder={tr('analysis.eGLocalMulticastTraffic')} />
        </div>

        <div style={{ display: 'flex', gap: 8, marginTop: 14, alignItems: 'center' }}>
          <Button variant="primary" icon={<FileCode2 size={15} />} disabled={busy} onClick={genPreview}>{tr('common.generatePreview')}</Button>
          <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>{tr('analysis.noGlobalExceptionDestinationMandatory')}</span>
        </div>
          ...
          <div style={{ marginTop: 14 }}>
            {preview.errors.length > 0 && (
              <div style={{ fontSize: 12.5, color: 'var(--danger)', background: 'var(--danger-soft)', border: '1px solid var(--danger)', borderRadius: 'var(--radius-sm)', padding: '8px 10px', marginBottom: 8 }}>
                <strong>{tr('text.notCreated')}</strong><ul style={{ margin: '4px 0 0', paddingLeft: 16 }}>{preview.errors.map((e) => <li key={e}>{e}</li>)}</ul>
              </div>
            )}
            ...
            {preview.ok && (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{tr('common.targetColon')} <code>{preview.file}</code> · Rule-ID <code>{preview.ruleId}</code> <Badge tone="muted">{tr('common.preview')}</Badge></span>
                  <span style={{ display: 'flex', gap: 6 }}>
                    <Button variant="ghost" size="sm" icon={<Copy size={13} />} onClick={() => navigator.clipboard?.writeText(preview.xml)}>{tr('analysis.copyXml')}</Button>
          ...
          <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border-soft)' }}>
            {!applyEnabled && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--text-muted)', background: 'var(--bg-card-soft)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '7px 10px', marginBottom: 10 }}>
                <AlertTriangle size={14} style={{ color: 'var(--text-dim)' }} /> <strong>Wazuh FP apply is disabled by configuration</strong> (<code>WAZUH_FP_APPLY_ENABLED</code>{tr('analysis.previewExportStillPossible')}</div>
            )}
            {/* Analyst: erstellen + weiterleiten (kein Write) */}
            {canForward && (
              <Button variant="ghost" icon={<ShieldCheck size={15} />} disabled={busy} onClick={doForward} style={{ marginRight: 8 }}>{tr('analysis.forwardForApproval')}</Button>
            )}
            {!canApplyRole ? (
              <span style={{ fontSize: 12, color: 'var(--text-dim)' }}><Trans i18nKey="analysis.applyNeedsRole" components={{ b: <strong /> }} /></span>
            ) : !confirmApply ? (
              ...
              <div style={{ background: 'var(--danger-soft)', border: '1px solid var(--danger)', borderRadius: 'var(--radius-sm)', padding: '10px 12px' }}>
                <div style={{ fontSize: 12.5, color: 'var(--text)', marginBottom: 8 }}><Trans i18nKey="analysis.confirmWritesRule" components={{ b: <strong />, c: <code /> }} /></div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Button variant="danger" size="sm" disabled={busy} onClick={doApply}>{tr('analysis.confirmAmpApply')}</Button>
                  <Button variant="ghost" size="sm" disabled={busy} onClick={() => setConfirmApply(false)}>{tr('common.cancel2')}</Button>
                </div>
              </div>
            )}
          </div>
        ...
        {exceptions.length > 0 && (
          <div style={{ marginTop: 16 }}>
            <div style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>{tr('analysis.exceptionsOnTicket')}</div>
            {exceptions.map((ex) => (
