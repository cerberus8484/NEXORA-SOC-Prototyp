...
export function ImportDataModal({ ticketId, onClose, onImported }: Props) {
  ...
  async function doParse() {
    ...
    try {
      ...
      setParseError(err instanceof Error ? err.message : tr('analysis.parseErrorPleaseTryAgain'));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 14 }}>Smart-Parser — <code>POST /v1/tickets/:id/import</code> {tr('analysis.previewNothingPersisted')}</div>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', marginBottom: 12, flexWrap: 'wrap' }}>
          ...
          />
          <Button variant="ghost" icon={<Upload size={15} />} onClick={() => fileRef.current?.click()}>{tr('text.loadFile')}</Button>
        </div>

        <textarea
          value={raw} onChange={(e) => setRaw(e.target.value)}
          placeholder={tr('analysis.pasteJsonSyslogFirewallLog')}
