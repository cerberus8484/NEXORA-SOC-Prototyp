...
const ACTION_MAP: Record<string, { label: string; tone: HistoryTone }> = {
  TICKET_CREATE: { label: 'Ticket erstellt', tone: 'success' },
  TICKET_UPDATE: { label: 'Ticket aktualisiert', tone: 'accent' },
  TICKET_DELETE: { label: 'Ticket gelöscht', tone: 'danger' },
  TICKET_VIEW: { label: 'Ticket angesehen', tone: 'muted' },
  AGENT_SUGGESTION_APPROVE: { label: 'KI-Vorschlag genehmigt', tone: 'success' },
  AGENT_SUGGESTION_REJECT: { label: 'KI-Vorschlag abgelehnt', tone: 'warning' },
};

// Feld-IDs → lesbare Labels (nur die, die Analysten wirklich ändern).
const FIELD_LABEL: Record<string, string> = {
  state: 'State', status: 'Status', priority: 'Priorität', analyst: 'Analyst',
  decision: 'Decision', confidence: 'Confidence', recommendation: 'Empfehlung',
  notes: 'Notizen', title: 'Titel', description: 'Beschreibung', iocs: 'IoCs',
  closeReason: 'Schließgrund', customer: 'Kunde',
};
...
export function revisions(entries: AuditEntry[]): RevisionRow[] {
  ...
  return relevant.map((e, i) => ({
    ...
    changes: e.action === 'TICKET_CREATE' ? 'Erstanlage' : (changedFieldLabels(e).join(', ') || 'Aktualisierung'),
