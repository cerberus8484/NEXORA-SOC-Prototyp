...
function CopyBtn({ value }: { value: string }) {
  ...
  return <button type="button" aria-label="Kopieren" title={done ? 'Kopiert' : 'Kopieren'} onClick={() => { navigator.clipboard?.writeText(value); setDone(true); setTimeout(() => setDone(false), 1200); }} style={{ background: 'transparent', border: 'none', color: done ? 'var(--success)' : 'var(--text-dim)', cursor: 'pointer', flexShrink: 0, padding: 2 }}><Copy size={12} /></button>;
}
  ...
  return (
    <Section title="Commands" icon={<Terminal size={15} style={{ color: 'var(--accent)' }} />} right={<span style={{ fontSize: 11, color: 'var(--text-dim)' }}>{rows.length}</span>}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10 }}>
        ...
        {rows.length > limit && <button type="button" onClick={() => setLimit((l) => l + PAGE)} style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', color: 'var(--accent)', fontSize: 12, fontWeight: 600, cursor: 'pointer', padding: '5px 12px' }}>Weitere laden</button>}
      </div>
    </Section>
  );
}
...
function ExecutionChain({ rows }: { rows: CommandRow[] }) {
  ...
  return (
    <Section title="Execution Chain (Process Tree)" icon={<GitBranch size={15} style={{ color: 'var(--accent)' }} />}>
      {tree.length === 0 ? (
        <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Keine Parent-/Child-Prozessbeziehung in der Evidence.</div>
...
function ListCard({ title, icon, empty, children }: { title: string; icon: ReactNode; empty: boolean; children: ReactNode }) {
  return <Section title={title} icon={icon}>{empty ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Nicht erkannt.</div> : children}</Section>;
}
    ...
    return (
      <Card style={{ padding: '40px 24px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, textAlign: 'center', maxWidth: 460, margin: '0 auto' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>Noch keine Command-Evidence vorhanden</div>
          <div style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.55 }}>
            Dieses Ticket enthält derzeit keine strukturiert importierten Prozess- oder Command-Line-Daten (z. B. Sysmon Event 1 / Security 4688).
          </div>
        </div>
      </Card>
    );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ fontSize: 10.5, color: 'var(--text-dim)' }}>
        Quelle: Prozess-Evidence (Sysmon 1 / 4688) + korrelierte Flow-Prozesse (Sysmon 3). Severity/Confidence aus realen Signalen (Encoded/LOLBin/Flags/Korroboration) — kein erfundener Score. LOLBins/Interpreter über deterministisches Namens-Matching.
      </div>
    </div>
  );
}
