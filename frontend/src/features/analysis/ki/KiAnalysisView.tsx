...
function KiDisclaimer() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, color: 'var(--warning)', background: 'var(--warning-soft)', border: '1px solid var(--warning)', borderRadius: 'var(--radius-sm)', padding: '7px 10px' }}>
      <Sparkles size={14} style={{ flexShrink: 0 }} /> <span><Trans i18nKey="analysis.aiDisclaimer" components={{ b: <strong /> }} /></span>
    </div>
  );
}
...
export function KiAnalysisView({ ev, ticketId, canAct, onAddNote }: {
  ...
  const load = useCallback(async () => {
    ...
    catch (e) { if (alive.current) setLoadErr(e instanceof Error ? e.message : i18n.t('text.loadingFailed')); }
  ...
  async function propose(kind: string, note?: string) {
    ...
    catch (e) { setActErr(e instanceof Error ? e.message : tr('analysis.analysisFailed', { kind })); }
  ...
  if (loading) return <Card style={{ padding: '16px' }}><Spinner label={tr('analysis.loadingAiAnalysis')} /></Card>;
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 300px', gap: 18, alignItems: 'start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0 }}>
        ...
        <Section title="KI-Abfragen" icon={<Bot size={15} style={{ color: 'var(--accent)' }} />}>
          <div style={{ fontSize: 11.5, color: 'var(--text-dim)', marginBottom: 12 }}>{tr('analysis.selectAnalysisGetAiAssisted')}</div>
                ...
                <div key={q.kind} style={{ display: 'flex', gap: 10, padding: '12px', background: 'var(--bg-card-soft)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-sm)' }}>
                  ...
                  <div style={{ minWidth: 0, flex: 1 }}>
                    ...
                    <Button variant="ghost" size="sm" disabled={!canAct || busyKind !== null}
                      ...
                      onClick={() => void propose(q.kind)}>{busy ? tr('analysis.running') : tr('common.startVerb')}</Button>
                  </div>
                </div>
              );
            })}
          </div>
          {!canAct && <div style={{ fontSize: 11.5, color: 'var(--text-dim)', marginTop: 10 }}>{tr('settings.analysisFromRole')} <strong>analyst</strong>.</div>}
        ...
        {!a ? (
          <Card style={{ padding: '32px 24px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, textAlign: 'center', maxWidth: 460, margin: '0 auto' }}>
              ...
              <div style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--text)' }}>{tr('analysis.noAiAnalysisTicketYet')}</div>
              <div style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.55 }}>{tr('analysis.pickQueryAbove')}<strong>{tr('analysis.suggestions')}</strong>{tr('analysis.notConfirmedEvidence')}</div>
            </div>
          </Card>
        ) : (
          <>
            ...
            <div style={grid3}>
              ...
              <Section title="Evidence-backed Reasoning" icon={<CheckCircle2 size={15} style={{ color: 'var(--success)' }} />}>
                {(a.confirmedFacts.length === 0 && a.suspiciousIndicators.length === 0) ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('text.noEvidenceBackedPointsNamed')}</div> : (
              ...
              <Section title="Confidence & Recommendation" icon={<Activity size={15} style={{ color: 'var(--accent)' }} />}>
                ...
                {a.recommendedActions.length === 0 ? <div style={{ fontSize: 11.5, color: 'var(--text-dim)' }}>{tr('text.noneSuggested')}</div>
            ...
            <Section title="KI-Verlauf" icon={<Activity size={15} style={{ color: 'var(--accent)' }} />} right={<Badge tone="accent">{history.summary.totalRuns}</Badge>}>
              ...
              {history.points.length < 2 && (
                <div style={{ fontSize: 11.5, color: 'var(--text-dim)', marginBottom: 10 }}>{tr('analysis.onlyOneRun')}</div>
              )}
                ...
                {history.points.map((point, index) => (
                  <div key={point.id} role="listitem" style={{ padding: '11px 12px', background: 'var(--bg-card-soft)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-sm)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 8, fontSize: 11.5, color: 'var(--text-muted)' }}>
                      ...
                      <span style={{ flex: 1, minWidth: 220, color: 'var(--text)' }}>{point.note || tr('text.noAdditionalAnalysisRationale')}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <Section title="Why the AI says this / Traceability" icon={<Info size={15} style={{ color: 'var(--accent)' }} />}>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.55, marginBottom: 10 }}>{tr('analysis.aiBasisNote')}</div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--accent)', background: 'var(--accent-soft)', borderRadius: 'var(--radius-sm)', padding: '7px 10px', marginTop: 10 }}>
                <Info size={13} style={{ flexShrink: 0 }} />{tr('analysis.aiNeedsValidation')}</div>
            </Section>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Section title="Analyst Summary" icon={<Bot size={15} style={{ color: 'var(--accent)' }} />}>
          <div style={{ fontSize: 12, color: a?.assessment ? 'var(--text)' : 'var(--text-dim)', lineHeight: 1.5 }}>{a?.assessment || a?.whyItMatters || tr('analysis.noAiSummaryYetStart')}</div>
        </Section>
        ...
        <Section title="Threat Intel Snapshot" icon={<Globe size={15} style={{ color: 'var(--accent)' }} />}>
          ...
          <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 8 }}>{tr('analysis.fromTicketEvidenceNoLive')}</div>
        </Section>
        ...
        <Section title="Risk & Impact" icon={<Activity size={15} style={{ color: 'var(--accent)' }} />}>
          {a ? (
            <>
              ...
              <div style={{ fontSize: 10.5, color: 'var(--text-dim)' }}>{tr('analysis.confidenceComesStraightFromAi')}</div>
            </>
          ) : <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('analysis.noAssessmentYet')}</div>}
        </Section>
        ...
        <Section title="Approval Queue" icon={<Gavel size={15} style={{ color: 'var(--accent)' }} />}>
          ...
          <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 8 }}><Trans i18nKey="analysis.humanInTheLoop" components={{ b: <strong /> }} /></div>
        </Section>
      </div>
    </div>
  );
...
function ApprovalBar({ current, canAct, onApprove, onReject, onAddNote, onNeedsContext }: {
  ...
  return (
    <Section title="Analyst Approval" icon={<Gavel size={15} style={{ color: 'var(--accent)' }} />}>
      <div style={{ fontSize: 11.5, color: 'var(--text-dim)', marginBottom: 10 }}><Trans i18nKey="analysis.reviewAndDecide" components={{ b: <strong /> }} /></div>
      {reviewed ? (
        ...
        <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('settings.approvalFromRole')} <strong>analyst</strong>.</div>
      ) : (
        <>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <Button variant="ghost" size="sm" disabled={busy || noteDone || !onAddNote}
              ...
              title={onAddNote ? tr('analysis.appendsVerdictAssessmentTicketNotes') : tr('analysis.noDedicatedBackendWorkflowNot')}
              ...
              {noteDone ? tr('analysis.addedNote') : 'Add as Note'}</Button>
            <Button variant="ghost" size="sm" disabled={busy || !onNeedsContext} aria-expanded={wantCtx}
              title={onNeedsContext ? tr('analysis.askAiAgainSpecificAdditional') : tr('analysis.noDedicatedBackendWorkflowNot')}
              onClick={() => setWantCtx((v) => !v)}>Needs More Context</Button>
          </div>
          {wantCtx && onNeedsContext && (
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginTop: 8 }}>
              <input value={ctxNote} onChange={(e) => setCtxNote(e.target.value)} aria-label={tr('analysis.missingContextAi')}
                placeholder={tr('analysis.whatContextAiMissingFeeds')}
              ...
              <Button variant="ghost" size="sm" disabled={busy || !ctxNote.trim()}
                ...
                onClick={async () => { setBusy(true); try { await onNeedsContext(ctxNote.trim()); setWantCtx(false); setCtxNote(''); } finally { setBusy(false); } }}>{tr('analysis.reanalyse')}</Button>
            </div>
          )}
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginTop: 8 }}>
            <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder={tr('analysis.rejectionReasonRequiredReject')} aria-label="Ablehnungsgrund"
