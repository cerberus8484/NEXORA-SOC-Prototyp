...
export function HistoryTabView({ ticketId }: { ticketId: string }) {
  ...
  if (loading) return <Card style={{ padding: '14px 16px' }}><Spinner label="Historie wird geladen …" /></Card>;
  if (!entries || entries.length === 0) {
    return <Card style={{ padding: '14px 16px' }}><EmptyState title="Keine Historie" message="Für dieses Ticket sind noch keine Audit-Einträge vorhanden." /></Card>;
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={grid3}>
        ...
        <Section title="Audit-Integrität" icon={<ShieldCheck size={15} style={{ color: 'var(--success)' }} />}>
          {[['Append-only Audit-Log', 'garantiert'], ['IP gehasht (SHA-256)', 'aktiv'], ['Nur Feldnamen, keine Werte', 'DSGVO']].map(([k, v]) => (
          ...
          <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 6 }}>Design-Eigenschaften des Audit-Logs — keine erfundene Manipulationsprüfung.</div>
        </Section>
      </div>
      ...
      <Section title="Case History / Activity Timeline" icon={<Clock size={15} style={{ color: 'var(--accent)' }} />}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', marginBottom: 12 }}>
          <div style={{ position: 'relative', flex: 1, minWidth: 180 }}>
            ...
            <input value={q} onChange={(e) => { setQ(e.target.value); setLimit(PAGE); }} placeholder="Historie durchsuchen …" aria-label="Historie durchsuchen" style={{ ...inp, width: '100%', boxSizing: 'border-box', paddingLeft: 30 }} />
          </div>
          <select value={cat} onChange={(e) => { setCat(e.target.value); setLimit(PAGE); }} style={inp} aria-label="Kategorie"><option value="all">Alle Kategorien</option>{categories.map((c) => <option key={c} value={c}>{c}</option>)}</select>
          <select value={usr} onChange={(e) => { setUsr(e.target.value); setLimit(PAGE); }} style={inp} aria-label="Benutzer"><option value="all">Alle Benutzer</option>{users.map((u) => <option key={u} value={u}>{u}</option>)}</select>
        </div>

        {shown.length === 0 ? <div style={{ fontSize: 12.5, color: 'var(--text-dim)', padding: '12px 0' }}>Keine Einträge für diese Filter.</div>
          : shown.map((e, i) => {
            ...
            return (
              <div key={e.id} style={{ position: 'relative', paddingLeft: 26, paddingBottom: i === shown.length - 1 ? 0 : 14 }}>
                ...
                <div style={{ fontSize: 11.5, color: 'var(--text-muted)', marginTop: 2 }}>{e.actorLabel || 'System'}{d.detail ? ` · Felder: ${d.detail}` : ''}</div>
              </div>
            );
          })}
      ...
      <div style={grid3}>
        <Section title="Revision History" icon={<GitBranch size={15} style={{ color: 'var(--accent)' }} />}>
          {revs.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Keine Revisionen.</div> : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr><th style={th}>Version</th><th style={th}>Datum</th><th style={th}>Von</th><th style={th}>Änderungen</th></tr></thead>
        ...
        <Section title="Decision Timeline" icon={<CheckCircle2 size={15} style={{ color: 'var(--success)' }} />}>
          {decisions.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Keine Decision-/Status-Änderungen.</div>
            : decisions.map((e, i) => {
        ...
        <Section title="Change Details" icon={<Pencil size={15} style={{ color: 'var(--accent)' }} />}>
          {!lastChange ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Keine Feldänderung protokolliert.</div> : (
            <>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr><th style={th}>Feld</th><th style={th}>Before</th><th style={th}>After</th></tr></thead>
                <tbody>{changedFieldLabels(lastChange).map((f) => (
                  <tr key={f}><td style={tdc}>{f}</td><td style={{ ...tdc, color: 'var(--text-dim)' }}>nicht protokolliert</td><td style={{ ...tdc, color: 'var(--text-dim)' }}>nicht protokolliert</td></tr>
                ))}</tbody>
              </table>
              <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 8 }}>Werte werden aus Datenschutzgründen (DSGVO) NICHT protokolliert — nur die geänderten Feldnamen.</div>
            </>
          )}
        </Section>
      </div>
