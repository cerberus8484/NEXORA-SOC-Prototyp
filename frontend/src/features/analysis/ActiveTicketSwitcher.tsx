...
export function ActiveTicketSwitcher() {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 6 }}>
      <button type="button" aria-haspopup="dialog" aria-expanded={open} aria-label={tr('tickets.switchActive')}
        style={s.chip} onClick={() => setOpen((o) => !o)}>
        <span className="mono" style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--text)', flexShrink: 0 }}>
          {active?.ticketNr || active?.id?.slice(0, 8) || tr('text.noTicket')}
        </span>
      ...
      <button type="button" aria-label={tr('tickets.previous')} style={s.arrow} disabled={visibleTickets.length < 2} onClick={() => step(-1)}><ArrowLeft size={15} /></button>
      <button type="button" aria-label={tr('analysis.nextTicket')} style={s.arrow} disabled={visibleTickets.length < 2} onClick={() => step(1)}><ArrowRight size={15} /></button>

      {open && (
        <div role="dialog" aria-label={tr('tickets.selection')} style={s.pop}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', borderBottom: '1px solid var(--border-soft)', fontSize: 12, fontWeight: 600, color: 'var(--text-muted)' }}>
            <UserCheck size={13} style={{ color: 'var(--accent)' }} /> {seeAll ? tr('analysis.allTickets') : tr('tickets.myAssigned')}
          ...
          <div style={{ padding: 10, borderBottom: '1px solid var(--border-soft)', position: 'relative' }}>
            ...
            <input autoFocus placeholder={seeAll ? tr('tickets.searchPlaceholder') : tr('tickets.searchMinePlaceholder')} value={q} onChange={(e) => setQ(e.target.value)} style={s.input} />
          </div>
          <div style={{ maxHeight: 380, overflowY: 'auto' }}>
            {filtered.length === 0 ? (
              <div style={{ padding: 16, fontSize: 12.5, color: 'var(--text-dim)', textAlign: 'center' }}>
                {visibleTickets.length === 0 ? (seeAll ? tr('text.noTicketsAvailable') : tr('analysis.noTicketAssignedYouRight')) : tr('text.noTicketsFound')}
              </div>
            ) : filtered.map((t) => (
              <button key={t.id} type="button" style={{ ...s.row, background: t.id === activeId ? 'var(--accent-soft)' : 'transparent' }} onClick={() => go(t.id)}>
                ...
                <div style={{ fontSize: 12.5, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.title || tr('tickets.untitled')}</div>
          ...
          <button type="button" style={{ ...s.row, borderBottom: 'none', display: 'flex', alignItems: 'center', gap: 6, color: 'var(--accent)', fontSize: 12.5, fontWeight: 600 }}
            ...
            <ExternalLink size={13} />{tr('tickets.openAll')}</button>
        </div>
      )}
    </div>
  );
