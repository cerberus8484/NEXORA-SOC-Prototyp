...
function KvRow({ k, v, mono, danger, copy }: { k: string; v?: ReactNode; mono?: boolean; danger?: boolean; copy?: string }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '128px 1fr auto', gap: 10, alignItems: 'baseline', padding: '3px 0', fontSize: 12 }}>
      ...
      <span style={{ color: empty ? 'var(--text-dim)' : danger ? 'var(--danger)' : 'var(--text)', fontFamily: mono && !empty ? 'var(--font-mono)' : undefined, fontStyle: empty ? 'italic' : 'normal', overflowWrap: 'anywhere', minWidth: 0 }}>{empty ? tr('common.notAvailable') : v}</span>
...
function CopyBtn({ value }: { value: string }) {
  ...
  return <button type="button" aria-label={tr('common.copy')} title={c ? tr('common.copied') : tr('common.copy')} onClick={() => { navigator.clipboard?.writeText(value); setC(true); setTimeout(() => setC(false), 1200); }} style={{ background: 'transparent', border: 'none', color: c ? 'var(--success)' : 'var(--text-dim)', cursor: 'pointer', padding: 0 }}><Copy size={13} /></button>;
}
...
export function EvidenceView({ t, ev, tl, ticketId, risk, refreshKey, onOpenSection }: {
  ...
  if (loading) return <Card style={{ padding: 16 }}><Spinner label={tr('analysis.loadingEvidenceRecords')} /></Card>;
  if (records.length === 0) {
    return (
      <Card style={{ padding: '40px 24px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, textAlign: 'center', maxWidth: 480, margin: '0 auto' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>{tr('analysis.noEvidenceRecordsYet')}</div>
          <div style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.55 }}>{tr('analysis.noSourceEvents')}<strong>Import</strong>{tr('analysis.orSaveAsEvidence')}</div>
        </div>
      </Card>
    );
  }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '210px 270px minmax(0, 1fr) 280px', gap: 14, alignItems: 'start' }}>
      ...
      <Card style={{ padding: '12px 14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          <button type="button" onClick={() => setFilter(EMPTY_FILTER)} style={{ background: 'none', border: 'none', color: 'var(--accent)', cursor: 'pointer', fontSize: 11.5 }}>{tr('analysis.reset')}</button>
        </div>
        <div style={lbl}>Source</div>
        <select value={filter.source} onChange={(e) => set({ source: e.target.value })} style={{ ...inp, marginBottom: 10 }}>
          <option value="all">{tr('analysis.allSources')}</option>{facets.sources.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
        ...
        <select value={filter.eventType} onChange={(e) => set({ eventType: e.target.value })} style={{ ...inp, marginBottom: 10 }}>
          <option value="all">{tr('analysis.allTypes')}</option>{facets.eventTypes.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
        <div style={lbl}>Severity</div>
        <select value={filter.severity} onChange={(e) => set({ severity: e.target.value })} style={{ ...inp, marginBottom: 12 }}>
          <option value="all">{tr('common.all')}</option>{facets.severities.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
      ...
      <Card style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '11px 12px', borderBottom: '1px solid var(--border-soft)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <button type="button" title={tr('common.refresh')} onClick={() => setReloadKey((k) => k + 1)} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: 'var(--text-dim)', cursor: 'pointer' }}><RefreshCw size={14} /></button>
          </div>
          <div style={{ position: 'relative' }}>
            <Search size={13} style={{ position: 'absolute', left: 9, top: 8, color: 'var(--text-dim)' }} />
            <input value={filter.q} onChange={(e) => set({ q: e.target.value })} placeholder={tr('evidence.searchPlaceholder')} style={{ ...inp, paddingLeft: 28 }} aria-label={tr('evidence.search')} />
          </div>
        </div>
        <div style={{ maxHeight: 620, overflowY: 'auto' }}>
          {filtered.length === 0 ? <div style={{ padding: 16, fontSize: 12, color: 'var(--text-dim)', textAlign: 'center' }}>{tr('analysis.noRecordsTheseFilters')}</div>
            : filtered.map((r) => {
      ...
      <Card style={{ padding: '14px 16px', minHeight: 300 }}>
        {selected ? <RecordDetail r={selected} /> : <div style={{ fontSize: 12.5, color: 'var(--text-dim)' }}>{tr('analysis.noRecordSelected')}</div>}
      </Card>
