...
const LABELS: Record<CorrelationJobStatus, string> = {
  current:     'Aktuell',
  pending:     i18n.t('analysis.notCalculatedYet'),
  running:     i18n.t('text.updating'),
  retrying:    i18n.t('correlators.retryScheduled'),
  failed:      i18n.t('correlators.calculationFailed'),
  superseded:  i18n.t('analysis.dataHasChangedResultWill'),
  unavailable: i18n.t('analysis.correlationStatusCurrentlyUnavailable'),
};
...
export function sanitizeFailureReason(reason: string | null): string {
  if (!reason) return i18n.t('correlators.calculationFailed');
  const t = reason.trim();
  if (!t || !SAFE_PATTERN.test(t)) return i18n.t('correlators.calculationFailed');
