...
export function buildTimelineGroups(_t: Ticket, ev: ParsedEvidence, tl: TicketTimeline | null, network?: NetworkCorrelation | null): TimelineGroup[] {
  ...
  if (ruleName || ruleId || desc) {
    const sub: TimelineSubEvent[] = [];
    if (ruleId) sub.push({ time: base, text: i18n.t('analysis.ruleTriggered', {
      ruleId,
      level: blank(ev.detection.severity) ? i18n.t('analysis.levelSuffix', { level: ev.detection.severity }) : '',
      name: ruleName ? `: ${ruleName}` : '',
    }), source: sys });
    else if (desc) sub.push({ time: base, text: desc, source: sys });
    groups.push({ id: 'detection', title: ruleName || i18n.t('analysis.detectionTriggered'), category: 'detection', time: base, meta: [], sub });
  }
