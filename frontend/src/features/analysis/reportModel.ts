...
const PRIORITY_LABEL: Record<string, string> = {
  critical: 'Kritisch', high: 'Hoch', medium: 'Mittel', low: 'Niedrig', info: 'Information',
};
const CUSTOMER_STATUS: Record<string, string> = {
  open: 'In Bearbeitung', progress: 'In Bearbeitung', closed: 'Abgeschlossen',
  fp: 'Kein Sicherheitsvorfall (Fehlalarm)',
};
...
export function buildIncidentReport(input: IncidentReportInput): ReportDoc {
  ...
  if (indicatorItems.length) {
    sections.push({ category: 'indicators', heading: 'Auffällige Indikatoren', items: indicatorItems });
  }
  ...
  const assessmentItems = items(
    item(t.decision ? `Einstufung durch Analyst: ${t.decision}` : null, 'Analyst'),
  );
  if (assessmentItems.length) {
    sections.push({
      category: 'assessment',
      heading: 'Analystenbewertung',
      paragraphs: ['Einschätzung des Analysten — keine bestätigte Tatsache.'],
  ...
  const actionItems = items(
    ...
    ...lines(t.recommendation).map((l) => item(l, 'Ticket-Empfehlung')),
    ...recommendBullets.map((b) => item(b.text, 'Evidence')),
  );
  if (actionItems.length) {
    sections.push({ category: 'actions', heading: 'Maßnahmen & Empfehlungen', items: actionItems });
  }

  // ── 6. Traceability — woraus stammt der Report ───────────────────────────
  const traceItems = items(
    item(`Ticket: ${id}`, 'Ticket'),
    input.evidence ? item(`Evidence-Quelle: ${input.evidence.metadata.agentName || input.evidence.detection.sourceSystem || input.evidence.type}`, 'Evidence') : null,
  ...
  sections.push({ category: 'traceability', heading: 'Traceability / Quellen', items: traceItems });
...
export function buildCustomerReport(t: Ticket, bullets: SummaryBullet[]): ReportDoc {
  ...
  sections.push({
    ...
    paragraphs: summary.length ? summary : [t.title || 'Ein sicherheitsrelevantes Ereignis wurde bearbeitet.'],
  });
  ...
  sections.push({
    ...
    heading: 'Einstufung',
    fields: fields(
      field('Schweregrad', PRIORITY_LABEL[t.priority] ?? t.priority),
      field('Bearbeitungsstatus', CUSTOMER_STATUS[t.status] ?? t.status),
    ),
  });

  const actions = lines(t.actions);
  if (actions.length) {
    sections.push({ category: 'meta', heading: 'Durchgeführte Maßnahmen', bullets: actions });
  }

  if ((t.recommendation ?? '').trim()) {
    sections.push({ category: 'meta', heading: 'Empfehlung', paragraphs: lines(t.recommendation) });
  }
