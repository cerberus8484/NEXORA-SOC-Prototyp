...
export function buildAnalystSummary(ev: ParsedEvidence, tl: TicketTimeline | null): SummaryBullet[] {
  ...
  if (exe) {
    bullets.push({ kind: 'process', text: `${exe} wurde${host ? ` auf ${host}` : ''}${user ? ` durch ${user}` : ''} ausgeführt.` });
  }

  const decoded = decodeEncodedCommand(ev.process?.commandLine);
  if (decoded) bullets.push({ kind: 'decoded', text: 'Encoded Command erkannt und dekodiert.' });
  else if (ev.payload.containsBase64 === true) bullets.push({ kind: 'decoded', text: 'Base64-Inhalt im Payload erkannt.' });
  ...
  if (ev.destination.ip) {
    ...
    bullets.push({ kind: 'network', text: `Outbound-Verbindung zu ${ev.destination.ip}${port}${proto ? ` (${proto.toUpperCase()})` : ''}.` });
  }

  const totalBytes = (ev.network.bytesSent ?? 0) + (ev.network.bytesReceived ?? 0);
  if (totalBytes > 0) bullets.push({ kind: 'data', text: `${fmtBytes(totalBytes)} Daten übertragen.` });
  ...
  if (tl) {
    ...
    if (blocked > 0) bullets.push({ kind: 'network', text: `Firewall hat ${blocked} Verbindungen geblockt.` });
  }
  ...
  if (decoded && ev.destination.ip) {
    bullets.push({ kind: 'c2', text: 'Mögliche Command-and-Control-Kommunikation.' });
  }

  if (bullets.length === 0) {
    return [{ kind: 'info', text: 'Keine strukturierte Evidence vorhanden — Rohdaten importieren oder Wazuh-Quelle prüfen.' }];
  }
  bullets.push({ kind: 'recommend', text: 'Empfehlung: Commands und Payloads prüfen, Decision dokumentieren.' });
  return bullets;
}
...
export function buildExportMarkdown(t: Ticket, bullets: SummaryBullet[], entities: EntityItem[], tl: TicketTimeline | null): string {
  const lines: string[] = [
    ...
    `**Titel:** ${t.title || '—'}`,
    `**Priorität:** ${t.priority} · **State:** ${t.state ?? 'OPEN'} · **Status:** ${t.status}`,
    `**Analyst:** ${t.analyst || 'unassigned'} · **Kunde:** ${t.customer || '—'} · **Quelle:** ${t.source || 'manual'}`, '',
    '## Analyst Summary',
