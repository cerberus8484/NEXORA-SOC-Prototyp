...
export function PlaybooksTabView({ analystState, onSaveAnalystState, saveError, entities }: {
  ...
  function exportPlan() {
    const lines = [`Playbook: ${activePb.name}`, `Fortschritt: ${progress.done}/${progress.total} (${progress.pct}%)`, '', 'Schritte:',
          ...
          {PLAYBOOKS.map((p) => {
            ...
            return (
              <div key={p.id} style={{ padding: '12px', borderRadius: 'var(--radius-sm)', border: `1px solid ${active ? 'var(--accent)' : 'var(--border-soft)'}`, background: active ? 'var(--accent-soft)' : 'var(--bg-card-soft)' }}>
                ...
                {active ? <Badge tone="accent">Ausgewählt</Badge> : <Button variant="ghost" size="sm" onClick={() => selectPlaybook(p.id)}>Auswählen</Button>}
              </div>
            );
          })}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0 }}>
          ...
          <Section title={`Selected Playbook: ${activePb.name}`} icon={<BookOpen size={15} style={{ color: 'var(--accent)' }} />}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', paddingTop: 10, borderTop: '1px solid var(--border-soft)' }}>
              <span style={lbl}>Aktueller Schritt:</span>
              ...
              <Button variant="ghost" size="sm" icon={<ChevronRight size={13} />} disabled={currentStep >= activePb.steps.length - 1} onClick={advance}>Nächster Schritt</Button>
            </div>
            <div style={{ fontSize: 10.5, color: 'var(--success)', marginTop: 8 }}><Check size={11} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 4 }} />Fortschritt wird am Ticket gespeichert.</div>
          </Section>
          ...
          <Section title="Playbook Steps" icon={<ListChecks size={15} style={{ color: 'var(--accent)' }} />} right={<span style={{ fontSize: 11, color: 'var(--text-dim)' }}>{activePb.steps.length}</span>}>
            ...
            <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 8 }}>Owner/Updated pro Schritt sind im Datenmodell nicht hinterlegt — bewusst „—".</div>
          </Section>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <Section title="Quick Actions" icon={<Layers size={15} style={{ color: 'var(--accent)' }} />}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <Button variant="ghost" size="sm" icon={<UserPlus size={13} />} disabled title="Zuweisung noch nicht im Backend">Assign Playbook</Button>
            </div>
          </Section>
        </div>
      </div>
