...
function TopConversations({ tl, network }: { tl: TicketTimeline | null; network?: NetworkCorrelation | null }) {
  ...
  return (
    <Section title="Top Conversations" icon={<ListOrdered size={15} style={{ color: 'var(--accent)' }} />} right={<span style={{ fontSize: 11, color: 'var(--text-dim)' }}>{rows.length}</span>}>
      {rows.length === 0 ? (
        <div style={{ fontSize: 12, color: 'var(--text-dim)', lineHeight: 1.5 }}>Keine Verbindungen im Zeitfenster — Quelle liefert derzeit keine Flows/Events.</div>
      ) : (
...
function NatTranslation({ t, ev, network }: { t: Ticket; ev: ParsedEvidence; network?: NetworkCorrelation | null }) {
  ...
  return (
    <Section title="NAT Translation" icon={<ArrowLeftRight size={15} style={{ color: 'var(--warning)' }} />}>
      {rows.length === 0 ? (
        <div style={{ fontSize: 12, color: 'var(--text-dim)', lineHeight: 1.5 }}>Nicht verfügbar — die Quelle dieses Tickets liefert keine NAT-Translation (z. B. reines Host-/Endpoint-Event).</div>
      ) : (
        <>
          ...
          <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 10 }}>Unveränderte Felder werden als Passthrough (Pre = Post) gezeigt.</div>
        </>
      )}
    </Section>
  );
...
function Geolocation({ ev, network }: { ev: ParsedEvidence; network?: NetworkCorrelation | null }) {
  ...
  return (
    <Section title="Geolocation / Reputation" icon={<MapPin size={15} style={{ color: 'var(--accent)' }} />}>
      {!geo.hasAny ? (
        <div style={{ fontSize: 12, color: 'var(--text-dim)', lineHeight: 1.5 }}>Nicht verfügbar — keine öffentliche IP mit Geo-/ASN-/Reputationsdaten (interne/private IPs werden nicht extern abgefragt).</div>
      ) : (
        <>
          ...
          <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 8 }}>
            {geo.source === 'provider' || geo.source === 'cache' ? 'Quelle: VirusTotal/AbuseIPDB (auto-angereichert). ' : ''}Reputation = Kontext, keine automatische Ticket-Entscheidung.
          </div>
        </>
      )}
    </Section>
...
function HoneypotSessionCard({ s }: { s: HoneypotSessionView }) {
  const authBadge = s.authSuccess === true
    ...
    : s.authSuccess === false ? <Badge tone="warning">nur Fehlversuche</Badge>
      : <Badge tone="muted">keine Logins</Badge>;
  return (
    <div style={{ border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-sm)', padding: '12px 14px', background: 'var(--bg-card-soft)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0 18px' }}>
        <KV label="Honeypot-Ziel" value={s.honeypotIp ? `${s.honeypotIp}${s.port != null ? `:${s.port}` : ''}` : undefined} mono />
        ...
        <KV label="Passwort beobachtet" value={s.passwordObserved ? `ja (${s.passwordAttempts})` : 'nein'} />
      </div>

      {!s.honeypotIp && (
        <div style={{ marginTop: 8, fontSize: 11, color: 'var(--text-dim)', fontStyle: 'italic' }}>
          Session-basierte Verbindung — Ziel-IP/Port von Cowrie nicht geliefert (kein vollständiges 5-Tuple).
        </div>
      )}

      {s.usernames.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <div style={lblCss}>Beobachtete Usernamen{s.usernamesTruncated ? ' (gekürzt)' : ''}</div>
      ...
      <div style={{ marginTop: 10, paddingTop: 8, borderTop: '1px solid var(--border-soft)', fontSize: 11.5 }}>
        {s.hasRelatedFlow
          ...
          : <span style={{ color: 'var(--text-dim)' }}>Kein zugehöriger Netzwerkflow im Zeitfenster.</span>}
      </div>
    </div>
  );
}
...
export function HoneypotSessions({ network }: { network?: NetworkCorrelation | null }) {
  ...
  return (
    <Section title="Honeypot Sessions" icon={<Bug size={15} style={{ color: 'var(--danger)' }} />}
      ...
      {groups.length === 0 ? (
        <div style={{ fontSize: 12, color: 'var(--text-dim)', lineHeight: 1.5 }}>Keine Honeypot-Sessions für diesen Ticket-Kontext gefunden.</div>
      ) : (
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
        <div className="mono" style={{ fontSize: 12.5, color: 'var(--text)' }}>
          ...
          {dst
            ...
            : <span style={{ color: 'var(--text-dim)' }}>mehrere mögliche Verbindungen — nicht eindeutig</span>}
        </div>
  ...
  return (
    <Section title="Correlated Exposure Path" icon={<ArrowLeftRight size={15} style={{ color: 'var(--warning)' }} />} right={<span style={{ fontSize: 11, color: 'var(--text-dim)' }}>{rows.length}</span>}>
      <div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 10, lineHeight: 1.5 }}>
        Korreliert aus Firewall- und Honeypot-Evidence (Zeit + externe IP + Port). <strong style={{ color: 'var(--text-muted)' }}>Keine bestätigte NAT-Translation.</strong>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ fontSize: 10.5, color: 'var(--text-dim)' }}>
        Quelle: aus dem Ticket abgeleitete Evidence + korrelierte Flows/Timeline (Wazuh-Indexer). Fehlende Felder bleiben „—" — keine erfundenen Werte.
      </div>
    </div>
  );
}
