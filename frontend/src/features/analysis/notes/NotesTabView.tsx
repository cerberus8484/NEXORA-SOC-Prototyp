...
export function NotesTabView({ notes, currentUser, onAddNote, analystState, onSaveAnalystState, saveError }: NotesTabProps) {
  ...
  async function save() {
    ...
    try {
      ...
      setLocalErr(e instanceof Error ? e.message : 'Notiz konnte nicht gespeichert werden.');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(240px, 300px) minmax(0, 1fr)', gap: 14, alignItems: 'start' }}>
        ...
        <Section title="Notes Timeline" icon={<FileText size={15} style={{ color: 'var(--accent)' }} />}>
          {newest.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Noch keine Notizen. Lege rechts die erste an.</div>
            : newest.map((n, i) => {
        ...
        {/* Editor */}
        <Section title="Neue Notiz" icon={<FileText size={15} style={{ color: 'var(--accent)' }} />}>
          ...
          <textarea ref={taRef} value={body} onChange={(e) => setBody(e.target.value)} rows={6}
            placeholder="Notiz hier eingeben … (Markdown)"
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 5, alignItems: 'center' }}>
              {tags.map((t) => (
                <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'var(--bg-card-soft)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-pill)', fontSize: 11, color: 'var(--text)', padding: '2px 8px' }}>
                  {t}<button type="button" aria-label={`Tag ${t} entfernen`} onClick={() => setTags((prev) => prev.filter((x) => x !== t))} style={{ background: 'none', border: 'none', color: 'var(--text-dim)', cursor: 'pointer', display: 'inline-flex', padding: 0 }}><X size={11} /></button>
                </span>
              ))}
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                <input value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addTag(); } }} placeholder="+ Tag" aria-label="Tag hinzufügen"
                ...
                <button type="button" aria-label="Tag übernehmen" onClick={addTag} style={{ ...tbtn, color: 'var(--accent)' }}><Plus size={13} /></button>
              </span>
            </div>
          </div>
      ...
      <Section title="Recent Notes" icon={<FileText size={15} style={{ color: 'var(--accent)' }} />} right={<Badge tone="accent">{entries.length}</Badge>}>
        {newest.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Noch keine Notizen.</div> : (
        ...
        <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 10 }}>Notizen werden als lesbare Blöcke im Ticket-Feld gespeichert (kein separates Notes-Backend). Autor = angemeldeter Analyst.</div>
      </Section>
      ...
      <Section title={`Investigation Checklist (${doneCount}/${checklistItems.length})`} icon={<ListChecks size={15} style={{ color: 'var(--success)' }} />}>
        ...
        <div style={{ fontSize: 11, color: 'var(--success)', marginTop: 8 }}>
          ...
          Checkliste wird beim Abhaken am Ticket gespeichert — Speicherfehler erscheinen oben.
        </div>
      </Section>
    </div>
  );
