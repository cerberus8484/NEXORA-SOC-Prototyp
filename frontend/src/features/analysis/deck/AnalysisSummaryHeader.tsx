      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap' }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--text)' }}>{t.title || tr('tickets.untitled')}</span>
        ...
        {canAct && (
          <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            ...
            <Button variant="ghost" size="sm" icon={<HardDriveDownload size={14} />} disabled={busy || collecting} onClick={onCollectEvidence}
              ...
              {collecting ? 'Sammelt …' : tr('analysis.collectEndpointArtefacts')}
            </Button>
            {(t.state ?? 'OPEN') === 'OPEN' && <Button variant="danger" size="sm" icon={<XCircle size={14} />} disabled={busy} onClick={onClose}>{tr('common.close')}</Button>}
            {canAdmin && <Button variant="danger" size="sm" icon={<Trash2 size={14} />} disabled={busy} onClick={onDelete}
              title={tr('analysis.deleteTicketPermanentlyAdministratorsOnly')}>{tr('common.delete')}</Button>}
          </div>
        )}
      </div>
