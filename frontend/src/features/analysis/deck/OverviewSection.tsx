    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
      ...
      {corrRender.kind === 'meta' && corrRender.meta.eventCount > 1 && (
        <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap',
          ...
          <span style={{ fontSize: 11.5, color: 'var(--text-muted)' }}>{tr('analysis.correlatedFrom')}<strong style={{ color: 'var(--text)' }}>{corrRender.meta.eventCount} Events</strong>{tr('analysis.sourcesSuffix')}</span>
      ...
      {corrRender.kind === 'loading' && (
        <div style={{ gridColumn: '1 / -1', padding: '8px 12px', background: 'var(--bg-input)',
          ...
          fontSize: 12, color: 'var(--text-dim)' }}>{tr('analysis.correlationRunning')}</div>
      )}
      {corrRender.kind === 'failed_no_result' && (
        <div style={{ gridColumn: '1 / -1', padding: '8px 12px', background: 'var(--bg-input)',
          border: '1px solid var(--danger, #ef4444)', borderRadius: 'var(--radius-sm)',
          fontSize: 12, color: 'var(--danger, #ef4444)' }}>{tr('analysis.correlationFailed')}</div>
      )}
      {corrRender.kind === 'superseded' && (
        <div style={{ gridColumn: '1 / -1', padding: '8px 12px', background: 'var(--bg-input)',
          border: '1px solid var(--warning, #f59e0b)', borderRadius: 'var(--radius-sm)',
          fontSize: 12, color: 'var(--warning, #f59e0b)' }}>{tr('analysis.correlationRecalculating')}</div>
      )}
      {corrRender.kind === 'unavailable' && (
        <div style={{ gridColumn: '1 / -1', padding: '8px 12px', background: 'var(--bg-input)',
          border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-sm)',
          fontSize: 12, color: 'var(--text-dim)' }}>{tr('analysis.correlationUnavailable')}</div>
      )}
      ...
      <DeckCard title="Top Conversations" icon={<Network size={15} style={{ color: 'var(--accent)' }} />}
        action={<Button variant="ghost" size="sm" onClick={() => onOpenSection('network')}>{tr('analysis.allFlows')}</Button>}>
        {tlLoading ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('analysis.loadingTimeline')}</div>
          : conversations.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('analysis.noCorrelatedNetworkFlowsAvailable')}</div>
          : (
        ...
        <Hint>{tr('analysis.groupedFromOffenseTimelineWazuh')}</Hint>
      </DeckCard>
      ...
      <DeckCard title="Communication Map" icon={<Globe size={15} style={{ color: 'var(--accent)' }} />}>
        ...
        <Hint>{tr('analysis.evidenceSourceHint', { nat: ev.nat.postNatSourceIp ? tr('analysis.natDetected') : '' })}</Hint>
      </DeckCard>

      {/* C) Event Timeline Preview */}
      <DeckCard title="Event Timeline" icon={<Clock size={15} style={{ color: 'var(--purple)' }} />}
        action={<Button variant="ghost" size="sm" onClick={() => onOpenSection('timeline')}>{tr('analysis.allEvents')}</Button>}>
        {tlLoading ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('analysis.loadingTimeline')}</div>
          : previewEvents.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('text.noTimelineEventsTimeWindow')}</div>
          : previewEvents.map((e) => (
      ...
      <DeckCard title="Payload Preview" icon={<FileCode size={15} style={{ color: 'var(--warning)' }} />}
        ...
        {payloadTab === 'raw' && (
          ev.raw || ev.payload.preview
            ...
            : <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('text.noRawPayloadAvailable')}</div>
        )}
      </DeckCard>
      ...
      <DeckCard title="Commands Executed" icon={<Terminal size={15} style={{ color: 'var(--success)' }} />} style={{ gridColumn: '1 / -1' }}
        ...
        {!ev.process?.image && !ev.process?.commandLine ? (
          <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('text.noProcessEvidenceAvailableSysmon')}</div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 24px' }}>
            <div>
