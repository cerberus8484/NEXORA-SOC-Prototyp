...
const reasonLabel = (): Record<string, string> => ({
  source_provides_none: i18n.t('analysis.sourceDoesNotProvideField'),
  field_missing:        i18n.t('misc.notContainedEvent'),
  inventory_not_loaded: i18n.t('analysis.fromHostInventory'),
  not_in_inventory:     i18n.t('analysis.ipNotMappedToAgent'),
  threat_intel_pending: i18n.t('analysis.fromThreatIntelDns'),
  no_match:             i18n.t('misc.noMatch'),
});
...
const reasonOf = (mr: Record<string, string> | undefined, field: string) =>
  reasonLabel()[mr?.[field] || ''] || i18n.t('misc.notAvailable');
...
export function NetworkFlowsTable({ network }: { network?: NetworkCorrelation | null }) {
  ...
  if (flows.length === 0) {
    return (
      <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{tr('analysis.noCorrelatedFlows')}</div>
    );
  }
  return (
          ...
          return (
            <tr key={`${f.timestamp}-${f.sourceIp}:${f.sourcePort ?? ''}-${f.destinationIp}:${f.destinationPort ?? ''}`}>
              ...
              <td style={{ ...TD, verticalAlign: 'top' }}>
                ...
                <MetaLine label="Proto:" value={(f.protocol || '').toUpperCase() || null} missing={tr('misc.notAvailable')} />
              </td>
              <td style={{ ...TD, verticalAlign: 'top' }}>
                <div style={{ fontFamily: 'var(--font-mono)' }}>{baseName(f.processImage) || <Dash title={reasonOf(f.missingReason, 'processImage')} />}</div>
                <MetaLine label="User:" value={f.user} missing={tr('misc.notProvidedSource')} />
              </td>
    ...
    <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
      {gaps.map((g) => (
        <span key={`${g.field}|${g.missingReason}`} title={`${g.count}×`}
          ...
          {g.field}: {reasonLabel()[g.missingReason] || g.missingReason}
        </span>
      ))}
    </div>
  );
