  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <Card style={box}>
        ...
        {!targetIp ? (
          <div style={{ fontSize: 11.5, color: 'var(--text-dim)' }}>{tr('analysis.noIpEnrichDestinationSource')}</div>
        ) : !tiResult ? (
          <>
            <div style={{ fontSize: 11.5, color: 'var(--text-muted)', marginBottom: 8 }}>{tr('common.targetColon')} <span className="mono" style={{ color: 'var(--text)' }}>{targetIp}</span></div>
            ...
            <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 8 }}>{tr('analysis.mockIntelligenceProvidersVirustotalAbuseipdb')}</div>
          </>
        ) : (
          <>
            ...
            <Button variant="ghost" size="sm" disabled={busy} onClick={onEnrich} style={{ width: '100%', marginTop: 8 }}>{tr('analysis.checkAgain')}</Button>
            <div style={{ fontSize: 10.5, color: 'var(--text-dim)', marginTop: 6 }}>{tr('analysis.threatIntelContextNotAutomatic')}</div>
          </>
        )}
      </Card>
      ...
      <Card style={box}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <Button variant={saved ? 'ghost' : 'primary'} size="sm" disabled={!tiResult || saved} onClick={onSaveEvidence} title={!tiResult ? tr('text.runEnrichFirst') : ''}>{saved ? '✓ Als Evidence gespeichert' : 'Add enrichment as Evidence'}</Button>
