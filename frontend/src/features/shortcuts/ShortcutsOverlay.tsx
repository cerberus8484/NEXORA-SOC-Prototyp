    ...
    <div style={OVERLAY} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div ref={panelRef} style={PANEL} role="dialog" aria-modal="true" aria-labelledby="shortcuts-title">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <Button size="sm" variant="ghost" icon={<X size={14} />} onClick={onClose} aria-label="Schließen" style={{ marginLeft: 'auto' }} />
        </div>

        <div style={SECTION_LABEL}>Navigation — <Kbd>{LEADER_KEY}</Kbd> dann …</div>
        <div>
          {NAV_SHORTCUTS.map((s) => (
            <div key={s.key} style={ROW}>
              ...
              <span style={{ display: 'inline-flex', gap: 4, alignItems: 'center' }}>
                <Kbd>{LEADER_KEY}</Kbd>
                <span style={{ color: 'var(--text-dim)', fontSize: 11 }}>dann</span>
                <Kbd>{s.key}</Kbd>
              </span>
            </div>
          ))}
        ...
        <div style={{ fontSize: 11, color: 'var(--text-dim)', lineHeight: 1.5 }}>
          Shortcuts greifen nicht, während ein Eingabefeld den Fokus hat. <Kbd>Esc</Kbd> schließt dieses Fenster.
        </div>
      </div>
    </div>
  );
