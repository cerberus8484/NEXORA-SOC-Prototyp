...
export const EVIDENCE_TYPE_OPTIONS: [string, string][] = [
  ...
  ['threat_intel', 'Threat Intel'], ['file_upload', i18n.t('evidence.fileImport')], ['other', 'Other'],
];
...
export function validateEvidenceDraft(d: EvidenceDraft): string | null {
  if (!d.ticketId.trim()) return i18n.t('app.ticketIdRequired');
  if (!UUID_RE.test(d.ticketId.trim())) return i18n.t('app.ticketIdMustValidUuid');
  if (!d.title.trim()) return i18n.t('app.titleRequired');
  return null;
}
...
export function validateUploadFile(file: { name: string; size: number }): string | null {
  ...
  if (!name) return i18n.t('evidence.filenameMissing');
  const dot = name.lastIndexOf('.');
  const ext = dot >= 0 ? name.slice(dot).toLowerCase() : '';
  if (!(ALLOWED_UPLOAD_EXTENSIONS as readonly string[]).includes(ext)) {
    return i18n.t('evidence.fileTypeNotAllowed', { list: ALLOWED_UPLOAD_EXTENSIONS.join(', ') });
  }
  if (file.size > MAX_UPLOAD_BYTES) {
    return i18n.t('evidence.fileTooLarge', { mb: MAX_UPLOAD_BYTES / (1024 * 1024) });
  }
  return null;
}
