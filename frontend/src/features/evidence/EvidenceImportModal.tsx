    ...
    <div style={OVERLAY} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={PANEL}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          ...
          <strong style={{ fontSize: 14 }}>Evidence importieren</strong>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, padding: '8px 10px', background: 'var(--bg-input)', border: '1px dashed var(--border)', borderRadius: 'var(--radius-sm)' }}>
          ...
          <Button size="sm" variant="ghost" icon={<FileUp size={14} />} onClick={() => fileRef.current?.click()}>Datei importieren</Button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Field label="Titel (Pflicht)">
            <Input value={draft.title} onChange={set('title')} placeholder="z.B. Verdächtiger PowerShell-Prozess" />
          </Field>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <Field label="Typ"><Select options={toOpts(EVIDENCE_TYPE_OPTIONS)} value={draft.type} onChange={set('type')} /></Field>
            <Field label="Quelle"><Select options={toOpts(EVIDENCE_SOURCE_OPTIONS)} value={draft.source} onChange={set('source')} /></Field>
          </div>
          <Field label="Kommentar / Analyst-Einschätzung">
            <Textarea value={draft.comment} onChange={set('comment')} style={{ minHeight: 70 }} placeholder="Warum ist das relevant?" />
          </Field>
          <Field label="Rohdaten / Raw Event (optional)">
            <Textarea value={draft.rawText} onChange={set('rawText')} className="input-mono" style={{ minHeight: 110 }} placeholder="Log-Zeile, JSON, Commandline …" />
          </Field>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16 }}>
          ...
          <Button variant="success" size="sm" icon={<Upload size={14} />} disabled={busy} onClick={() => void submit()}>
            {busy ? 'Speichern …' : 'Evidence anlegen'}
          </Button>
        </div>
      </div>
    </div>
