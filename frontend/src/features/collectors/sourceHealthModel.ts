...
function quietLabel(lastSeen: string | null, now: Date): string {
  if (!lastSeen) return i18n.t('text.neverSeen');
...
function buildRow(integration: IntegrationStatus, activity: SourceActivity | undefined, now: Date): SourceHealthRow {
  ...
  if (!integration.configured) {
    return {
      ...
      lastSeen,
      tone: 'muted',
      statusLabel: i18n.t('text.notConfigured'),
      detail: i18n.t('app.sourceNotSetUpYet'),
    };
  }
  ...
  if (recent > 0) {
    return {
      ...
      detail: i18n.t('collectors.ticketsLast24h', { count: recent }),
    };
  }
  ...
  if (lastSeen) {
    return {
      ...
      detail: i18n.t('collectors.totalNoneIn24h', { count: total }),
    };
  }
  ...
  return {
    ...
    lastSeen,
    tone: 'danger',
    statusLabel: i18n.t('text.neverSeen'),
    detail: i18n.t('app.configuredButNoTicketsHave'),
  };
}
