...
export function computeHostRisk(host: RegisteredHost): HostRisk {
  ...
  if (host.heartbeatStatus === 'offline') add(35, i18n.t('app.agentOfflineNoVisibility'));
  else if (host.heartbeatStatus === 'delayed') add(15, i18n.t('app.heartbeatDelayed'));

  if (host.inventoryStatus === 'missing') add(20, i18n.t('hosts.inventoryMissing'));
  else if (host.inventoryStatus === 'partial') add(10, i18n.t('app.inventoryIncomplete'));
  ...
  if (suspicious > 0) add(Math.min(suspicious * 10, 20), i18n.t('hosts.suspiciousPrograms', { count: suspicious }));
