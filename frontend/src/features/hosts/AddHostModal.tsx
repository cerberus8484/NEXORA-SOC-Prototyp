...
export function AddHostModal({ onClose, onEnrolled }: Props) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Host-Enrollment fehlgeschlagen. Ist die Wazuh-API konfiguriert?');
  ...
  async function handleManualSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Asset anlegen fehlgeschlagen.');
    } finally { setBusy(false); }
  }
  ...
  const title = mode === 'manual' ? 'Manuelles Asset anlegen' : 'Host registrieren (Wazuh-Agent)';
    ...
    >
      <div style={s.modal} role="dialog" aria-modal="true" aria-label={title}>
        <div style={s.head}>
          ...
          <button style={s.closeBtn} onClick={onClose} aria-label="Schließen"><X size={18} /></button>
        </div>
        ...
        {enrolled ? (
          <div>
            <div style={s.body}>
              <ol style={s.ol}>
                {enrollmentSteps(enrolled).map((step) => <li key={step}>{step}</li>)}
              </ol>
          ...
          <form onSubmit={handleSubmit}>
            <div style={s.body}>
              <p style={s.hint}>Registriert einen neuen Agent am Wazuh-Manager und erzeugt den Enrollment-Key. Den Agent installierst du danach auf dem Ziel-Host.</p>
              <div style={{ marginTop: 12 }}>
                <Field label="Agent-Name" hint="a–z A–Z 0–9 . _ - (kein Leerzeichen)">
                  <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="z. B. web01" autoFocus />
                </Field>
                <Field label="IP (optional)" hint="Leer lassen → Wazuh erlaubt jede Quell-IP.">
          ...
          <form onSubmit={handleManualSubmit}>
            <div style={s.body}>
              <p style={s.hint}>Ein Asset ohne Wazuh-Agent (Appliance, Netz-Hardware, Fremdsystem). Es erscheint in der Host-Liste als Quelle „Manual" — ohne Heartbeat/Inventory (bewusst „unmonitored").</p>
              <div style={{ marginTop: 12 }}>
                <Field label="Hostname" hint="a–z A–Z 0–9 . _ - (kein Leerzeichen)">
                  <Input value={manual.hostname} onChange={(e) => setManual({ ...manual, hostname: e.target.value })} placeholder="z. B. fw-edge" autoFocus />
                </Field>
                <Field label="IP-Adressen (optional)" hint="Mehrere per Komma trennen.">
