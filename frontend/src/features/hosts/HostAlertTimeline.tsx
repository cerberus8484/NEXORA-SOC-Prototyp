...
export function HostAlertTimeline({ agentId, hostname }: Props) {
  ...
  useEffect(() => {
    ...
    hostsApi.alerts(agentId, hostname)
      ...
      .catch((err) => { if (active) setError(err instanceof Error ? err.message : i18n.t('text.loadingFailed2')); });
    return () => { active = false; };
  }, [agentId, hostname]);
  ...
  return (
    <div style={s.wrap}>
      ...
      {!error && items === null && <div style={s.empty}>{tr('app.loadingAlerts')}</div>}
      {!error && items?.length === 0 && <div style={s.empty}>{tr('app.noAlertsHostSoonTickets')}</div>}
