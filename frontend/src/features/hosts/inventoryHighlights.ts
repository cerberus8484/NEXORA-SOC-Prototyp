...
const AV_NOT_AVAILABLE_REASON =
  'AV-Erkennung nicht verfügbar — syscollector liefert nur packagesTotal, keine Paketliste';
...
export function deriveHostRisk(
  ...
  if (isOffline) {
    add(35, `Agent ${status === 'never_connected' ? 'nie verbunden' : 'offline'} (keine Sicht)`);
  }
  ...
  if (keepAliveAge !== null && !isOffline) {
    if (keepAliveAge > HEARTBEAT_STALE_MIN) {
      add(15, `Letzter Heartbeat vor ${keepAliveAge} min (>15 min)`);
    }
  }
