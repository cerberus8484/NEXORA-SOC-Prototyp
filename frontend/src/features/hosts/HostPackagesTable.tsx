...
export function HostPackagesTable({ agentId }: Props) {
  ...
  useEffect(() => {
    ...
    hostsApi.packages(agentId, q)
      ...
      .catch((err) => {
        if (reqRef.current !== reqId) return;
        setState((prev) => applyError(prev, err instanceof Error ? err.message : 'Laden fehlgeschlagen'));
      });
  ...
  function loadMore() {
    ...
    setState((prev) => {
      ...
      hostsApi.packages(agentId, q)
        ...
        .catch((err) => {
          if (reqRef.current !== reqId) return;
          setState((cur) => applyError(cur, err instanceof Error ? err.message : 'Laden fehlgeschlagen'));
        });
      return prev;
    });
  }
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.searchBox}>
        ...
        <input
          style={s.input}
          placeholder="Software durchsuchen (Name) …"
          value={searchInput}
          onChange={(e) => setSearchInput(e.target.value)}
          aria-label="Installierte Software durchsuchen"
        />
      </div>

      {phase === 'disabled' && (
        <div style={s.hint}>
          Wazuh nicht verbunden — sobald die Wazuh-API konfiguriert ist, erscheint hier die
          installierte Software dieses Hosts.
        </div>
      )}
      ...
      {phase === 'loading' && items.length === 0 && (
        <div style={s.hint}>Software wird geladen …</div>
      )}

      {phase === 'loaded' && items.length === 0 && (
        <div style={s.hint}>
          {activeSearch
            ? `Keine Pakete für „${activeSearch}".`
            : 'Keine installierte Software für diesen Host gemeldet.'}
        </div>
      )}
      ...
      {phase === 'loading' && items.length > 0 && (
        <div style={s.hint}>Weitere Pakete werden geladen …</div>
      )}
    </div>
  );
}
