    ...
    <div style={s.wrap}>
      ...
      <div style={s.grid}>
        ...
        <EntityCard title="User" icon={<User size={14} style={{ color: ac }} />} rows={[
          [tr('common.users'), e.user?.username], [tr('app.domain'), e.user?.domain], ['SID', e.user?.sid], ['Privileg', e.user?.privilege],
        ]} />
        <EntityCard title="Process" icon={<Cpu size={14} style={{ color: ac }} />} rows={[
          ...
          ['SHA256', e.process?.hashSha256], [tr('settings.signed'), signedLabel(e.process?.signed)],
        ...
        <EntityCard title="File" icon={<FileText size={14} style={{ color: ac }} />} rows={[
          ['Name', e.file?.name], ['Pfad', e.file?.path], [tr('common.directory'), e.file?.directory],
          ['SHA256', e.file?.hashSha256], [tr('settings.signed'), signedLabel(e.file?.signed)], ['Erstellt', e.file?.createdAt],
        ]} />
        ...
        <EntityCard title="Network" icon={<Network size={14} style={{ color: ac }} />} rows={[
          ...
          ['Protokoll', e.network?.protocol], [tr('common.direction'), e.network?.direction], ['Prozess', e.network?.image],
        ]} />
      ...
      <BulletList label={tr('analysis.confirmedFacts')} icon={<CheckCircle2 size={13} />} items={analysis.confirmedFacts || []} color="var(--success)" />
      <BulletList label={tr('app.suspiciousIndicators')}
      ...
      {fp && (
        <div style={{ fontSize: 12.5, color: 'var(--text)' }}>
          <span style={s.secLabel as CSSProperties}>{tr('app.falsePositivePossibility')} </span>
