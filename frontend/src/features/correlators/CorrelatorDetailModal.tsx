...
export function CorrelatorDetailModal({ correlator, role, onClose }: Props) {
  ...
  function onActionError(e: unknown) { setActionError(e instanceof Error ? e.message : 'Aktion fehlgeschlagen'); }
          ...
          {!error && jobs && config && (
            <>
              ...
              <div style={s.grid}>
                ...
                <Field label="Status-Herkunft"><span style={s.hint}>aus Job-Daten abgeleitet (live)</span></Field>
              </div>
              ...
              {/* Config */}
              <div style={s.section}><ShieldOff size={13} /> Konfiguration — Apply nicht unterstützt</div>
  ...
  return (
    <div style={s.capCard}>
      ...
      <div style={{ ...s.banner, marginTop: 6, background: health.applyReady ? 'rgba(234,179,8,0.10)' : 'rgba(120,120,120,0.10)', border: `1px solid ${health.applyReady ? 'rgba(234,179,8,0.30)' : 'var(--border-soft)'}` }}>
        <Badge tone={applyReadinessTone(health)}>{health.applyReady ? 'Apply-Ready' : 'Apply blockiert'}</Badge>
...
function JobsTable({ jobs }: { jobs: JobSummary[] }) {
  if (jobs.length === 0) return <span style={s.hint}>Keine Jobs.</span>;
  return (
...
function ResultsTable({ results }: { results: ResultSummary[] }) {
  if (results.length === 0) return <span style={s.hint}>Keine Results.</span>;
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead><tr>{['Ticket', 'Events', 'Quellen', 'Evidence-Refs', 'Erstellt'].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr></thead>
        <tbody>
          {results.map((r) => (
            <tr key={r.id}>
...
function AuditTimeline({ audit }: { audit: AuditEntry[] }) {
  if (audit.length === 0) return <span style={s.hint}>Keine Audit-Einträge.</span>;
  return (
    <div>
      {audit.map((a) => (
  ...
  return (
    <>
      ...
      {config.reserved.length > 0 && (
        <>
          <div style={{ ...s.label, marginTop: 14 }}>Reserviert — sichtbar, nicht editierbar</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          ...
          {canEdit && (
            <Button variant="primary" size="sm" disabled={busy} onClick={() => void createDraft()}>
              {busy ? 'Anlegen …' : 'Draft anlegen & validieren'}
            </Button>
          )}
        </div>
      )}
    ...
    <div style={{ borderTop: '1px solid var(--border-soft)', paddingTop: 8, marginTop: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, flexWrap: 'wrap' }}>
        ...
        <span style={{ marginLeft: 'auto', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          ...
          {canEdit && (
            <Button variant="ghost" size="sm" disabled={busy} onClick={() => void doValidate()}>Validieren</Button>
          )}
          {/* Apply-Plan-Vorschau — read-only, KEIN Apply */}
          <Button variant="ghost" size="sm" disabled={busy} onClick={() => void doPlan()}>Apply-Plan anzeigen</Button>
          {canEdit && draft.status === 'draft' && (
            <Button variant="ghost" size="sm" disabled={busy}
              onClick={() => void run(() => correlatorsApi.submitDraft(correlatorId, draft.id, { expectedVersion: draft.version }))}>
      ...
      {plan && (
        <div style={{ ...s.capCard, marginTop: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            <strong style={{ fontSize: 12 }}>Apply-Plan (Vorschau)</strong>
          ...
          <div style={s.hint}>
            <div><u>Würde sich ändern</u>: {plan.changes.length === 0 ? '—' : plan.changes.map((c) => <span key={c.key} style={{ marginRight: 10 }}><code>{c.key}</code>: {String(c.before)} → {String(c.after)}</span>)}</div>
            <div><u>Bliebe unverändert</u>: {plan.unchanged.length === 0 ? '—' : plan.unchanged.map((u) => <span key={u.key} style={{ marginRight: 10 }}><code>{u.key}</code>: {String(u.value)}</span>)}</div>
          </div>
          <div style={{ ...s.banner, background: 'rgba(234,179,8,0.10)', border: '1px solid rgba(234,179,8,0.30)' }}>
