  ...
  return (
    <div ref={panelRef} style={{ position: 'relative' }}>
      ...
      >
        <Bell size={18} />
        {unreadCount > 0 && (
          <span className="bell-badge" aria-label={tr('notifications.unreadCount', { count: unreadCount })}>
      ...
      {/* Dropdown-Panel */}
      {open && (
        <div style={s.panel} role="dialog" aria-label={tr('nav.notifications')}>
          {/* Header */}
          <div style={s.header}>
            <span style={s.headerTitle}>{tr('nav.notifications')}</span>
          ...
          <div style={s.list}>
            {loading ? (
              <div style={s.emptyState}>{tr('text.loading')}</div>
            ) : items.length === 0 ? (
              <div style={s.emptyState}>
                <Inbox size={22} style={{ marginBottom: 6, opacity: 0.4 }} />
                <div>{tr('text.noNotifications')}</div>
              </div>
            ) : (
              items.map((n) => (
                <button
          ...
          <div style={s.footer}>
            ...
            >{tr('notifications.showAllArrow')}</button>
          </div>
        </div>
      )}
    </div>
