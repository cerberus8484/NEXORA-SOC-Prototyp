...
const RUN_STATUS_LABEL: Record<DeployRunStatus, string> = {
  ...
  approved:         'Genehmigt',
  applying:         i18n.t('text.applying'),
  cloning:          'Klont Template',
  starting:         'Startet VM',
  configuring:      i18n.t('common.configured'),
  verifying:        i18n.t('common.verified'),
  deployed:         'Deployt',
  rolling_back:     i18n.t('deploy.rollbackRunning'),
  rolled_back:      i18n.t('deploy.rolledBack'),
...
const STEP_LABEL: Record<string, string> = {
  ...
  attach_network:   i18n.t('deploy.attachNetwork'),
  start:            i18n.t('deploy.startVm'),
  config_import:    i18n.t('deploy.importConfiguration'),
  verify:           i18n.t('common.verify'),
  agent_install:    i18n.t('deploy.installAgent'),
  rollback_destroy: i18n.t('deploy.rollBackDestroy'),
};
...
export function applyDisabledReason(ctx: ApplyContext): string | null {
  if (!can.admin(ctx.role)) return i18n.t('deploy.onlyAdministratorsMayDeploy');
  if (ctx.runStatus !== 'approved') return i18n.t('deploy.runMustApprovedFirstFour');
  if (!ctx.deployEnabled) return i18n.t('deploy.deploymentBlockedServerDeployEnabled');
  return null;
}
...
export function summarizeParams(params: Record<string, unknown>): string {
  ...
  if (p.targetHost) parts.push(i18n.t('deploy.targetX', { host: String(p.targetHost) }));
...
export function preconditionsSummary(pre: Preconditions | null | undefined): string {
  if (!pre) return i18n.t('deploy.noPreconditionsChecked');
  // agent-install: reiner Dry-Run ohne Ziel-Kontakt — zeigt die geplante Aktion.
  if (pre.kind === 'agent-install') {
    return i18n.t('deploy.wouldInstallAgent', { host: pre.targetHost ?? '?', manager: pre.wazuhManager ?? '?' });
  }
  if (pre.ok) return i18n.t('deploy.allPreconditionsMetTemplateBridge');
  return i18n.t('deploy.notSatisfied', { issues: (pre.issues || []).join('; ') || i18n.t('common.unknown') });
}
