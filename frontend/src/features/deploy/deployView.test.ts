...
import {
  ...
  stepLabel, stepStatusTone, buildAgentInstallSpecBody, buildWindowsServerSpecBody, buildLxcSpecBody,
} from './deployView';

describe('buildLxcSpecBody', () => {
  test('übernimmt Platzierung und Netz, aber keine Zugangsdaten', () => {
    const spec = buildLxcSpecBody('rocky-linux-container', { id: 'pve', targetNode: 'pve', storage: 'local-lvm', bridge: 'vmbr10' }, { cpu: 2, ramMB: 2048, diskGB: 20 }, {
      hostname: 'rocky-web-01', ipMode: 'static', staticIp: '10.0.10.30', cidr: 24,
      gateway: '10.0.10.1', dns: ['10.0.10.10'], lxcTemplate: 'local:vztmpl/rockylinux-9-default.tar.xz',
    });
    expect(spec).toMatchObject({ connectorId: 'pve', targetNode: 'pve', bridge: 'vmbr10', params: { lxcTemplate: 'local:vztmpl/rockylinux-9-default.tar.xz', hostname: 'rocky-web-01' } });
    expect(JSON.stringify(spec)).not.toMatch(/password|token|privateKey/i);
  });
});
