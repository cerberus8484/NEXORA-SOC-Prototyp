...
export function nodeActionState(node: InstalledNode, keypair: DeployKeypairInfo): NodeActionState {
  ...
  if (!updatable) updateBlockedReason = i18n.t('deploy.updatesOnlyAvailableWindowsLinux');
  else if (!keypair.isSet) updateBlockedReason = i18n.t('text.noDeployKeyPairGenerate');
  else if (!hasIp) updateBlockedReason = i18n.t('deploy.nodeHasNoIpRegistry');
  else if (!hostKeyPinned) updateBlockedReason = i18n.t('deploy.hostKeyNotRecordedRecord');
