  ...
  return (
    <div style={{ ...s.card, borderLeft: `3px solid ${zone.color}` }}>
      <div style={s.head}>
        ...
        {zone.isolated ? <span style={s.isolated}>{tr('hosts.isolatedWarn')}</span> : null}
      ...
      {points.length > 0 ? (
        ...
        <div style={s.noData}>{tr('app.noAgentTelemetryZone')}</div>
      )}
    </div>
  );
}
...
export function NetworkZonesPanel({ telemetry, agents }: NetworkZonesPanelProps) {
  ...
  return (
    <div style={{ marginTop: 14 }}>
      <OpsSectionHead tag="[02]" title={tr('siem.networkZonesHosts')} right="Events pro VLAN · live" />
      <div style={s.grid}>
        {zones.map((z) => (
