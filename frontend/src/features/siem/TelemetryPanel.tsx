...
export function TelemetryPanel() {
  ...
  if (enabled === null) {
    return (
      <Card className="card-pad" style={{ marginBottom: 24 }}>
        <Spinner label={tr('app.loadingTelemetry')} />
      </Card>
    );
  }
  if (!enabled || !telemetry) {
    return (
      <Card style={{ marginBottom: 24 }}>
        ...
        <CardBody>
          <EmptyState
            title={tr('app.siemTelemetryNotAvailable')}
            message={tr('app.wazuhIndexerNotConfiguredUnreachable')}
          />
        </CardBody>
      </Card>
    );
  ...
  const charts: { title: string; color: string; sub: string; live: string; unit: string; series: ChartSeries[] }[] = [
    ...
    {
      title: 'Virus Detection — Defender', color: 'var(--accent-orange)', sub: tr('text.events11161117AllAgents'),
    ...
    <div style={s.term}>
      ...
      <div style={s.termBody}>
        {lines.length + defenderLines.length === 0
          ? <div style={{ color: 'var(--text-dim)' }}>{tr('app.noAlertsTimeWindow')}</div>
          : <>{defenderLines}{lines}</>}
      </div>
    </div>
  );
