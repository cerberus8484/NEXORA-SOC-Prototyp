...
export function deriveBuffer(list: SiemTelemetry['bufferAgents'] | null): BufferKpi {
  if (!list) return { value: '—', tone: 'muted', hint: 'SIEM nicht erreichbar', dot: false };

  const affected = list.filter((b) => b.warn > 0 || b.full > 0);
  if (affected.length === 0) return { value: 0, tone: 'success', hint: 'Alle Agents OK', dot: true };
...
export function resolveBufferTile(outcome: BufferFetchOutcome): BufferKpi {
  switch (outcome.kind) {
    case 'error':
      return { value: '!', tone: 'warning', hint: 'Telemetrie nicht ladbar', dot: false };
    case 'disabled':
      return { value: '—', tone: 'muted', hint: 'Telemetrie nicht aktiviert', dot: false };
    case 'ok':
