  ...
  return (
    <Field label={label}>
      ...
      <span style={{ ...s.msg, color: invalid ? 'var(--danger)' : 'var(--text-dim)' }} role={invalid ? 'alert' : undefined}>
        {invalid ? 'Ungültiger Hex-Wert (#rrggbb)' : 'Format: #rrggbb'}
      </span>
    </Field>
  );
}
...
export function BrandingPanel() {
  ...
  if (loading) {
    return <Card><CardBody style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Spinner /><span>Branding wird geladen…</span></CardBody></Card>;
  }
      ...
      <CardBody>
        <div style={s.grid}>
          ...
          <Field label="Schriftart">
            ...
            <span style={s.note}>Systemsichere Stacks (keine externen Fonts).</span>
          </Field>
        ...
        {!isAdmin && <p style={s.note}>Nur Administratoren können Branding-Einstellungen ändern.</p>}
        {saveError && <p style={{ color: 'var(--danger)', fontSize: 13, marginTop: '0.5rem' }} role="alert">{saveError}</p>}
        {saveSuccess && <p style={{ color: 'var(--success)', fontSize: 13, marginTop: '0.5rem' }} role="status">Branding-Einstellungen gespeichert.</p>}
