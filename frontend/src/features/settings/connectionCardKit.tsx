...
export function TestLine({ result }: { result: ConnTestResult }) {
  const status = result.ok
    ...
    : (() => {
        ...
        const label = notConfigured ? 'Nicht konfiguriert' : `Fehlgeschlagen: ${result.error ?? result.reason ?? 'unbekannt'}`;
...
export function useConnectionCard<M extends MaskedBase, F, P>(cfg: ConnectionCardConfig<M, F, P>): ConnectionCardController<M, F> {
  ...
  async function doSave(stepUpPassword: string) {
    ...
    try {
      ...
      const status = err instanceof ApiError ? err.status : 0;
      if (status === 403) setDialogError('Passwort ungültig.');
      ...
      <CardBody>
        {c.loadError ? (
          <div>
            ...
            <Button size="sm" variant="ghost" style={{ marginTop: 10 }} onClick={c.reload}>Erneut versuchen</Button>
          </div>
        ) : c.masked === null ? (
          <Spinner />
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={cardStyles.testRow}>
              <Button size="sm" variant="ghost" icon={<PlugZap size={13} />} disabled={c.testBusy} onClick={c.runTest}>
                {c.testBusy ? 'Teste…' : 'Verbindung testen'}
              </Button>
              {c.test && <TestLine result={c.test} />}
            </div>

            <div style={cardStyles.footer}>
              <Button variant="primary" icon={<Save size={14} />} disabled={!c.canSave || c.saving} onClick={c.openSave}>
                Speichern
              </Button>
              {!c.canSave && <span style={{ fontSize: 11.5, color: 'var(--text-dim)' }}>Keine Änderungen.</span>}
              {c.feedback && (
