...
export function WebhookSecretsCard() {
  ...
  useEffect(() => {
    ...
    fetchWebhookSecrets()
      ...
      .catch(() => { if (alive) setLoadError(i18n.t('settings.webhookSecretsCouldNotLoaded')); });
    return () => { alive = false; };
  }, [reloadKey]);
  ...
  async function save(source: string, secret: string) {
    ...
    try {
      ...
      setFeedback({
        tone: 'success',
        text: secret === ''
          ? tr('settings.secretDeletedFallback', { source: webhookSourceLabel(source) })
          : tr('settings.secretSaved', { source: webhookSourceLabel(source) }),
      });
    } catch (err) {
      const status = err instanceof ApiError ? err.status : 0;
      setFeedback({ tone: 'error', text: status === 400 ? tr('settings.invalidSource') : tr('common.savingFailed') });
    } finally {
      setBusy('');
    }
  }
      ...
      <CardBody>
        {loadError ? (
          <div>
            ...
            <Button size="sm" variant="ghost" style={{ marginTop: 10 }} onClick={() => setReloadKey((k) => k + 1)}>{tr('common.retry')}</Button>
          </div>
        ) : masked === null ? (
          <Spinner />
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <p style={cardStyles.hint}>{tr('settings.webhookSecretsIntro')}<code>generic</code>{tr('settings.webhookSecretsSuffix')}</p>
                ...
                <label style={{ flex: 1, minWidth: 160, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <span style={{ fontSize: 11, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: 0.4 }}>
                    <HelpLabel text="HMAC-Secret" topic="integrationen" hint={(
                      <ExampleHint
                        title={tr('settings.sharedSigningSecretSourceNexora')}
                        text={tr('settings.secretMustExactlySameBoth')}
                        exampleLabel="Merksatz"
                        rows={[
                          { label: 'Sender', value: tr('settings.signsWebhook') },
                          { label: 'Nexora', value: 'prueft dieselbe Signatur' },
                        ]}
                        footer={tr('settings.afterRotatingExternalSystemMust')}
                      />
                    )} />
                  </span>
                  <Input
                    ...
                    placeholder={m.set ? tr('settings.secretSetRotate') : 'Secret setzen'}
                    onChange={(e) => setDraft(m.source, e.target.value)}
                    style={{ flex: 1, minWidth: 160 }}
                  />
                </label>
                <Button size="sm" variant="ghost" icon={<RefreshCw size={12} />} onClick={() => setDraft(m.source, generateWebhookSecret())}>{tr('common.generate')}</Button>
                <Button size="sm" variant="primary" icon={<Save size={13} />}
                  disabled={busy === m.source || (drafts[m.source] ?? '') === ''}
                  onClick={() => save(m.source, drafts[m.source] ?? '')}>{tr('common.save')}</Button>
                {m.origin === 'db' && (
                  <Button size="sm" variant="ghost" icon={<Trash2 size={12} />} disabled={busy === m.source} onClick={() => save(m.source, '')}>{tr('common.delete2')}</Button>
                )}
              </div>
            ))}
