      ...
      <CardBody>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, opacity: 0.65 }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--text-dim)', fontStyle: 'italic' }}>
            Backend-Subsystem vorhanden — nur Flag-gesteuert deaktiviert.
          </div>
...
function ActiveCard() {
  ...
  function loadTokens() {
    ...
    listTokens()
      .then(setTokens)
      .catch(() => setError('Tokens konnten nicht geladen werden.'))
  ...
  async function handleRevoke(id: string) {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Token konnte nicht widerrufen werden.');
    } finally {
      setRevoking(null);
    }
  }
    ...
    <Card>
      ...
      <CardBody>
        {loading ? (
          ...
          <EmptyState title="Fehler" message={error} />
        ) : tokens.length === 0 ? (
          <EmptyState
            title="Keine Tokens"
            message="Erstellen Sie ein Token für externe Skripte und API-Integrationen."
          />
        ) : (
        ...
        <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 12, lineHeight: 1.6 }}>
          Token-Werte werden nur einmalig bei der Erstellung angezeigt.
          Der vollständige Wert ist danach nicht mehr rekonstruierbar.
        </div>
      </CardBody>
...
function CreateTokenModal({ onClose, onCreated, createdToken }: CreateModalProps) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setError('Token konnte nicht erstellt werden.');
    } finally {
      setCreating(false);
    }
  }
      ...
      <div style={s.modal}>
        <div style={s.modalHeader}>
          <span style={{ fontWeight: 600, fontSize: 14 }}>
            {createdToken ? 'Token erstellt' : 'Neues Token erstellen'}
          </span>
        ...
        {createdToken ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ fontSize: 12, color: 'var(--text-dim)', lineHeight: 1.6 }}>
              Kopieren Sie den Token jetzt. Er wird nach dem Schließen nicht mehr angezeigt.
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              ...
              <input
                ref={inputRef}
                style={s.input}
                placeholder="Name des Tokens (z.B. CI-Skript)"
                value={name}
