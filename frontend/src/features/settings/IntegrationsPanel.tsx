...
export function IntegrationsPanel() {
  ...
  useEffect(() => {
    ...
    getStatus()
      ...
      .catch(() => { if (alive) { setError('Integrationen konnten nicht geladen werden.'); setLoading(false); } });
    return () => { alive = false; };
  }, []);
  ...
  const donutSegments: DonutSegment[] = [
    { label: 'Verbunden',          value: counts.connected,      color: 'var(--success)' },
    { label: 'Konfiguriert',       value: counts.configured,     color: 'var(--accent)' },
    { label: 'Nicht konfiguriert', value: counts.not_configured, color: 'var(--border-soft)' },
  ];
  ...
  const handleTest = async (id: string) => {
    ...
    try {
      ...
      setTestResults((r) => ({
        ...r,
        [id]: { reachable: false, testedAt: new Date().toISOString(), message: 'Anfrage fehlgeschlagen' },
      }));
    } finally {
  ...
  if (error) {
    return (
      <Card>
        <CardBody>
          <EmptyState icon={<AlertCircle size={28} />} title="Fehler" message={error} />
        </CardBody>
      </Card>
    );
  }
      ...
      <Card>
        <CardHeader
          ...
          actions={
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <span
                title="Diese Seite zeigt den effektiven Status. Pflege erfolgt in der Integrations-Konfiguration; vorhandene Systemwerte werden nur noch als Herkunft angezeigt."
              ...
              <Button
                ...
                size="sm"
                disabled
                title="Hier ist nur die Uebersicht. Konfiguration erfolgt auf der Seite Integrations-Konfiguration."
              >
        ...
        <CardBody>
          ...
          {paged.items.length === 0 ? (
            <EmptyState title="Keine Treffer" message="Filter oder Suche anpassen." />
          ) : (
...
function OverviewDonut({ counts, segments }: OverviewDonutProps) {
  return (
    <Card>
      <CardHeader title="Integrations-Übersicht" />
      <CardBody>
...
function HealthSummary({ counts, items }: HealthSummaryProps) {
  ...
  return (
    <Card>
      <CardHeader title="Integrations-Gesundheit" />
      <CardBody>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
          <HealthTile icon={<CheckCircle size={18} />} color="var(--success)"  value={counts.connected}      label="Verbunden" />
          <HealthTile icon={<Activity size={18} />}    color="var(--accent)"   value={counts.configured}     label="Konfiguriert" />
        ...
        <div style={s.meta}>
          ...
          <span style={{
            ...
            {testResult.reason === 'model_missing' || testResult.modelAvailable === false
              ? 'Modell fehlt'
              : testResult.reachable ? 'Erreichbar' : 'Nicht erreichbar'}
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
        {item.testable ? (
          ...
          <Button
            ...
            size="sm"
            disabled
            title="Kein Live-Test verfügbar — diese Karte zeigt nur den Status der gespeicherten Verbindung."
        ...
        <Button
          ...
          size="sm"
          disabled
          title="Konfiguration nicht hier — nutze die Seite Integrations-Konfiguration."
        >
          Konfigurieren
        </Button>
      </div>
...
function StatusFilterBar({ value, onChange, counts }: StatusFilterBarProps) {
  const options: { label: string; value: FilterStatus; count: number }[] = [
    { label: 'Alle',               value: 'all',            count: counts.total },
    { label: 'Verbunden',          value: 'connected',      count: counts.connected },
    { label: 'Konfiguriert',       value: 'configured',     count: counts.configured },
    { label: 'Nicht konfiguriert', value: 'not_configured', count: counts.not_configured },
  ];
