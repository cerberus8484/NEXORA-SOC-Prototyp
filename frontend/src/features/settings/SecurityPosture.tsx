...
export function SecurityPostureCards({ posture }: SecurityPostureProps) {
  ...
  return (
    <div style={s.grid}>
      {/* Passwort-Richtlinie */}
      <div style={s.card}>
        <div style={s.label}>{tr('settings.passwordStrength')}</div>
        <div style={s.value}>
        ...
        <div style={s.sub}>{tr('settings.serverEnforced')}</div>
      </div>
      ...
      {/* MFA — bewusst 0 % */}
      <div style={s.card}>
        <div style={s.label}>{tr('metrics.mfaCoverage')}</div>
        <div style={{ ...s.value, color: 'var(--text-dim)' }}>0 %</div>
        <div style={s.sub}>{tr('text.notImplementedPlanned')}</div>
      </div>
      ...
      <div style={s.card}>
        ...
        <div style={{ ...s.value, color: 'var(--text-dim)' }}>
          <Badge tone="muted">{tr('text.notConfigured')}</Badge>
        </div>
        <div style={s.sub}>{tr('settings.plannedNotAvailableYet')}</div>
      </div>
    </div>
  );
}
