...
const providerMeta = (): Meta[] => [
  ...
  { kind: 'stub',      label: 'Stub',      tagline: i18n.t('settings.testNotRealModel'),     cloud: false },
];
...
const fallbackOptions = () => [{ value: '', label: i18n.t('settings.noFallback') },
  ...providerMeta().filter((p) => p.kind !== 'stub').map((p) => ({ value: p.kind, label: p.label }))];
...
export function KiSettingsCard({ isAdmin }: { isAdmin: boolean }) {
  ...
  const reloadProviders = useCallback(() => {
    fetchKiProviders().then((p) => setProviders(p.providers))
      .catch(() => setErrMsg(i18n.t('settings.providerListCouldNotRefreshed')));
  }, []);
  ...
  useEffect(() => {
    ...
    Promise.all([fetchKiConfig(), fetchKiModels(), fetchKiProviders()])
      ...
      .catch(() => { if (alive) setErrMsg(i18n.t('text.loadingFailed')); })
  ...
  const reloadModels = useCallback(() => {
    fetchKiModels()
      ...
      .catch(() => {
        ...
        setOllamaMessage(i18n.t('settings.ollamaUnreachable'));
      });
  }, []);
  ...
  const runTest = async (kind: string) => {
    ...
    try {
      ...
      setTests((t) => ({ ...t, [kind]: { ok: false, error: e instanceof Error ? e.message : tr('common.error') } }));
    }
  };
  ...
  if (loading) return <Card><CardHeader title={tr('settings.aiSettings')} /><CardBody><Spinner /></CardBody></Card>;
  ...
  const modelOptions = models.length > 0 ? models.map((m) => ({ value: m, label: m })) : [{ value: config.ollamaModel, label: config.ollamaModel || tr('text.noModel') }];
  ...
  const cardBadge = (m: Meta) => {
    ...
    if (m.kind === 'ollama') {
      ...
      if (state === 'not_configured') return <Badge tone="muted" dot>{tr('text.notConfigured')}</Badge>;
      if (state === 'offline') return <Badge tone="warning" dot>Offline</Badge>;
      if (state === 'model_missing') return <Badge tone="warning" dot>{tr('settings.modelMissing')}</Badge>;
      if (state === 'configured') return <Badge tone="accent" dot>{tr('common.configured')}</Badge>;
      return <Badge tone="success" dot>{tr('settings.connectedLabel')}</Badge>;
    }
    if (m.cloud) return s?.configured ? <Badge tone="success" dot>Key gesetzt</Badge> : <Badge tone="warning" dot>{tr('text.noKey')}</Badge>;
  ...
  return (
    <>
    <Card>
      <CardHeader title={tr('settings.kiStep1')} actions={<span style={s.hint}><Plug size={12} /> Primär: {provider}</span>} />
      <CardBody>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {cloud && (
            <div style={s.warn}>
              ...
              <span><strong>Cloud-Provider:</strong>{tr('settings.cloudProviderWarning')}</span>
            </div>
          )}

          {/* 2. Provider-Konfiguration */}
          <div style={s.section}>{tr('settings.kiStep2')}</div>
          {provider === 'ollama' && (
            <>
              <Field label="Ollama Base-URL">
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  {isAdmin && <Button variant="ghost" onClick={reloadModels} title={tr('text.reloadModels')}><RefreshCw size={13} /></Button>}
                </div>
              </Field>
              {ollamaMessage && (
                <div style={{
          ...
          {cloud && (
            <Field label={`API-Key (${statusOf(provider)?.keyEnvVar || ''})`}>
              <Input type="password" disabled={!isAdmin}
                ...
                placeholder={statusOf(provider)?.configured ? tr('settings.setLeaveEmptyKeep') : 'API-Key eingeben'}
          ...
          {provider !== 'stub' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              {isAdmin && <Button variant="ghost" size="sm" onClick={() => runTest(provider)} disabled={tests[provider] === 'testing'}>
                {tests[provider] === 'testing' ? 'Teste …' : tr('settings.testConnection')}
              </Button>}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, borderTop: '1px solid var(--border-soft)', paddingTop: 12 }}>
            <div>
              <div style={{ fontSize: 13, color: 'var(--text)' }}>{tr('settings.ragEnabled')}</div>
              <div style={{ fontSize: 10, color: 'var(--text-dim)' }}>{tr('settings.knowledgeRetrievalFromQdrantAi')}</div>
            </div>
            <Toggle checked={config.ragEnabled} label={tr('settings.ragEnabled')}
          ...
          {isAdmin && (
            <Button variant={saveState === 'saved' ? 'success' : 'primary'} onClick={handleSave} disabled={saveState === 'saving'}>
              {saveState === 'saving' ? tr('text.saving') : saveState === 'saved' ? tr('common.savedCheck') : tr('settings.saveAiSettings')}
            </Button>
          )}
        </div>
      </CardBody>
