...
export function imapConnError(form: ImapForm, masked: ImapMasked): string {
  const host = form.host.trim();
  if (host === '') return '';
  if (!isValidHost(host)) return 'Host muss ein gültiger Hostname oder eine IP sein.';
  const portStr = form.port.trim();
  if (portStr !== '') {
    const port = Number(portStr);
    if (!Number.isInteger(port) || port < 1 || port > 65535) return 'Port muss zwischen 1 und 65535 liegen.';
  }
  ...
  if (form.imapPassword === '' && !hasStoredPassword) return 'Passwort erforderlich (wird verschlüsselt gespeichert).';
  return '';
}
