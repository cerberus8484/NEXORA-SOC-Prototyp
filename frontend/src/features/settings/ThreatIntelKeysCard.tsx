...
function TestLine({ result }: { result: TiTestResult }) {
  ...
  const label = result.reason === 'not_configured'
    ? 'Kein Key gesetzt'
    : `Fehlgeschlagen: ${result.error ?? result.reason ?? 'unbekannt'}`;
  return (
...
export function ThreatIntelKeysCard() {
  ...
  useEffect(() => {
    ...
    fetchTiKeys()
      ...
      .catch(() => { if (alive) setLoadError('Threat-Intel-Keys konnten nicht geladen werden.'); });
    return () => { alive = false; };
  }, [reloadKey]);
  ...
  async function runTest(provider: TiProvider) {
    ...
    try {
      ...
      if (!isStale()) setTests((prev) => ({ ...prev, [provider]: { ok: false, error: 'Test fehlgeschlagen.' } }));
    } finally {
  ...
  async function doSave() {
    ...
    try {
      ...
      setTests({});
      setFeedback({ tone: 'success', text: 'Threat-Intel-Keys gespeichert — sofort aktiv, ohne Neustart. Die Änderung wurde protokolliert.' });
    } catch {
      ...
      <CardBody>
        {loadError ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <p style={{ fontSize: 12.5, color: 'var(--text-dim)', margin: 0, lineHeight: 1.6, maxWidth: 760 }}>
              API-Keys für VirusTotal und AbuseIPDB — verschlüsselt in der Datenbank gespeichert und sofort
              wirksam. Falls noch ein vorhandener Systemwert existiert, wird dieser nur genutzt,
              solange hier kein UI-Key hinterlegt ist. Ein leeres Feld lässt den gespeicherten Key unverändert.
            </p>
                ...
                return (
                  <div key={meta.provider} style={s.provider}>
                    ...
                    <Field label={<HelpLabel text="API-Key" topic="integrationen" hint={(
                      <ExampleHint
                        title={`${meta.label}-Schluessel fuer Reputation-Lookups`}
                        text="Hier traegst du den echten Provider-Key ein, den Nexora fuer TI-Abfragen nutzen soll."
                        exampleLabel="Praxis"
                        rows={[
                          { label: 'Provider', value: meta.label },
                          { label: 'Aenderung', value: 'eintragen, leer = behalten' },
                        ]}
                        footer="Das Feld dient nur zum Setzen oder Rotieren. Bereits gespeicherte Keys werden nie wieder im Klartext angezeigt."
                      />
                    )} />}>
                      <Input
                        type="password"
                        ...
                        placeholder={st?.keySet ? '••••••••  (gesetzt — leer lassen zum Behalten)' : 'API-Key eingeben'}
                    ...
                    <div style={s.testRow}>
                      ...
                      >
                        {testBusy[meta.provider] === true ? 'Teste…' : 'Verbindung testen'}
                      </Button>
            ...
            <div style={s.footer}>
              ...
              {!hasTiChanges(form) && <span style={{ fontSize: 11.5, color: 'var(--text-dim)' }}>Keine Änderungen.</span>}
              {feedback && (
                <div
                  role="status"
                  style={{
