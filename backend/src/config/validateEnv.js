...
function validateEnv() {
  ...
  // ── TLS-Härtung Wazuh ────────────────────────────────────
  // Sobald Wazuh-API oder Indexer in Produktion angebunden sind, darf TLS-
  // Zertifikatprüfung nicht still deaktiviert bleiben. Ohne Wazuh-Ziel bleibt es
  // eine Warnung, damit reine Lab-/Demo-Installationen nicht unnötig brechen.
  const wazuhConfigured = Boolean(process.env.WAZUH_API_URL || process.env.WAZUH_INDEXER_URL);
  if (wazuhConfigured && process.env.WAZUH_TLS_REJECT_UNAUTHORIZED !== 'true') {
    errors.push(
      'Wazuh-API/Indexer ist konfiguriert, aber WAZUH_TLS_REJECT_UNAUTHORIZED ist nicht "true" — ' +
      'TLS-Zertifikat-Validierung in Produktion aktivieren oder Wazuh-URLs entfernen.',
    );
  } else if (process.env.WAZUH_TLS_REJECT_UNAUTHORIZED !== 'true') {
    console.warn(
  ...
  const qradarConfigured = Boolean(process.env.QRADAR_BASE_URL || process.env.QRADAR_HOST);
  if (qradarConfigured && process.env.QRADAR_TLS_REJECT_UNAUTHORIZED === 'false') {
    errors.push(
      'QRadar ist konfiguriert, aber QRADAR_TLS_REJECT_UNAUTHORIZED ist "false" — ' +
      'TLS-Zertifikat-Validierung aktivieren oder QRadar-Ziele entfernen.',
    );
  }
