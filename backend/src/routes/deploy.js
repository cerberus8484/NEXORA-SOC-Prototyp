...
const { getDeployService, getDeployPreflight } = require('../deploy/deployServiceFactory');
...
// Read-only Kapazität/Template-Inventar für die Zielauswahl. Der Connector-Token
// bleibt ausschließlich im Adapter und wird niemals an den Client zurückgegeben.
router.get('/connectors/:id/capacity', requireAuth, requireRole('admin'), async (req, res, next) => {
  try { res.json({ data: await svc().getConnectorCapacity(req.params.id), requestId: req.id }); } catch (err) { next(err); }
});
router.get('/connectors/:id/certificate', requireAuth, requireRole('admin'), async (req, res, next) => {
  try { res.json({ data: await svc().inspectConnectorCertificate(req.params.id), requestId: req.id }); } catch (err) { next(err); }
});
router.post('/connectors/:id/certificate/trust', requireAuth, requireRole('admin'), async (req, res, next) => {
  try { res.json({ data: await svc().trustConnectorCertificate(req.params.id, String(req.body && req.body.fingerprint || ''), actorFrom(req), req.get('X-Reauth-Token')), requestId: req.id }); } catch (err) { next(err); }
});
...
router.get('/runs', requireAuth, requireRole('admin'), async (req, res, next) => {
  try { res.json({ data: await svc().listRuns(), requestId: req.id }); } catch (err) { next(err); }
});
...
// ── Zwei-Schlüssel-Systemstatus + Betriebs-Toggle ────────────────────────────
// GET /deploy/preflight — armed/inert + Boot-Bedingungen (grün/rot). Read-only,
// admin-only; macht das SSH-in-die-Box-Raten überflüssig (Administrierbarkeit).
router.get('/preflight', requireAuth, requireRole('admin'), async (req, res, next) => {
  try { res.json({ data: await getDeployPreflight(), requestId: req.id }); } catch (err) { next(err); }
});

// POST /deploy/arm — betrieblich scharfschalten (admin + frische Reauth + Audit).
// Der env-Boden (Kommissionierung) bleibt Voraussetzung: ist er aus oder fehlt eine
// Boot-Bedingung, ist Armen NICHT möglich (409). Nicht über den generischen
// Settings-PUT, damit die Step-up-Prüfung nicht umgangen werden kann.
router.post('/arm', requireAuth, requireRole('admin'), async (req, res, next) => {
  try {
    const actor = actorFrom(req);
    const reauth = await authService.verifyDeployReauth(req.get('X-Reauth-Token'), actor.id);
    if (!reauth || !reauth.ok) {
    ...
    const pre = await getDeployPreflight();
    if (!pre.canArm) {
      return res.status(409).json({ error: 'E_NOT_ARMABLE', message: 'Armen nicht möglich — offene Voraussetzungen.', data: pre, requestId: req.id });
    }
    await auditService.write({
      actorUserId: actor.id ?? null, actorLabel: actor.label ?? 'unknown', ip: req.ip || '',
      action: 'DEPLOY_ARMED', targetType: 'deploy_gate', targetId: 'deploy_gate', metadata: {},
    });
    await deployArmStore.setArmed(true);
    res.json({ data: await getDeployPreflight(), requestId: req.id });
  } catch (err) { next(err); }
});

// POST /deploy/disarm — entwaffnen (admin + Reauth + Audit). Immer erlaubt: das ist
// die sichere Richtung (macht inert), unabhängig von den Boot-Bedingungen.
router.post('/disarm', requireAuth, requireRole('admin'), async (req, res, next) => {
  try {
    const actor = actorFrom(req);
    const reauth = await authService.verifyDeployReauth(req.get('X-Reauth-Token'), actor.id);
    if (!reauth || !reauth.ok) {
    ...
    await auditService.write({
      actorUserId: actor.id ?? null, actorLabel: actor.label ?? 'unknown', ip: req.ip || '',
      action: 'DEPLOY_DISARMED', targetType: 'deploy_gate', targetId: 'deploy_gate', metadata: {},
    }).catch(() => {});
    res.json({ data: await getDeployPreflight(), requestId: req.id });
  } catch (err) { next(err); }
});
