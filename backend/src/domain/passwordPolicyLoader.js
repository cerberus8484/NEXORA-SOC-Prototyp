...
// Plattform-Sprache — der Default fuer NEU angelegte Benutzer.
// Muss mit PLATFORM_DEFAULTS.language in routes/settings.js uebereinstimmen.
const DEFAULT_PLATFORM_LANGUAGE = 'de';

/**
 * Liest die eingestellte Plattform-Sprache.
 *
 * Sie wirkt ausschliesslich bei der ANLAGE eines Benutzers: users.language ist
 * NOT NULL, ein bestehender Benutzer hat also immer einen eigenen Wert, den
 * diese Einstellung nicht ueberschreiben darf.
 *
 * @returns {Promise<string>} 'de' oder 'en'
 */
async function loadPlatformLanguage() {
  try {
    const value = await _getRepo().get('platform_language');
    return value === 'en' || value === 'de' ? value : DEFAULT_PLATFORM_LANGUAGE;
  } catch {
    // Fail-safe: eine unlesbare Einstellung darf die Benutzeranlage nicht kippen.
