...
const HUNT_TYPES = {
  ...
  suspicious_powershell_hunt: {
    ...
    description: 'Finds obfuscated or encoded PowerShell execution (for example, -EncodedCommand) and unusual parent processes such as WINWORD.',
    category: 'Execution',
    mitre: 'T1059.001',
  ...
  opnsense_multicast_review: {
    ...
    description: 'Checks firewall and OPNsense events for local multicast noise (224.0.0.0/24) and marks false-positive candidates.',
  ...
  rdp_exposure_hunt: {
    ...
    description: 'Checks whether RDP (port 3389) is exposed or reachable on a host — a common initial-access vector.',
    category: 'Exposure',
    mitre: 'T1021.001',
  ...
  persistence_hunt: {
    ...
    description: 'Looks for persistence mechanisms: Run keys, scheduled tasks, services and autoruns.',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'An autorun or Run-key entry starts a binary from %APPDATA%, a typical persistence pattern.',
        severity: 'medium',
  ...
  failed_logon_hunt: {
    ...
    description: 'Detects brute-force and password-spraying patterns through repeated failed logons.',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'Repeated failed logons from a single source indicate possible brute-force or password-spraying activity.',
        severity: 'medium',
  ...
  dns_tunneling_hunt: {
    ...
    description: 'Looks for DNS exfiltration or tunnelling: unusually long or frequent queries to rare domains.',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'High-entropy subdomains for a rarely resolved domain indicate possible DNS tunnelling or exfiltration.',
        severity: 'medium',
  ...
  scheduled_tasks_hunt: {
    ...
    description: 'Looks for suspicious scheduled tasks (schtasks) used for persistence or execution.',
    category: 'Persistence',
    mitre: 'T1053.005',
  ...
  services_hunt: {
    label: 'Services Hunt',
    description: 'Checks Windows services for unusual binary paths or unsigned service binaries.',
    category: 'Persistence',
    mitre: 'T1543.003',
    dataSources: ['Windows Agent', 'Sysmon'],
    targetType: 'host',
  ...
  autoruns_hunt: {
    label: 'Autoruns Hunt',
    description: 'Analyses autostart persistence: Run and RunOnce registry keys, startup folders, and new entries with suspicious paths.',
    category: 'Persistence',
    mitre: 'T1547.001',
  ...
  remote_access_tools_hunt: {
    ...
    description: 'Looks for RMM and remote-access tools (AnyDesk, TeamViewer, ngrok, etc.) to distinguish legitimate use from attacker tooling.',
  ...
  lsass_access_hunt: {
    ...
    description: 'Looks for processes accessing lsass.exe (Sysmon Event 10), a classic credential-dumping indicator such as Mimikatz or comsvcs.dll.',
    category: 'Credential Access',
    mitre: 'T1003.001',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'A non-standard process opened lsass.exe with dump-typical GrantedAccess — a strong indicator of credential dumping (for example Mimikatz or comsvcs MiniDump).',
        severity: 'critical',
  ...
  lateral_movement_hunt: {
    ...
    description: 'Detects PsExec-style lateral movement through ADMIN$ service creation, the PSEXESVC service, and remotely executed commands.',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'The PSEXESVC service was created from a remote host through the ADMIN$ share, a typical PsExec lateral-movement pattern.',
        severity: 'high',
  ...
  bits_jobs_hunt: {
    ...
    description: 'Looks for abused BITS transfer jobs (bitsadmin or PowerShell Start-BitsTransfer) used for download, persistence, or exfiltration.',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'A BITS job downloads an executable from an external URL into a user directory, a LOLBin technique that can bypass application allowlists.',
        severity: 'medium',
        confidence: pctToConfidence(72),
        mitreAttack: 'T1197',
        recommendation: [
  ...
  wmi_persistence_hunt: {
    ...
    description: 'Looks for WMI event-subscription persistence through __EventFilter, CommandLineEventConsumer, and FilterToConsumerBinding (Sysmon 19/20/21).',
    category: 'Persistence',
    mitre: 'T1546.003',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'A WMI EventConsumer is linked to an EventFilter and executes code when triggered, a fileless persistence technique (T1546.003).',
        severity: 'high',
  ...
  token_theft_hunt: {
    ...
    description: 'Looks for access token theft and impersonation: processes running under a foreign (elevated) token — logon type 9 (NewCredentials), SeImpersonate/SeDebugPrivilege, DuplicateTokenEx.',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'A process is running under a foreign, probably elevated token — typical of token theft and impersonation for privilege escalation (for example after abusing a SeImpersonate service).',
        severity: 'high',
  ...
  asrep_roasting_hunt: {
    ...
    description: 'Detects AS-REP roasting: Kerberos AS-REQ without pre-authentication (Security 4768) for accounts with pre-auth disabled, often RC4 (0x17) — the attacker extracts AS-REP hashes that can be cracked offline.',
    category: 'Credential Access',
    mitre: 'T1558.004',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'A Kerberos AS-REQ without pre-authentication for an account with pre-auth disabled, RC4-encrypted — the returned AS-REP hash can be cracked offline.',
        severity: 'high',
  ...
  shadow_copy_deletion_hunt: {
    ...
    description: 'Detects deletion of volume shadow copies and recovery sabotage (vssadmin delete shadows, wmic shadowcopy delete, bcdedit recoveryenabled no, wbadmin delete) — a classic ransomware precursor.',
    category: 'Impact',
    mitre: 'T1490',
    dataSources: ['Sysmon', 'Windows Security Log', 'Wazuh'],
    targetType: 'host',
    ...
    build(session) {
      ...
      const findings = [{
        ...
        description: 'A command deleted volume shadow copies or disabled system recovery, preventing restoration and commonly preceding ransomware encryption.',
        severity: 'critical',
