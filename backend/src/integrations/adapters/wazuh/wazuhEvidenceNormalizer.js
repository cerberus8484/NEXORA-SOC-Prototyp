...
// Dauer des beobachteten Flow-Fensters (firstSeen→lastSeen) in ms; undefined ohne Fenster.
function flowDurationMs(firstSeen, lastSeen) {
...
function normalizeWazuhEvidence(ticket = {}) {
  ...
  return {
    ...
    network: {
      ...
      // Fallback auf das flache Ticket-Feld (Perimeter/Firewall-Quellen tragen block/pass dort).
      action: str(d.action) || (win.initiated === 'true' ? 'initiated (connection logged)' : undefined) || str(ticket.firewallAction),
      connectionState: str(d.state),
      // Roh-Alert bevorzugt; sonst die flachen Flow-Zähler des Tickets (Conntrack/Firewall).
      bytesSent: num(d.bytes_out || ticket.bytesSent),
      bytesReceived: num(d.bytes_in || ticket.bytesRecv),
      packetsSent: num(d.packets_out) ?? num(ticket.pktsSent),
      packetsReceived: num(d.packets_in) ?? num(ticket.pktsRecv),
      eventCount: num(rule.firedtimes) ?? num(ticket.eventCount),
      firstSeen: str(ticket.firstSeen),
      lastSeen: str(ticket.lastSeen),
      durationMs: flowDurationMs(ticket.firstSeen, ticket.lastSeen),
      firewallRule: str(d.rulenum || d.rule),
    },
    payload: {
      type: undefined,
