...
class InMemoryProxmoxConnector {
  constructor({ templates = [], lxcTemplates = [], bridges = [], usedVmids = [], nextVmid = 1000, failOn = {} } = {}) {
    this._templates = new Set(templates.map(String));
    this._lxcTemplates = new Set(lxcTemplates.map(String));
  ...
  async checkLxcPreconditions({ templateRef, bridge, vmid } = {}) {
    const templateExists = this._lxcTemplates.has(String(templateRef));
    const bridgeExists = this._bridges.has(bridge);
    const vmidFree = vmid == null || !this._usedVmids.has(Number(vmid));
    const issues = [];
    if (!templateExists) issues.push(`LXC-Template '${templateRef}' nicht gefunden`);
    if (!bridgeExists) issues.push(`Bridge '${bridge}' nicht gefunden`);
    if (!vmidFree) issues.push(`CTID ${vmid} bereits belegt`);
    return { ok: issues.length === 0, templateExists, bridgeExists, vmidFree, issues };
  }
  ...
  async createLxc(templateRef, spec = {}) {
    this._maybeFail('createLxc');
    if (!this._lxcTemplates.has(String(templateRef))) throw new Error(`LXC-Template '${templateRef}' nicht gefunden (create)`);
    const vmid = spec.vmid != null ? Number(spec.vmid) : this._nextVmid++;
    this._usedVmids.add(vmid);
    this._vms.set(vmid, { status: 'stopped', type: 'lxc', resources: { cpu: spec.cpu, ramMB: spec.ramMB, diskGB: spec.diskGB }, net: { bridge: spec.bridge, vlanTag: spec.vlanTag, ipMode: spec.ipMode }, name: spec.hostname || null });
    return { vmid };
  }

  async startLxc(vmid) { return this.start(vmid); }
  async lxcStatus(vmid) { return this.status(vmid); }
  async destroyLxc(vmid) { return this.destroy(vmid); }
