...
class DeployOrchestrator {
   ...
   */
  async execute({ run, spec, connector, templateRef, configApplier, gateContext, actor } = {}) {
    ...
    try {
      ...
      const pre = await connector.checkPreconditions({ templateRef, bridge: spec.bridge });
      if (!pre.ok) throw new Error(`Vorbedingungen nicht erfüllt: ${(pre.issues || []).join('; ')}`);

      // 3) cloning: clone → setResources → attachNetwork.
      run.toCloning();
      await this._repo.updateRun(run.toJSON());
      const { vmid } = await connector.cloneFromTemplate(templateRef, { name: p.hostname });
      run.setVmid(vmid);
      await this._repo.updateRun(run.toJSON());
      ...
      run.toVerifying();
      await this._repo.updateRun(run.toJSON());
      const st = await this._pollStatus(connector, vmid);
      if (!st.running) throw new Error('VM nach Start nicht erreichbar (Status-Poll timeout)');
      ...
      return this._rollback({ run, spec, connector, actor, reason: err.message });
    }
  }
  ...
  async _rollback({ run, spec, connector, actor, reason }) {
    ...
    try {
      if (run.vmid != null) await connector.destroy(run.vmid);
      run.toRolledBack();
      await this._repo.updateRun(run.toJSON());
