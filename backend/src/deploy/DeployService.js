...
class DeployService {
  ...
  async plan(specId, actor) {
    ...
    const preconditions = await connector.checkPreconditions({ templateRef, bridge: specRow.bridge });
    const run = DeployRun.plan({ specId, startedBy: actor && actor.label, startedById: actor && actor.id });
    await this._repo.createRun(run.toJSON());
  ...
  async apply(runId, actor, reauthToken) {
    ...
    const gateContext = {
      killSwitchEnabled: Boolean(this._isDeployEnabled()),
    ...
    try {
      ...
      const result = await this._orchestrator.execute({ run, spec: specDomain, connector, templateRef, configApplier, gateContext, actor });
...
function publicConnectorView(row) {
  ...
  const { apiTokenEnc, privateKeyEnc, passphraseEnc, ...rest } = row;
  return rest;
}
