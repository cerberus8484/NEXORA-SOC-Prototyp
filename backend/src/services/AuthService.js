...
const { loadPasswordPolicy, loadSessionMaxHours, loadLockoutPolicy, loadPasswordAgingPolicy, loadMaxConcurrentSessions, loadMfaRequired, loadPlatformLanguage } = require('../domain/passwordPolicyLoader');
...
class AuthService {
  ...
  async register({ email, displayName, password, role = 'analyst', mustChangePassword = false, language }) {
    ...
    // Sprache: ausdrückliche Angabe schlägt die Plattform-Vorgabe. Die Vorgabe
    // wirkt NUR hier — users.language ist NOT NULL, ein bestehender Benutzer hat
    // also immer einen eigenen Wert, den eine Einstellung nicht überschreiben darf.
    const initialLanguage = language || (await loadPlatformLanguage()) || 'de';
  ...
  /**
   * Administrativer Passwort-Reset OHNE Kenntnis des alten Passworts.
   *
   * Bewusst NICHT ueber eine HTTP-Route erreichbar: der einzige Aufrufer ist
   * scripts/resetAdminPassword.js, und der setzt Shell-Zugriff auf den Container
   * voraus. Wer den hat, koennte den Hash ohnehin direkt in die DB schreiben — es
   * entsteht also keine neue Angriffsflaeche, nur ein dokumentierter Weg statt
   * eines handgeschriebenen bcrypt-Einzeilers.
   *
   * Bestehendes Passwort wandert in die History (wie bei changePassword), die
   * Policy gilt unveraendert, und der Wechsel beim naechsten Login wird erzwungen —
   * das Ergebnis ist ein TEMPORAeRES Passwort, kein dauerhaftes.
   * Loggt NIEMALS das Passwort.
   */
  async resetPasswordByEmail({ email, newPassword, mustChangePassword = true }) {
    if (!email || !newPassword) throw new ValidationError('email und newPassword sind Pflichtfelder');

    const policy = await loadPasswordPolicy();
    const { ok, errors } = validatePassword(newPassword, policy);
    if (!ok) throw new ValidationError(errors.join('; '));

    // Kein User-Enumeration-Risiko: dieser Pfad ist CLI-only und der Aufrufer ist
    // bereits Operator. Ein stilles "ok" waere hier das eigentliche Problem — der
    // Operator wuerde ein Passwort notieren, mit dem sich niemand anmelden kann.
    const user = await this._findByEmail(email);
    if (!user) throw new ValidationError(`Kein Benutzer mit E-Mail ${email}`);
    if (!user.isActive) {
      throw new ValidationError(`Benutzer ${email} ist deaktiviert — ein Passwort-Reset wuerde daran nichts aendern`);
    }

    const { historyCount } = await loadPasswordAgingPolicy();
    // History fortschreiben, aber NICHT gegen sie pruefen: ein Recovery darf nicht
    // daran scheitern, dass der Zufallsgenerator etwas Aehnliches erzeugt hat.
    user.passwordHistory    = pushPasswordHistory(user.passwordHistory, user.passwordHash, historyCount);
    user.passwordHash       = await this.hashPassword(newPassword);
    user.passwordChangedAt  = new Date().toISOString();
    ...
    logger.info('admin_password_reset', { email: user.email });
