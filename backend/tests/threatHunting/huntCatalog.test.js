...
describe('Hunt-Katalog (vorgefertigte Hunts)', () => {
  ...
  it('liefert die sichtbaren Kern-Hunt-Beschreibungen auf Englisch', () => {
    const catalog = new Map(getCatalog().map((item) => [item.key, item]));
    expect(catalog.get('suspicious_powershell_hunt').description).toMatch(/^Finds obfuscated/);
    expect(catalog.get('opnsense_multicast_review').description).toMatch(/^Checks firewall/);
    expect(catalog.get('failed_logon_hunt').description).toMatch(/^Detects brute-force/);
  });

  it('liefert die nachgezogenen BITS- und WMI-Beschreibungen auf Englisch', () => {
    const translatedKeys = new Set(['bits_jobs_hunt', 'wmi_persistence_hunt']);
    for (const item of getCatalog().filter((item) => translatedKeys.has(item.key))) {
      expect(item.description).not.toMatch(/[äöüÄÖÜß]/);
    }
  });
...
describe('Neue Hunts erzeugen Logs + Findings', () => {
  ...
  it('BITS- und WMI-Hunts erzeugen englische Finding-Beschreibungen', async () => {
    const translatedKeys = new Set(['bits_jobs_hunt', 'wmi_persistence_hunt']);
    for (const item of getCatalog().filter((item) => translatedKeys.has(item.key))) {
      const { findings } = await run(item.key, item.defaultTarget || 'Host-X');
      expect(findings[0].description).toBeTruthy();
      expect(findings[0].description).not.toMatch(/[äöüÄÖÜß]/);
    }
  });
});
