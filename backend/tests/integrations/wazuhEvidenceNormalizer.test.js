...
describe('wazuh network — flache Flow-Felder als Fallback (Perimeter/Conntrack)', () => {
  test('Pakete/Firewall-Action/Events/Fenster/Dauer aus den flachen Ticketfeldern, wenn der Roh-Alert sie nicht trägt', () => {
    const ev = normalizeWazuhEvidence({
      id: 'w1', source: 'wazuh', title: 'FW-Block', priority: 'high',
      logs: '', // kein Roh-Alert → Fallback auf flache Felder
    ...
    expect(ev.network.durationMs).toBe(300000);
    expect(ev.firstSeen).toBe('2026-06-16T10:00:00Z');
    expect(ev.lastSeen).toBe('2026-06-16T10:05:00Z');
  });
});
