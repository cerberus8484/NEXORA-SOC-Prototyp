...
describe('PUT /api/v1/servicenow/connection', () => {
  const VALID = { password: ADMIN_PW, baseUrl: 'https://10.0.10.60', username: 'soc', servicenowPassword: 'sn-pass' };
  ...
  test('admin + korrektes Passwort → 200, Passwort verschlüsselt, Adapter rekonfiguriert, Audit ohne Passwort', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.data).toMatchObject({ baseUrl: 'https://10.0.10.60', username: 'soc', table: 'incident', passwordSet: true, source: 'db' });

    const stored = await createSettingsRepository().get(SERVICENOW_CONNECTION_KEY);
    expect(isEncrypted(stored.password)).toBe(true);
    expect(spy).toHaveBeenCalledWith('servicenow', expect.objectContaining({ baseUrl: 'https://10.0.10.60', username: 'soc', password: 'sn-pass' }));
  ...
  test('gesetzte URL ohne Benutzer → 400 (SERVICENOW_INCOMPLETE)', async () => {
    const res = await putConn(adminToken, { password: ADMIN_PW, baseUrl: 'https://10.0.10.60', username: '', servicenowPassword: 'x' });
    expect(res.status).toBe(400);
    expect(res.body.error).toBe('SERVICENOW_INCOMPLETE');
  });

  test('fehlendes Step-up-Passwort → 400', async () => {
    expect((await putConn(adminToken, { baseUrl: 'https://10.0.10.60', username: 'soc', servicenowPassword: 'x' })).status).toBe(400);
  });
...
describe('POST /api/v1/servicenow/connection/test', () => {
  ...
  test('sendet gespeichertes Passwort nicht an ein abweichendes Ziel', async () => {
    await putConn(adminToken, { password: ADMIN_PW, baseUrl: 'https://10.0.10.60', username: 'soc', servicenowPassword: 'stored-secret' });
    const res = await testConn(adminToken, { baseUrl: 'https://10.0.10.61', username: 'soc' });
    expect(res.status).toBe(400);
