...
describe('POST /api/v1/qradar/connection/test', () => {
  ...
  test('sendet gespeicherten Token nicht an ein abweichendes Ziel', async () => {
    await putConn(adminToken, { password: ADMIN_PW, baseUrl: 'https://10.0.10.60', token: 'stored-token' });
    const res = await testConn(adminToken, { baseUrl: 'https://10.0.10.61' });
    expect(res.status).toBe(400);
