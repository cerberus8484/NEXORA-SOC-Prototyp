...
jest.mock('../../src/integrations/http/internalUrlAllowlist', () => {
  ...
  const TEST_HOST = 'otrs.acme.local';
...
describe('PUT /api/v1/otrs/connection', () => {
  const VALID = { password: ADMIN_PW, baseUrl: 'https://otrs.acme.local', username: 'soc', otrsPassword: 'otrs-pass', queue: 'SOC' };
  ...
  test('admin + korrektes Passwort → 200, Passwort verschlüsselt, Adapter rekonfiguriert, Audit ohne Passwort', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.data).toMatchObject({ baseUrl: 'https://otrs.acme.local', username: 'soc', queue: 'SOC', passwordSet: true, source: 'db' });

    const stored = await createSettingsRepository().get(OTRS_CONNECTION_KEY);
    expect(isEncrypted(stored.password)).toBe(true);
    expect(spy).toHaveBeenCalledWith('otrs', expect.objectContaining({ baseUrl: 'https://otrs.acme.local', username: 'soc', password: 'otrs-pass' }));
  ...
  test('gesetzte URL ohne Benutzer → 400 (OTRS_INCOMPLETE)', async () => {
    const res = await putConn(adminToken, { password: ADMIN_PW, baseUrl: 'https://otrs.acme.local', username: '', otrsPassword: 'x' });
    expect(res.status).toBe(400);
    expect(res.body.error).toBe('OTRS_INCOMPLETE');
  });

  test('fehlendes Step-up-Passwort → 400', async () => {
    expect((await putConn(adminToken, { baseUrl: 'https://otrs.acme.local', username: 'soc', otrsPassword: 'x' })).status).toBe(400);
  });
