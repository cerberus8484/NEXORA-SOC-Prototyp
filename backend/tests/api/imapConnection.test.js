...
describe('POST /api/v1/imap/connection/test', () => {
  ...
  test('sendet gespeichertes Passwort nicht an ein abweichendes Ziel', async () => {
    await putConn(adminToken, { password: ADMIN_PW, host: '10.0.10.85', port: 993, user: 'soc@nexora', imapPassword: 'stored-secret', secure: true });
    const res = await testConn(adminToken, { host: '10.0.10.86', user: 'soc@nexora' });
    expect(res.status).toBe(400);
