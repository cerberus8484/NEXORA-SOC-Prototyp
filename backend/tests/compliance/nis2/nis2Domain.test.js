...
const { NIS2_CONTROLS, CONTROL_KEYS, CATALOG_VERSION, isKnownControlKey } = require('../../../src/compliance/nis2/nis2ControlCatalog');
...
describe('NIS2 Control Catalog', () => {
  test('liefert genau zehn Controls mit stabilen Keys und englischen UI-Texten', () => {
    ...
    expect(NIS2_CONTROLS[0].title).toBe('Risk analysis and information security policies');
    expect(CATALOG_VERSION).toBe('2026-06.2');
    expect(NIS2_CONTROLS.every((c) => !/[äöüß]/i.test(`${c.title} ${c.shortDescription}`))).toBe(true);
  });
